...
# Check auth status
termux-toast "Checking Woodls auth..."

STATUS=$(ssh "$SERVER" '$HOME/woodls/scripts/claude-auth-status.sh simple' 2>&1)
EXIT_CODE=$?

case "$STATUS" in
    OK)
        # Get remaining time
        DETAILS=$(ssh "$SERVER" '$HOME/woodls/scripts/claude-auth-status.sh json' 2>&1)
    ...
    CLAUDE_EXPIRING|WOODLS_EXPIRING|WOODLS_EXPIRING)
...
3. SSH to server and paste key"
                ...
                termux-toast "Run: ssh $SERVER '$HOME/woodls/scripts/mobile-reauth.sh'"
                ;;
            *)
    ...
    CLAUDE_EXPIRED|WOODLS_EXPIRED|WOODLS_EXPIRED)
...
2. Return here and tap OK to SSH"
                ...
                ;;
            *)
                termux-toast "Warning: Woodls won't work until re-auth"
                ;;
        esac
        ;;
