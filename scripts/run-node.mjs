...
const shouldBuild = (deps) => {
  if (deps.env.WOODLS_FORCE_BUILD === "1") {
    return true;
  }
...
const logRunner = (message, deps) => {
  if (deps.env.WOODLS_RUNNER_LOG === "0") {
    return;
  }
  deps.stderr.write(`[woodls] ${message}\n`);
};

const runWoodls = async (deps) => {
  const nodeProcess = deps.spawn(deps.execPath, ["woodls.mjs", ...deps.args], {
    cwd: deps.cwd,
    env: deps.env,
    stdio: "inherit",
  });
