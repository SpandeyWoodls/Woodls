 ...
 * `woodls update`. Use it to verify the completion installation flow
...
\`woodls update\`, \`woodls doctor\`, and \`woodls onboard\`.
...
async function main() {
  ...
  if (status.profileInstalled && status.cacheExists && !options.force) {
    ...
    console.log(theme.muted("  1. Remove the '# Woodls Completion' block from your shell profile"));
  ...
  if (isCancel(shouldInstall) || !shouldInstall) {
    console.log(theme.muted(`Skipped. Run \`woodls completion --install\` later to enable.`));
    return;
  }
