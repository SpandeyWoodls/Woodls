...
# Build and bundle Woodls into a minimal .app we can open.
# Outputs to dist/Woodls.app

ROOT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
APP_ROOT="$ROOT_DIR/dist/Woodls.app"
BUILD_ROOT="$ROOT_DIR/apps/macos/.build"
PRODUCT="Woodls"
BUNDLE_ID="${BUNDLE_ID:-ai.woodls.mac.debug}"
...
SPARKLE_FEED_URL="${SPARKLE_FEED_URL:-https://raw.githubusercontent.com/woodls/woodls/main/appcast.xml}"
AUTO_CHECKS=true
...
INFO_PLIST_SRC="$ROOT_DIR/apps/macos/Sources/Woodls/Resources/Info.plist"
...
/usr/libexec/PlistBuddy -c "Set :WoodlsBuildTimestamp ${BUILD_TS}" "$APP_ROOT/Contents/Info.plist" || true
/usr/libexec/PlistBuddy -c "Set :WoodlsGitCommit ${GIT_COMMIT}" "$APP_ROOT/Contents/Info.plist" || true
...
echo "🚚 Copying binary"
cp "$BIN_PRIMARY" "$APP_ROOT/Contents/MacOS/Woodls"
if [[ "${#BUILD_ARCHS[@]}" -gt 1 ]]; then
  BIN_INPUTS=()
  for arch in "${BUILD_ARCHS[@]}"; do
    BIN_INPUTS+=("$(bin_for_arch "$arch")")
  done
  /usr/bin/lipo -create "${BIN_INPUTS[@]}" -output "$APP_ROOT/Contents/MacOS/Woodls"
fi
chmod +x "$APP_ROOT/Contents/MacOS/Woodls"
# SwiftPM outputs ad-hoc signed binaries; strip the signature before install_name_tool to avoid warnings.
/usr/bin/codesign --remove-signature "$APP_ROOT/Contents/MacOS/Woodls" 2>/dev/null || true
...
cp "$ROOT_DIR/apps/macos/Sources/Woodls/Resources/Woodls.icns" "$APP_ROOT/Contents/Resources/Woodls.icns"

echo "📦 Copying device model resources"
rm -rf "$APP_ROOT/Contents/Resources/DeviceModels"
cp -R "$ROOT_DIR/apps/macos/Sources/Woodls/Resources/DeviceModels" "$APP_ROOT/Contents/Resources/DeviceModels"
...
echo "📦 Copying WoodlsKit resources"
WOODLSKIT_BUNDLE="$(build_path_for_arch "$PRIMARY_ARCH")/$BUILD_CONFIG/WoodlsKit_WoodlsKit.bundle"
if [ -d "$WOODLSKIT_BUNDLE" ]; then
  rm -rf "$APP_ROOT/Contents/Resources/WoodlsKit_WoodlsKit.bundle"
  cp -R "$WOODLSKIT_BUNDLE" "$APP_ROOT/Contents/Resources/WoodlsKit_WoodlsKit.bundle"
else
  echo "WARN: WoodlsKit resource bundle not found at $WOODLSKIT_BUNDLE (continuing)" >&2
fi
...
echo "⏹  Stopping any running Woodls"
killall -q Woodls 2>/dev/null || true
