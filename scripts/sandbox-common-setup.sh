...
BASE_IMAGE="${BASE_IMAGE:-woodls-sandbox:bookworm-slim}"
TARGET_IMAGE="${TARGET_IMAGE:-woodls-sandbox-common:bookworm-slim}"
...
If you want a clean re-create, remove old sandbox containers:
  docker rm -f \$(docker ps -aq --filter label=woodls.sandbox=1)
NOTE
