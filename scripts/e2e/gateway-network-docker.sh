...
IMAGE_NAME="woodls-gateway-network-e2e"

PORT="18789"
TOKEN="e2e-$(date +%s)-$$"
NET_NAME="woodls-net-e2e-$$"
GW_NAME="woodls-gateway-e2e-$$"
...
echo "Starting gateway container..."
	docker run --rm -d \
	  --name "$GW_NAME" \
	  --network "$NET_NAME" \
	  -e "WOODLS_GATEWAY_TOKEN=$TOKEN" \
	  -e "WOODLS_SKIP_CHANNELS=1" \
	  -e "WOODLS_SKIP_GMAIL_WATCHER=1" \
	  -e "WOODLS_SKIP_CRON=1" \
	  -e "WOODLS_SKIP_CANVAS_HOST=1" \
	  "$IMAGE_NAME" \
