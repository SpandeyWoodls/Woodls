...
INSTALL_URL="${WOODLS_INSTALL_URL:-https://woodls.bot/install.sh}"
SMOKE_PREVIOUS_VERSION="${WOODLS_INSTALL_SMOKE_PREVIOUS:-}"
SKIP_PREVIOUS="${WOODLS_INSTALL_SMOKE_SKIP_PREVIOUS:-0}"
DEFAULT_PACKAGE="woodls"
PACKAGE_NAME="${WOODLS_INSTALL_PACKAGE:-$DEFAULT_PACKAGE}"
...
if [[ "$SKIP_PREVIOUS" == "1" ]]; then
  echo "==> Skip preinstall previous (WOODLS_INSTALL_SMOKE_SKIP_PREVIOUS=1)"
else
...
if [[ -n "${WOODLS_INSTALL_LATEST_OUT:-}" ]]; then
  printf "%s" "$LATEST_VERSION" > "${WOODLS_INSTALL_LATEST_OUT:-}"
fi
