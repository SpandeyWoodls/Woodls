...
STATUS=$(ssh -o ConnectTimeout=5 "$SERVER" '$HOME/woodls/scripts/claude-auth-status.sh simple' 2>&1)
...
case "$STATUS" in
    ...
    *EXPIRED*|*MISSING*)
        ...
        sleep 2
        termux-notification -t "Woodls Re-Auth" -c "After getting key, run: ssh $SERVER '~/woodls/scripts/mobile-reauth.sh'" --id woodls-auth
        ;;
    *)
