...
#
# Suggested cron: */30 * * * * /home/admin/woodls/scripts/auth-monitor.sh
#
# Environment variables:
#   NOTIFY_PHONE - Phone number to send Woodls notification (e.g., +1234567890)
#   NOTIFY_NTFY  - ntfy.sh topic for push notifications (e.g., woodls-alerts)
...
STATE_FILE="$HOME/.woodls/auth-monitor-state"
...
send_notification() {
    ...
    # Send via Woodls if phone configured and auth still valid
    if [ -n "$NOTIFY_PHONE" ]; then
        # Check if we can still use woodls
        if "$SCRIPT_DIR/claude-auth-status.sh" simple 2>/dev/null | grep -q "OK\|EXPIRING"; then
            echo "Sending via Woodls to $NOTIFY_PHONE..."
            woodls send --to "$NOTIFY_PHONE" --message "$message" 2>/dev/null || true
        fi
    fi
...
if [ "$DIFF_MS" -lt 0 ]; then
    send_notification "Claude Code auth EXPIRED! Woodls is down. Run: ssh l36 '~/woodls/scripts/mobile-reauth.sh'" "urgent"
    exit 1
