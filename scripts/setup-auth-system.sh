#!/bin/bash
# Setup Woodls Auth Management System
...
echo "  2. Subscribe to a topic (e.g., 'woodls-alerts')"
echo ""
...
# Phone notification
echo ""
echo "  Woodls message: Send warning via Woodls itself"
echo "Enter your phone number for alerts (or leave blank to skip):"
read -r PHONE_NUMBER

# Update service file
SERVICE_FILE="$SCRIPT_DIR/systemd/woodls-auth-monitor.service"
...
cp "$SCRIPT_DIR/systemd/woodls-auth-monitor.service" ~/.config/systemd/user/
cp "$SCRIPT_DIR/systemd/woodls-auth-monitor.timer" ~/.config/systemd/user/
systemctl --user daemon-reload
systemctl --user enable --now woodls-auth-monitor.timer
...
echo "   scp $SCRIPT_DIR/termux-quick-auth.sh phone:~/.shortcuts/WoodlsAuth"
echo "   scp $SCRIPT_DIR/termux-auth-widget.sh phone:~/.shortcuts/WoodlsAuth-Full"
echo ""
echo "4. Make them executable on phone:"
echo "   ssh phone 'chmod +x ~/.shortcuts/Woodls*'"
echo ""
echo "5. Add Termux:Widget to your home screen"
echo "6. Tap the widget to see your auth scripts"
echo ""
echo "The quick widget (WoodlsAuth) shows status and opens auth URL if needed."
echo "The full widget (WoodlsAuth-Full) provides guided re-auth flow."
echo ""
...
echo "  - Auth monitor: systemctl --user status woodls-auth-monitor.timer"
echo ""
echo "Quick commands:"
