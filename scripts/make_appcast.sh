...
ZIP=${1:?"Usage: $0 Woodls-<ver>.zip"}
FEED_URL=${2:-"https://raw.githubusercontent.com/woodls/woodls/main/appcast.xml"}
...
if [[ -z "$VERSION" ]]; then
  if [[ "$ZIP_NAME" =~ ^Woodls-([0-9]+(\.[0-9]+){1,2}([-.][^.]*)?)\.zip$ ]]; then
...
DOWNLOAD_URL_PREFIX=${SPARKLE_DOWNLOAD_URL_PREFIX:-"https://github.com/woodls/woodls/releases/download/v${VERSION}/"}
