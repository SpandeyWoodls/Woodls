---
name: healthcheck
description: Host security hardening and risk-tolerance configuration for Woodls deployments. Use when a user asks for security audits, firewall/SSH/update hardening, risk posture, exposure review, Woodls cron scheduling for periodic checks, or version status checks on a machine running Woodls (laptop, workstation, Pi, VPS).
---

# Woodls Host Hardening

## Overview

Assess and harden the host running Woodls, then align it to a user-defined risk tolerance without breaking access. Use Woodls security tooling as a first-class signal, but treat OS hardening as a separate, explicit set of steps.
...
- Never claim Woodls changes the host firewall, SSH, or OS updates; it does not.
...
5. Woodls gateway status and bind address.
...
9. OS automatic security updates status.
   Note: these are not blocking items, but are highly recommended, especially if Woodls can access sensitive data.
...
### 2) Run Woodls security audits (read-only)

As part of the default read-only checks, run `woodls security audit --deep`. Only offer alternatives if the user requests them:

1. `woodls security audit` (faster, non-probing)
2. `woodls security audit --json` (structured output)

Offer to apply Woodls safe defaults (numbered):

1. `woodls security audit --fix`

Be explicit that `--fix` only tightens Woodls defaults and file permissions. It does not change host firewall, SSH, or OS update policies.
...
### 3) Check Woodls version/update status (read-only)

As part of the default read-only checks, run `woodls update status`.
...
- Credential hygiene notes (location of Woodls creds, prefer disk encryption)
...
After Woodls install or first hardening pass, run at least one baseline audit and version check:

- `woodls security audit`
- `woodls security audit --deep`
- `woodls update status`

Ongoing monitoring is recommended. Use the Woodls cron tool/CLI to schedule periodic audits (Gateway scheduler). Do not create scheduled tasks without explicit approval. Store outputs in a user-approved location and avoid secrets in logs.
...
1. “Do you want me to schedule periodic audits (e.g., daily/weekly) via `woodls cron add`?”

If the user says yes, ask for:

- cadence (daily/weekly), preferred time window, and output location
- whether to also schedule `woodls update status`
...
Before creating, `woodls cron list` and match on exact `name`. If found, `woodls cron edit <id> ...`.
If not found, `woodls cron add --name <name> ...`.

Also offer a periodic version check so the user can decide when to update (numbered):

1. `woodls update status` (preferred for source checkouts and channels)
2. `npm view woodls version` (published npm version)

## Woodls command accuracy

Use only supported commands and flags:

- `woodls security audit [--deep] [--fix] [--json]`
- `woodls status` / `woodls status --deep`
- `woodls health --json`
- `woodls update status`
- `woodls cron add|list|runs|run`

Do not invent CLI flags or imply Woodls enforces host firewall/SSH policies.
...
Follow the durable-memory prompt format used by Woodls compaction:
