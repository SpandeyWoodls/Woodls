...
function snapshotEnv(): EnvSnapshot {
  return {
    ...
    woodlsHome: process.env.WOODLS_HOME,
...
function restoreEnv(snapshot: EnvSnapshot) {
  ...
  restoreKey("WOODLS_HOME", snapshot.woodlsHome);
  restoreKey("WOODLS_STATE_DIR", snapshot.stateDir);
}
...
function setTempHome(base: string) {
  ...
  // Ensure tests using HOME isolation aren't affected by leaked WOODLS_HOME.
  delete process.env.WOODLS_HOME;
  process.env.WOODLS_STATE_DIR = path.join(base, ".woodls");
...
export async function withTempHome<T>(
  ...
  const base = await fs.mkdtemp(path.join(os.tmpdir(), opts.prefix ?? "woodls-test-home-"));
  ...
  setTempHome(base);
  await fs.mkdir(path.join(base, ".woodls", "agents", "main", "sessions"), { recursive: true });
  if (opts.env) {
