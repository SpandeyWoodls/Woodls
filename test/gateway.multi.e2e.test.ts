...
const spawnGatewayInstance = async (name: string): Promise<GatewayInstance> => {
  ...
  const homeDir = await fs.mkdtemp(path.join(os.tmpdir(), `woodls-e2e-${name}-`));
  const configDir = path.join(homeDir, ".woodls");
    ...
    child = spawn(
      ...
      ],
      {
        ...
        env: {
          ...
          WOODLS_SKIP_BROWSER_CONTROL_SERVER: "1",
