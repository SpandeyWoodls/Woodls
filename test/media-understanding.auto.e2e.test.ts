...
import type { WoodlsConfig } from "../src/config/config.js";
...
const makeTempMedia = async (ext: string) => {
  const dir = await makeTempDir("woodls-media-e2e-");
...
describe("media understanding auto-detect (e2e)", () => {
  ...
  it("uses sherpa-onnx-offline when available", async () => {
    ...
    try {
      const binDir = await makeTempDir("woodls-bin-sherpa-");
      const modelDir = await makeTempDir("woodls-sherpa-model-");
      ...
      const cfg: WoodlsConfig = { tools: { media: { audio: {} } } };
  ...
  it("uses whisper-cli when sherpa is missing", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("woodls-bin-whispercpp-");
      const modelDir = await makeTempDir("woodls-whispercpp-model-");
      tempPaths.push(binDir, modelDir);
  ...
  it("uses gemini CLI for images when available", async () => {
    const snapshot = envSnapshot();
    try {
      const binDir = await makeTempDir("woodls-bin-gemini-");
      ...
      const cfg: WoodlsConfig = { tools: { media: { image: {} } } };
