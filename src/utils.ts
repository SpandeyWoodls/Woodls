...
export function resolveConfigDir(
  ...
  const newDir = path.join(resolveRequiredHomeDir(env, homedir), ".woodls");
  try {
...
function resolveHomeDisplayPrefix(): { home: string; prefix: string } | undefined {
  ...
  const explicitHome = process.env.WOODLS_HOME?.trim();
  if (explicitHome) {
    return { home, prefix: "$WOODLS_HOME" };
  }
  return { home, prefix: "~" };
}
...
// Configuration root; can be overridden via WOODLS_STATE_DIR.
