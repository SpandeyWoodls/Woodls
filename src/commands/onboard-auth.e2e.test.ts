...
describe("writeOAuthCredentials", () => {
  ...
  it("writes auth-profiles.json under WOODLS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("woodls-oauth-");
...
describe("setMinimaxApiKey", () => {
  ...
  it("writes to WOODLS_AGENT_DIR when set", async () => {
    const env = await setupAuthTestEnv("woodls-minimax-", { agentSubdir: "custom-agent" });
    lifecycle.setStateDir(env.stateDir);
