...
describe("noteMacLaunchctlGatewayEnvOverrides", () => {
  it("prints clear unsetenv instructions for token override", async () => {
    ...
    const getenv = vi.fn(async (name: string) =>
      name === "WOODLS_GATEWAY_TOKEN" ? "launchctl-token" : undefined,
    );
    const cfg = {
      gateway: {
        auth: {
    ...
    expect(message).toContain("WOODLS_GATEWAY_TOKEN");
    expect(message).toContain("launchctl unsetenv WOODLS_GATEWAY_TOKEN");
    expect(message).not.toContain("WOODLS_GATEWAY_PASSWORD");
  });
