...
beforeEach(() => {
  originalAgentDir = process.env.WOODLS_AGENT_DIR;
  originalPiAgentDir = process.env.PI_CODING_AGENT_DIR;
  tempAgentDir = fs.mkdtempSync(path.join(os.tmpdir(), "woodls-auth-"));
...
afterEach(() => {
  if (originalAgentDir === undefined) {
    ...
    process.env.WOODLS_AGENT_DIR = originalAgentDir;
  }
...
describe("maybeRemoveDeprecatedCliAuthProfiles", () => {
  it("removes deprecated CLI auth profiles from store + config", async () => {
    ...
    const next = await maybeRemoveDeprecatedCliAuthProfiles(
      cfg as unknown as WoodlsConfig,
      makePrompter(true),
    );
