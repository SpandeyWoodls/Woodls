...
export const resolveWoodlsPackageRoot = vi.fn().mockResolvedValue(null) as unknown as MockFn;
...
vi.mock("../plugins/loader.js", () => ({
  loadWoodlsPlugins: () => ({ plugins: [], diagnostics: [] }),
}));
...
beforeEach(() => {
  ...
  resolveWoodlsPackageRoot.mockReset().mockResolvedValue(null);
  ...
  setStdinTty(true);
  originalStateDir = process.env.WOODLS_STATE_DIR;
  originalUpdateInProgress = process.env.WOODLS_UPDATE_IN_PROGRESS;
  process.env.WOODLS_UPDATE_IN_PROGRESS = "1";
  tempStateDir = fs.mkdtempSync(path.join(os.tmpdir(), "woodls-doctor-state-"));
...
afterEach(() => {
  ...
  if (originalStateDir === undefined) {
    ...
    process.env.WOODLS_STATE_DIR = originalStateDir;
  }
  if (originalUpdateInProgress === undefined) {
    delete process.env.WOODLS_UPDATE_IN_PROGRESS;
  } else {
    process.env.WOODLS_UPDATE_IN_PROGRESS = originalUpdateInProgress;
  }
  if (tempStateDir) {
