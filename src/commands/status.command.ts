...
export async function statusCommand(
  ...
  runtime.log(theme.heading("Woodls status"));
  runtime.log("");
  ...
  runtime.log(theme.muted(`Full report: ${formatCliCommand("woodls security audit")}`));
  runtime.log(theme.muted(`Deep probe: ${formatCliCommand("woodls security audit --deep")}`));
  ...
  runtime.log("");
  runtime.log("FAQ: https://docs.woodls.ai/faq");
  runtime.log("Troubleshooting: https://docs.woodls.ai/troubleshooting");
  runtime.log("");
  ...
  runtime.log(`  Need to share?      ${formatCliCommand("woodls status --all")}`);
  runtime.log(`  Need to debug live? ${formatCliCommand("woodls logs --follow")}`);
  if (gatewayReachable) {
    runtime.log(`  Need to test channels? ${formatCliCommand("woodls status --deep")}`);
  } else {
    runtime.log(`  Fix reachability first: ${formatCliCommand("woodls gateway probe")}`);
  }
}
