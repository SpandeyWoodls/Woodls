...
function stripUnknownConfigKeys(config: WoodlsConfig): {
  config: WoodlsConfig;
  removed: string[];
} {
  const parsed = WoodlsSchema.safeParse(config);
  if (parsed.success) {
...
function noteOpencodeProviderOverrides(cfg: WoodlsConfig) {
...
function scanTelegramAllowFromUsernameEntries(cfg: WoodlsConfig): TelegramAllowFromUsernameHit[] {
...
async function maybeRepairTelegramAllowFromUsernames(cfg: WoodlsConfig): Promise<{
  config: WoodlsConfig;
  changes: string[];
}> {
...
function scanDiscordNumericIdEntries(cfg: WoodlsConfig): DiscordNumericIdHit[] {
...
function maybeRepairDiscordNumericIds(cfg: WoodlsConfig): {
  config: WoodlsConfig;
  changes: string[];
} {
 ...
 */
function maybeRepairOpenPolicyAllowFrom(cfg: WoodlsConfig): {
  config: WoodlsConfig;
  changes: string[];
} {
...
async function maybeMigrateLegacyConfig(): Promise<string[]> {
  ...
  const targetDir = path.join(home, ".woodls");
  const targetPath = path.join(targetDir, "woodls.json");
  try {
  ...
  const legacyCandidates = [
    path.join(home, ".woodls", "woodls.json"),
...
export async function loadAndMaybeMigrateDoctorConfig(params: {
  ...
  if (snapshot.legacyIssues.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("woodls doctor --fix")}" to apply legacy migrations.`);
    }
  }
  ...
  if (normalized.changes.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push(`Run "${formatCliCommand("woodls doctor --fix")}" to apply these changes.`);
    }
  }
    ...
    if (hits.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("woodls doctor --fix")}" to auto-resolve @username entries to numeric IDs (requires a Telegram bot token).`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (discordHits.length > 0) {
      note(
        [
          ...
          `- Discord IDs must be strings; run "${formatCliCommand("woodls doctor --fix")}" to convert numeric IDs to quoted strings.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
    ...
    if (allowFromScan.changes.length > 0) {
      note(
        [
          ...
          `- Run "${formatCliCommand("woodls doctor --fix")}" to add missing allowFrom wildcards.`,
        ].join("\n"),
        "Doctor warnings",
      );
    }
  ...
  if (unknown.removed.length > 0) {
    ...
    if (shouldRepair) {
      ...
      fixHints.push('Run "woodls doctor --fix" to remove these keys.');
    }
  }
