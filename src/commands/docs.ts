...
const SEARCH_TOOL = "https://docs.woodls.ai/mcp.SearchWoodls";
...
export async function docsSearchCommand(queryParts: string[], runtime: RuntimeEnv) {
  ...
  if (!query) {
    const docs = formatDocsLink("/", "docs.woodls.ai");
    if (isRich()) {
      runtime.log(`${theme.muted("Docs:")} ${docs}`);
      runtime.log(`${theme.muted("Search:")} ${formatCliCommand('woodls docs "your query"')}`);
    } else {
      runtime.log("Docs: https://docs.woodls.ai/");
      runtime.log(`Search: ${formatCliCommand('woodls docs "your query"')}`);
    }
    return;
  }
