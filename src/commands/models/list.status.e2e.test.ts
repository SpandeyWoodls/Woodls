...
const mocks = vi.hoisted(() => {
  ...
  return {
    store,
    resolveWoodlsAgentDir: vi.fn().mockReturnValue("/tmp/woodls-agent"),
    resolveAgentDir: vi.fn().mockReturnValue("/tmp/woodls-agent"),
    ...
    resolveAuthStorePathForDisplay: vi.fn().mockReturnValue("/tmp/woodls-agent/auth-profiles.json"),
...
vi.mock("../../agents/agent-paths.js", () => ({
  resolveWoodlsAgentDir: mocks.resolveWoodlsAgentDir,
}));
...
async function withAgentScopeOverrides<T>(
  ...
  try {
    ...
    if (originalAgentDir) {
      ...
      mocks.resolveAgentDir.mockReturnValue("/tmp/woodls-agent");
    }
  }
}
...
describe("modelsStatusCommand auth overview", () => {
  it("includes masked auth sources in JSON output", async () => {
    ...
    expect(mocks.resolveWoodlsAgentDir).toHaveBeenCalled();
    expect(payload.defaultModel).toBe("anthropic/claude-opus-4-5");
    expect(payload.auth.storePath).toBe("/tmp/woodls-agent/auth-profiles.json");
    ...
    await withAgentScopeOverrides(
      {
        ...
        agentDir: "/tmp/woodls-agent-custom",
      },
      async () => {
        ...
        expect(payload.agentDir).toBe("/tmp/woodls-agent-custom");
