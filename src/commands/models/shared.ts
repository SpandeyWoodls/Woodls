...
import { type WoodlsConfig, readConfigFileSnapshot, writeConfigFile } from "../../config/config.js";
...
export async function loadValidConfigOrThrow(): Promise<WoodlsConfig> {
...
export async function updateConfig(
  mutator: (cfg: WoodlsConfig) => WoodlsConfig,
...
export function resolveModelTarget(params: { raw: string; cfg: WoodlsConfig }): {
  provider: string;
  model: string;
} {
...
export function buildAllowlistSet(cfg: WoodlsConfig): Set<string> {
...
export function resolveKnownAgentId(params: {
  ...
  if (!knownAgents.includes(agentId)) {
    throw new Error(
      `Unknown agent id "${raw}". Use "${formatCliCommand("woodls agents list")}" to see configured agents.`,
    );
  }
  return agentId;
}
