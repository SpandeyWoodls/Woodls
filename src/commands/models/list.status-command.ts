...
export async function modelsStatusCommand(
  ...
  const agentDir = agentId ? resolveAgentDir(cfg, agentId) : resolveWoodlsAgentDir();
  ...
  if (missingProvidersInUse.length > 0) {
    ...
    for (const provider of missingProvidersInUse) {
      const hint =
        provider === "anthropic"
          ? `Run \`claude setup-token\`, then \`${formatCliCommand("woodls models auth setup-token")}\` or \`${formatCliCommand("woodls configure")}\`.`
          : `Run \`${formatCliCommand("woodls configure")}\` or set an API key env var.`;
