...
function resolveAgentName(cfg: WoodlsConfig, agentId: string) {
...
function resolveAgentModel(cfg: WoodlsConfig, agentId: string) {
  const entry = listAgentEntries(cfg).find(
    (agent) => normalizeAgentId(agent.id) === normalizeAgentId(agentId),
  );
  if (entry?.model) {
...
export function buildAgentSummaries(cfg: WoodlsConfig): AgentSummary[] {
