...
function resolveMode(cfg: WoodlsConfig): "local" | "remote" {
...
export async function doctorCommand(
  ...
  const root = await resolveWoodlsPackageRoot({
  ...
  let cfg: WoodlsConfig = configResult.cfg;
  ...
  if (!cfg.gateway?.mode) {
    const lines = [
      ...
      `Fix: run ${formatCliCommand("woodls configure")} and set Gateway mode (local/remote).`,
      `Or set directly: ${formatCliCommand("woodls config set gateway.mode local")}`,
    ];
    if (!fs.existsSync(configPath)) {
      lines.push(`Missing config: run ${formatCliCommand("woodls setup")} first.`);
    }
    note(lines.join("\n"), "Gateway");
  }
  ...
  if (shouldWriteConfig) {
    ...
    runtime.log(`Run "${formatCliCommand("woodls doctor --fix")}" to apply changes.`);
  }
