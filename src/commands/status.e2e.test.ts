...
beforeAll(() => {
  envSnapshot = captureEnv(["WOODLS_PROFILE"]);
  process.env.WOODLS_PROFILE = "isolated";
});
...
vi.mock("../infra/woodls-root.js", () => ({
  resolveWoodlsPackageRoot: vi.fn().mockResolvedValue("/tmp/woodls"),
}));
...
vi.mock("../infra/update-check.js", () => ({
  checkUpdateStatus: vi.fn().mockResolvedValue({
    ...
    deps: {
      manager: "pnpm",
      status: "ok",
      lockfilePath: "/tmp/woodls/pnpm-lock.yaml",
      markerPath: "/tmp/woodls/node_modules/.modules.yaml",
    },
...
describe("statusCommand", () => {
  ...
  it("prints formatted lines otherwise", async () => {
    ...
    await statusCommand({}, runtime as never);
    const logs = runtimeLogMock.mock.calls.map((c: unknown[]) => String(c[0]));
    expect(logs.some((l: string) => l.includes("Woodls status"))).toBe(true);
    ...
    expect(
      logs.some(
        (l: string) =>
          l.includes("woodls status --all") ||
          l.includes("woodls --profile isolated status --all") ||
          l.includes("woodls status --all") ||
          l.includes("woodls --profile isolated status --all"),
      ),
    ).toBe(true);
  ...
  it("shows gateway auth when reachable", async () => {
    ...
    process.env.WOODLS_GATEWAY_TOKEN = "abcd1234";
    try {
