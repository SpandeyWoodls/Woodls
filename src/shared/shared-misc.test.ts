...
describe("shared/frontmatter", () => {
  ...
  test("resolveWoodlsManifestBlock parses JSON5 metadata and picks woodls block", () => {
    const frontmatter = {
      metadata: "{ woodls: { foo: 1, bar: 'baz' } }",
    };
    expect(resolveWoodlsManifestBlock({ frontmatter })).toEqual({ foo: 1, bar: "baz" });
  });

  test("resolveWoodlsManifestBlock returns undefined for invalid input", () => {
    expect(resolveWoodlsManifestBlock({ frontmatter: {} })).toBeUndefined();
    expect(resolveWoodlsManifestBlock({ frontmatter: { metadata: "not-json5" } })).toBeUndefined();
    expect(
      resolveWoodlsManifestBlock({ frontmatter: { metadata: "{ nope: { a: 1 } }" } }),
    ).toBeUndefined();
  });
});
