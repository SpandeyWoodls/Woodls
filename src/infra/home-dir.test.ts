...
describe("resolveEffectiveHomeDir", () => {
  it("prefers WOODLS_HOME over HOME and USERPROFILE", () => {
    const env = {
    ...
    expect(resolveEffectiveHomeDir(env, () => "/fallback")).toBe(path.resolve("/srv/woodls-home"));
  });
  ...
  it("expands WOODLS_HOME when set to ~", () => {
    const env = {
      WOODLS_HOME: "~/svc",
      HOME: "/home/alice",
    } as NodeJS.ProcessEnv;
...
describe("resolveRequiredHomeDir", () => {
  ...
  it("returns a fully resolved path for WOODLS_HOME", () => {
    const result = resolveRequiredHomeDir(
      { WOODLS_HOME: "/custom/home" } as NodeJS.ProcessEnv,
      () => "/fallback",
    );
    expect(result).toBe(path.resolve("/custom/home"));
  });

  it("returns cwd when WOODLS_HOME is tilde-only and no fallback home exists", () => {
    expect(
      resolveRequiredHomeDir({ WOODLS_HOME: "~" } as NodeJS.ProcessEnv, () => {
        throw new Error("no home");
      }),
    ).toBe(process.cwd());
  });
...
describe("expandHomePrefix", () => {
  it("expands tilde using effective home", () => {
    const value = expandHomePrefix("~/x", {
      env: { WOODLS_HOME: "/srv/woodls-home" } as NodeJS.ProcessEnv,
    });
    expect(value).toBe(`${path.resolve("/srv/woodls-home")}/x`);
  });
