...
export function resolveWideAreaDiscoveryDomain(params?: {
  ...
  const candidate = params?.configDomain ?? env.WOODLS_WIDE_AREA_DOMAIN ?? null;
...
function extractContentHash(zoneText: string): string | null {
  const match = zoneText.match(/^\s*;\s*woodls-content-hash:\s*(\S+)\s*$/m);
  return match?.[1] ?? null;
}
...
function renderZone(opts: WideAreaGatewayZoneOpts & { serial: number }): string {
  const hostname = os.hostname().split(".")[0] ?? "woodls";
  const hostLabel = dnsLabel(opts.hostLabel ?? hostname, "woodls");
  const instanceLabel = dnsLabel(opts.instanceLabel ?? `${hostname}-gateway`, "woodls-gw");
  ...
  records.push(`_woodls-gw._tcp IN PTR ${instanceLabel}._woodls-gw._tcp`);
  records.push(`${instanceLabel}._woodls-gw._tcp IN SRV 0 0 ${opts.gatewayPort} ${hostLabel}`);
  records.push(`${instanceLabel}._woodls-gw._tcp IN TXT ${txt.map(txtQuote).join(" ")}`);
  ...
  return `; woodls-content-hash: ${contentHash}\n${contentBody}`;
}
