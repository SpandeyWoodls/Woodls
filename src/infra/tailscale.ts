...
export async function getTailscaleBinary(): Promise<string> {
  const forcedBinary = process.env.WOODLS_TEST_TAILSCALE_BINARY?.trim();
  if (forcedBinary) {
...
export async function ensureFunnel(
  ...
  try {
    ...
    runtime.error(
      info(
        `Tip: Funnel is optional for Woodls. You can keep running the web gateway without it: \`${formatCliCommand("woodls gateway")}\``,
      ),
    );
    if (shouldLogVerbose()) {
      const rich = isRich();
