...
describe("tailscale helpers", () => {
  const originalForcedBinary = process.env.WOODLS_TEST_TAILSCALE_BINARY;

  beforeEach(() => {
    process.env.WOODLS_TEST_TAILSCALE_BINARY = "tailscale";
  });

  afterEach(() => {
    if (originalForcedBinary === undefined) {
      delete process.env.WOODLS_TEST_TAILSCALE_BINARY;
    } else {
      process.env.WOODLS_TEST_TAILSCALE_BINARY = originalForcedBinary;
    }
    vi.restoreAllMocks();
  });
