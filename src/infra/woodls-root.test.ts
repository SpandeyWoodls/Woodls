...
const VITEST_FS_BASE = path.join(path.parse(process.cwd()).root, "__woodls_vitest__");
const FIXTURE_BASE = path.join(VITEST_FS_BASE, "woodls-root");
...
describe("resolveWoodlsPackageRoot", () => {
  beforeEach(() => {
    state.entries.clear();
  ...
  it("resolves package root from .bin argv1", async () => {
    const { resolveWoodlsPackageRootSync } = await import("./woodls-root.js");

    const project = fx("bin-scenario");
    const argv1 = path.join(project, "node_modules", ".bin", "woodls");
    const pkgRoot = path.join(project, "node_modules", "woodls");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "woodls" }));

    expect(resolveWoodlsPackageRootSync({ argv1 })).toBe(pkgRoot);
  });
  ...
  it("resolves package root via symlinked argv1", async () => {
    ...
    const bin = path.join(project, "bin", "woodls");
    const realPkg = path.join(project, "real-pkg");
    state.realpaths.set(abs(bin), abs(path.join(realPkg, "woodls.mjs")));
    setFile(path.join(realPkg, "package.json"), JSON.stringify({ name: "woodls" }));

    expect(resolveWoodlsPackageRootSync({ argv1: bin })).toBe(realPkg);
  });
  ...
  it("prefers moduleUrl candidates", async () => {
    ...
    expect(resolveWoodlsPackageRootSync({ moduleUrl })).toBe(pkgRoot);
  });

  it("returns null for non-woodls package roots", async () => {
    const { resolveWoodlsPackageRootSync } = await import("./woodls-root.js");

    const pkgRoot = fx("not-woodls");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "not-woodls" }));

    expect(resolveWoodlsPackageRootSync({ cwd: pkgRoot })).toBeNull();
  });

  it("async resolver matches sync behavior", async () => {
    const { resolveWoodlsPackageRoot } = await import("./woodls-root.js");

    const pkgRoot = fx("async");
    setFile(path.join(pkgRoot, "package.json"), JSON.stringify({ name: "woodls" }));

    await expect(resolveWoodlsPackageRoot({ cwd: pkgRoot })).resolves.toBe(pkgRoot);
  });
});
