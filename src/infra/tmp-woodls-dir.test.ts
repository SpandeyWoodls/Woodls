...
import { POSIX_WOODLS_TMP_DIR, resolvePreferredWoodlsTmpDir } from "./tmp-woodls-dir.js";

type TmpDirOptions = NonNullable<Parameters<typeof resolvePreferredWoodlsTmpDir>[0]>;

function fallbackTmp(uid = 501) {
  return path.join("/var/fallback", `woodls-${uid}`);
}
...
function resolveWithMocks(params: {
  ...
  const resolved = resolvePreferredWoodlsTmpDir({
    accessSync,
...
describe("resolvePreferredWoodlsTmpDir", () => {
  it("prefers /tmp/woodls when it already exists and is writable", () => {
    ...
    expect(resolved).toBe(POSIX_WOODLS_TMP_DIR);
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("prefers /tmp/woodls when it does not exist but /tmp is writable", () => {
    ...
    expect(mkdirSync).toHaveBeenCalledWith(POSIX_WOODLS_TMP_DIR, expect.any(Object));
    expect(tmpdir).not.toHaveBeenCalled();
  });

  it("falls back to os.tmpdir()/woodls when /tmp/woodls is not a directory", () => {
  ...
  it("falls back to os.tmpdir()/woodls when /tmp is not writable", () => {
  ...
  it("falls back when /tmp/woodls is a symlink", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => true,
      uid: 501,
  ...
  it("falls back when /tmp/woodls is not owned by the current user", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 0,
  ...
  it("falls back when /tmp/woodls is group/other writable", () => {
    const lstatSync = vi.fn(() => ({
      isDirectory: () => true,
      isSymbolicLink: () => false,
      uid: 501,
