...
function candidateBinDirs(opts: EnsureWoodlsPathOpts): { prepend: string[]; append: string[] } {
  ...
  // Bundled macOS app: `woodls` lives next to the executable (process.execPath).
  try {
    const execDir = path.dirname(execPath);
    const siblingCli = path.join(execDir, "woodls");
  ...
  const allowProjectLocalBin =
    ...
    isTruthyEnvValue(process.env.WOODLS_ALLOW_PROJECT_LOCAL_BIN);
  if (allowProjectLocalBin) {
    const localBinDir = path.join(cwd, "node_modules", ".bin");
    if (isExecutable(path.join(localBinDir, "woodls"))) {
...
/**
 * Best-effort PATH bootstrap so skills that require the `woodls` CLI can run
 * under launchd/minimal environments (and inside the macOS app bundle).
 */
export function ensureWoodlsCliOnPath(opts: EnsureWoodlsPathOpts = {}) {
  if (isTruthyEnvValue(process.env.WOODLS_PATH_BOOTSTRAPPED)) {
    return;
  }
