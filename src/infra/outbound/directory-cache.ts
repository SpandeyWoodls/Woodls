...
export class DirectoryCache<T> {
  ...
  private lastConfigRef: WoodlsConfig | null = null;
  ...
  get(key: string, cfg: WoodlsConfig): T | undefined {
  ...
  set(key: string, value: T, cfg: WoodlsConfig): void {
    this.resetIfConfigChanged(cfg);
    const now = Date.now();
  ...
  private resetIfConfigChanged(cfg: WoodlsConfig): void {
