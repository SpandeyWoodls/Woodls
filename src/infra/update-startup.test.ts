...
describe("update-startup", () => {
  ...
  let resolveWoodlsPackageRoot: (typeof import("./woodls-root.js"))["resolveWoodlsPackageRoot"];
  ...
  beforeAll(async () => {
    suiteRoot = await fs.mkdtemp(path.join(os.tmpdir(), "woodls-update-check-suite-"));
  });
  ...
  beforeEach(async () => {
    ...
    hadStateDir = Object.prototype.hasOwnProperty.call(process.env, "WOODLS_STATE_DIR");
    prevStateDir = process.env.WOODLS_STATE_DIR;
    process.env.WOODLS_STATE_DIR = tempDir;
    ...
    if (!loaded) {
      ({ resolveWoodlsPackageRoot } = await import("./woodls-root.js"));
  ...
  async function runUpdateCheckAndReadState(channel: "stable" | "beta") {
    vi.mocked(resolveWoodlsPackageRoot).mockResolvedValue("/opt/woodls");
