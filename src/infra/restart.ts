...
export function triggerWoodlsRestart(): RestartAttempt {
  ...
  if (process.platform !== "darwin") {
    if (process.platform === "linux") {
      const unit = normalizeSystemdUnit(
        process.env.WOODLS_SYSTEMD_UNIT,
        process.env.WOODLS_PROFILE,
      );
  ...
  const label =
    process.env.WOODLS_LAUNCHD_LABEL || resolveGatewayLaunchAgentLabel(process.env.WOODLS_PROFILE);
