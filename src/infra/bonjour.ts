...
function isDisabledByEnv() {
  if (isTruthyEnvValue(process.env.WOODLS_DISABLE_BONJOUR)) {
    return true;
  }
...
function safeServiceName(name: string) {
  ...
  return trimmed.length > 0 ? trimmed : "Woodls";
}

function prettifyInstanceName(name: string) {
  const normalized = name.trim().replace(/\s+/g, " ");
  return normalized.replace(/\s+\(Woodls\)\s*$/i, "").trim() || normalized;
}
...
export async function startGatewayBonjourAdvertiser(
  ...
  const hostnameRaw =
    process.env.WOODLS_MDNS_HOSTNAME?.trim() ||
    process.env.WOODLS_MDNS_HOSTNAME?.trim() ||
    "woodls";
  const hostname =
    hostnameRaw
