...
const DEFAULT_PACKAGE_NAME = "woodls";
...
export async function runGatewayUpdate(opts: UpdateRunnerOptions = {}): Promise<UpdateRunResult> {
  ...
  if (gitRoot && pkgRoot && path.resolve(gitRoot) === path.resolve(pkgRoot)) {
    ...
    if (channel === "dev") {
      ...
      const preflightRoot = await fs.mkdtemp(path.join(os.tmpdir(), "woodls-update-preflight-"));
    ...
    const doctorEntry = path.join(gitRoot, "woodls.mjs");
    ...
    const doctorStep = await runStep(
      step("woodls doctor", doctorArgv, gitRoot, { WOODLS_UPDATE_IN_PROGRESS: "1" }),
    );
