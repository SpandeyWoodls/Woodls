...
function makeTempDir() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "woodls-exec-approvals-"));
}
...
describe("resolve exec approvals defaults", () => {
  it("expands home-prefixed default file and socket paths", () => {
    ...
    const prevWoodlsHome = process.env.WOODLS_HOME;
    try {
      process.env.WOODLS_HOME = dir;
      expect(path.normalize(resolveExecApprovalsPath())).toBe(
        path.normalize(path.join(dir, ".woodls", "exec-approvals.json")),
      );
      expect(path.normalize(resolveExecApprovalsSocketPath())).toBe(
        path.normalize(path.join(dir, ".woodls", "exec-approvals.sock")),
      );
    } finally {
      if (prevWoodlsHome === undefined) {
        delete process.env.WOODLS_HOME;
      } else {
        process.env.WOODLS_HOME = prevWoodlsHome;
      }
    }
  });
});
...
describe("exec approvals wildcard agent", () => {
  it("merges wildcard allowlist entries with agent entries", () => {
    ...
    try {
      process.env.WOODLS_HOME = dir;
      const approvalsPath = path.join(dir, ".woodls", "exec-approvals.json");
