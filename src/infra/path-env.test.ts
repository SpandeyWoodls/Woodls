...
let ensureWoodlsCliOnPath: typeof import("./path-env.js").ensureWoodlsCliOnPath;

describe("ensureWoodlsCliOnPath", () => {
  const envKeys = [
    "PATH",
    "WOODLS_PATH_BOOTSTRAPPED",
    "WOODLS_ALLOW_PROJECT_LOCAL_BIN",
    "MISE_DATA_DIR",
    "HOMEBREW_PREFIX",
    "HOMEBREW_BREW_FILE",
    "XDG_BIN_HOME",
  ...
  beforeAll(async () => {
    ({ ensureWoodlsCliOnPath } = await import("./path-env.js"));
  });
  ...
  it("prepends the bundled app bin dir when a sibling woodls exists", () => {
    const tmp = abs("/tmp/woodls-path/case-bundled");
    const appBinDir = path.join(tmp, "AppBin");
    const cliPath = path.join(appBinDir, "woodls");
    setDir(tmp);
    setDir(appBinDir);
    setExe(cliPath);

    process.env.PATH = "/usr/bin";
    delete process.env.WOODLS_PATH_BOOTSTRAPPED;

    ensureWoodlsCliOnPath({
      execPath: cliPath,
      cwd: tmp,
      homeDir: tmp,
      platform: "darwin",
  ...
  it("is idempotent", () => {
    ...
    process.env.WOODLS_PATH_BOOTSTRAPPED = "1";
    ensureWoodlsCliOnPath({
  ...
  it("prepends mise shims when available", () => {
    const tmp = abs("/tmp/woodls-path/case-mise");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "woodls");
    setDir(tmp);
    setDir(appBinDir);
    setExe(appCli);
  ...
  it("only appends project-local node_modules/.bin when explicitly enabled", () => {
    const tmp = abs("/tmp/woodls-path/case-project-local");
    const appBinDir = path.join(tmp, "AppBin");
    const appCli = path.join(appBinDir, "woodls");
    setDir(tmp);
    setDir(appBinDir);
    ...
    const localCli = path.join(localBinDir, "woodls");
  ...
  it("prepends Linuxbrew dirs when present", () => {
    const tmp = abs("/tmp/woodls-path/case-linuxbrew");
