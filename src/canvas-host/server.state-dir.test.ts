...
describe("canvas host state dir defaults", () => {
  ...
  it("uses WOODLS_STATE_DIR for the default canvas root", async () => {
    const tempRoot = await fs.mkdtemp(path.join(os.tmpdir(), "woodls-canvas-state-"));
    ...
    try {
      ...
      expect(indexContents).toContain("Woodls Canvas");
    } finally {
      await handler.close();
