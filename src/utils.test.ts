...
describe("ensureDir", () => {
  it("creates nested directory", async () => {
    const tmp = await fs.promises.mkdtemp(path.join(os.tmpdir(), "woodls-test-"));
...
describe("jidToE164", () => {
  ...
  it("maps @lid from authDir mapping files", () => {
    const authDir = fs.mkdtempSync(path.join(os.tmpdir(), "woodls-auth-"));
  ...
  it("falls back through lidMappingDirs in order", () => {
    const first = fs.mkdtempSync(path.join(os.tmpdir(), "woodls-lid-a-"));
    const second = fs.mkdtempSync(path.join(os.tmpdir(), "woodls-lid-b-"));
...
describe("resolveConfigDir", () => {
  it("prefers ~/.woodls when legacy dir is missing", async () => {
    const root = await fs.promises.mkdtemp(path.join(os.tmpdir(), "woodls-config-dir-"));
    try {
...
describe("resolveHomeDir", () => {
  it("prefers WOODLS_HOME over HOME", () => {
    vi.stubEnv("WOODLS_HOME", "/srv/woodls-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveHomeDir()).toBe(path.resolve("/srv/woodls-home"));
...
describe("shortenHomePath", () => {
  it("uses $WOODLS_HOME prefix when WOODLS_HOME is set", () => {
    vi.stubEnv("WOODLS_HOME", "/srv/woodls-home");
    vi.stubEnv("HOME", "/home/other");

    expect(shortenHomePath(`${path.resolve("/srv/woodls-home")}/.woodls/woodls.json`)).toBe(
      "$WOODLS_HOME/.woodls/woodls.json",
    );
...
describe("shortenHomeInString", () => {
  it("uses $WOODLS_HOME replacement when WOODLS_HOME is set", () => {
    vi.stubEnv("WOODLS_HOME", "/srv/woodls-home");
    vi.stubEnv("HOME", "/home/other");

    expect(
      shortenHomeInString(`config: ${path.resolve("/srv/woodls-home")}/.woodls/woodls.json`),
    ).toBe("config: $WOODLS_HOME/.woodls/woodls.json");
...
describe("resolveUserPath", () => {
  ...
  it("expands ~/ to home dir", () => {
    expect(resolveUserPath("~/woodls")).toBe(path.resolve(os.homedir(), "woodls"));
  });
  ...
  it("prefers WOODLS_HOME for tilde expansion", () => {
    vi.stubEnv("WOODLS_HOME", "/srv/woodls-home");
    vi.stubEnv("HOME", "/home/other");

    expect(resolveUserPath("~/woodls")).toBe(path.resolve("/srv/woodls-home", "woodls"));
