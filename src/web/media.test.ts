...
beforeAll(async () => {
  fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "woodls-media-test-"));
...
describe("web media loading", () => {
  beforeAll(() => {
    // Ensure state dir is stable and not influenced by other tests that stub WOODLS_STATE_DIR.
    // Also keep it outside os.tmpdir() so tmpdir localRoots doesn't accidentally make all state readable.
    stateDirSnapshot = captureEnv(["WOODLS_STATE_DIR"]);
    process.env.WOODLS_STATE_DIR = path.join(
      path.parse(os.tmpdir()).root,
      "var",
      "lib",
      "woodls-media-state-test",
    );
  });
  ...
  beforeAll(() => {
    const fakeLookup: ssrf.LookupFn = async (_hostname, _opts) => {
      return [{ address: "93.184.216.34", family: 4 }];
    };
    const realResolvePinnedHostnameWithPolicy = ssrf.resolvePinnedHostnameWithPolicy;
    vi.spyOn(ssrf, "resolvePinnedHostname").mockImplementation(async (hostname) =>
      realResolvePinnedHostnameWithPolicy(hostname, { lookupFn: fakeLookup }),
    );
    vi.spyOn(ssrf, "resolvePinnedHostnameWithPolicy").mockImplementation(async (hostname, params) =>
      realResolvePinnedHostnameWithPolicy(hostname, { ...params, lookupFn: fakeLookup }),
    );
  });
...
describe("local media root guard", () => {
  ...
  it("allows default Woodls state workspace and sandbox roots", async () => {
  ...
  it("rejects default Woodls state per-agent workspace-* roots without explicit local roots", async () => {
    const { resolveStateDir } = await import("../config/paths.js");
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));

    await expect(
      loadWebMedia(path.join(stateDir, "workspace-woodlsy", "tmp", "render.bin"), {
        maxBytes: 1024 * 1024,
        readFile,
      }),
    ).rejects.toThrow(/not under an allowed directory/i);
  ...
  it("allows per-agent workspace-* paths with explicit local roots", async () => {
    ...
    const stateDir = resolveStateDir();
    const readFile = vi.fn(async () => Buffer.from("generated-media"));
    const agentWorkspaceDir = path.join(stateDir, "workspace-woodlsy");
