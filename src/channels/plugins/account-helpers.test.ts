...
function cfg(accounts?: Record<string, unknown> | null): WoodlsConfig {
  if (accounts === null) {
    return { channels: { testchannel: {} } } as unknown as WoodlsConfig;
  }
  if (accounts === undefined) {
    return {} as unknown as WoodlsConfig;
  }
  return { channels: { testchannel: { accounts } } } as unknown as WoodlsConfig;
}

describe("createAccountListHelpers", () => {
  describe("listConfiguredAccountIds", () => {
    it("returns empty for missing config", () => {
      expect(listConfiguredAccountIds({} as WoodlsConfig)).toEqual([]);
    });
  ...
  describe("listAccountIds", () => {
    it('returns ["default"] for empty config', () => {
      expect(listAccountIds({} as WoodlsConfig)).toEqual(["default"]);
    });
  ...
  describe("resolveDefaultAccountId", () => {
    ...
    it('returns "default" for empty config', () => {
      expect(resolveDefaultAccountId({} as WoodlsConfig)).toBe("default");
    });
  });
});
