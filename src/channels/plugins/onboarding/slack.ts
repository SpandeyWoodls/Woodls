...
function setSlackDmPolicy(cfg: WoodlsConfig, dmPolicy: DmPolicy) {
...
function buildSlackManifest(botName: string) {
  const safeName = botName.trim() || "Woodls";
  const manifest = {
    display_information: {
      name: safeName,
      description: `${safeName} connector for Woodls`,
    },
    features: {
      bot_user: {
...
function setSlackAllowFrom(cfg: WoodlsConfig, allowFrom: string[]): WoodlsConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
