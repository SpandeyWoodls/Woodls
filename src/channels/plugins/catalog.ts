...
import { discoverWoodlsPlugins } from "../../plugins/discovery.js";
import type { WoodlsPackageManifest } from "../../plugins/manifest.js";
...
type ExternalCatalogEntry = {
  ...
  version?: string;
  description?: string;
} & Partial<Record<ManifestKey, WoodlsPackageManifest>>;
...
const ENV_CATALOG_PATHS = ["WOODLS_PLUGIN_CATALOG_PATHS", "WOODLS_MPM_CATALOG_PATHS"];
...
function toChannelMeta(params: {
  channel: NonNullable<WoodlsPackageManifest["channel"]>;
  id: string;
}): ChannelMeta | null {
...
function resolveInstallInfo(params: {
  manifest: WoodlsPackageManifest;
  packageName?: string;
  packageDir?: string;
  workspaceDir?: string;
...
function buildCatalogEntry(candidate: {
  ...
  packageDir?: string;
  workspaceDir?: string;
  packageManifest?: WoodlsPackageManifest;
...
export function listChannelPluginCatalogEntries(
  ...
  const discovery = discoverWoodlsPlugins({ workspaceDir: options.workspaceDir });
