...
function telegramCfg(): WoodlsConfig {
  return { channels: { telegram: { botToken: "tok" } } } as WoodlsConfig;
}

function slackHarness() {
  const cfg = { channels: { slack: { botToken: "tok" } } } as WoodlsConfig;
...
describe("discord message actions", () => {
  it("lists channel and upload actions by default", async () => {
    const cfg = { channels: { discord: { token: "d0" } } } as WoodlsConfig;
...
// Signal channel has been removed from the codebase.
