...
function listAgents(cfg: WoodlsConfig) {
  const agents = cfg.agents?.list;
  return Array.isArray(agents) ? agents : [];
}

function pickFirstExistingAgentId(cfg: WoodlsConfig, agentId: string): string {
...
type EvaluatedBindingsCache = {
  bindingsRef: WoodlsConfig["bindings"];
  byChannelAccount: Map<string, EvaluatedBinding[]>;
};

const evaluatedBindingsCacheByCfg = new WeakMap<WoodlsConfig, EvaluatedBindingsCache>();
