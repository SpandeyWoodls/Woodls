...
import type { WoodlsConfig, ConfigValidationIssue } from "./types.js";
import { WoodlsSchema } from "./zod-schema.js";
...
function validateIdentityAvatar(config: WoodlsConfig): ConfigValidationIssue[] {
...
export function validateConfigObjectRaw(
  raw: unknown,
): { ok: true; config: WoodlsConfig } | { ok: false; issues: ConfigValidationIssue[] } {
  ...
  const validated = WoodlsSchema.safeParse(raw);
  ...
  const duplicates = findDuplicateAgentDirs(validated.data as WoodlsConfig);
  ...
  const avatarIssues = validateIdentityAvatar(validated.data as WoodlsConfig);
  if (avatarIssues.length > 0) {
    return { ok: false, issues: avatarIssues };
  }
  return {
    ok: true,
    config: validated.data as WoodlsConfig,
  };
}
