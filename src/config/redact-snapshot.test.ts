...
import type { ConfigFileSnapshot } from "./types.woodls.js";
...
function makeSnapshot<TConfig extends Record<string, unknown>>(
  ...
  return {
    path: "/home/user/.woodls/config.json5",
    exists: true,
...
describe("redactConfigSnapshot", () => {
  ...
  it("does not redact passwordFile path fields", () => {
    const snapshot = makeSnapshot({
      channels: {
        irc: {
          passwordFile: "/etc/woodls/irc-password.txt",
          nickserv: {
            passwordFile: "/etc/woodls/nickserv-password.txt",
    ...
    expect(irc.passwordFile).toBe("/etc/woodls/irc-password.txt");
    expect(nickserv.passwordFile).toBe("/etc/woodls/nickserv-password.txt");
...
describe("realredactConfigSnapshot_real", () => {
  it("main schema redact works (samples)", () => {
    const schema = WoodlsSchema.toJSONSchema({
      target: "draft-07",
      unrepresentable: "any",
    });
    schema.title = "WoodlsConfig";
    const hints = mapSensitivePaths(WoodlsSchema, "", {});
