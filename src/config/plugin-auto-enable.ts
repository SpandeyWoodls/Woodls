...
function isTelegramConfigured(cfg: WoodlsConfig, env: NodeJS.ProcessEnv): boolean {
...
function isDiscordConfigured(cfg: WoodlsConfig, env: NodeJS.ProcessEnv): boolean {
...
function isSlackConfigured(cfg: WoodlsConfig, env: NodeJS.ProcessEnv): boolean {
  if (
...
function isWhatsAppConfigured(cfg: WoodlsConfig): boolean {
...
function isGenericChannelConfigured(cfg: WoodlsConfig, channelId: string): boolean {
...
function collectModelRefs(cfg: WoodlsConfig): string[] {
  const refs: string[] = [];
...
function isProviderConfigured(cfg: WoodlsConfig, providerId: string): boolean {
...
function resolveConfiguredPlugins(cfg: WoodlsConfig, env: NodeJS.ProcessEnv): PluginEnableChange[] {
...
function isPluginExplicitlyDisabled(cfg: WoodlsConfig, pluginId: string): boolean {
  const entry = cfg.plugins?.entries?.[pluginId];
  return entry?.enabled === false;
}

function isPluginDenied(cfg: WoodlsConfig, pluginId: string): boolean {
...
function registerPluginEntry(cfg: WoodlsConfig, pluginId: string): WoodlsConfig {
  const entries = {
    ...cfg.plugins?.entries,
    [pluginId]: {
