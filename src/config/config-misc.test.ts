...
describe("$schema key in config (#14998)", () => {
  it("accepts config with $schema string", () => {
    const result = WoodlsSchema.safeParse({
      $schema: "https://woodls.ai/config.json",
    });
    expect(result.success).toBe(true);
    if (result.success) {
      expect(result.data.$schema).toBe("https://woodls.ai/config.json");
    }
  });
  ...
  it("rejects non-string $schema", () => {
    const result = WoodlsSchema.safeParse({ $schema: 123 });
...
describe("cron webhook schema", () => {
  it("accepts cron.webhookToken and legacy cron.webhook", () => {
    const res = WoodlsSchema.safeParse({
      cron: {
        enabled: true,
