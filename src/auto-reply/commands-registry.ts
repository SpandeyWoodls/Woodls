...
export function isCommandEnabled(cfg: WoodlsConfig, commandKey: string): boolean {
...
function resolveDefaultCommandContext(cfg?: WoodlsConfig): {
  provider: string;
  model: string;
} {
  const resolved = resolveConfiguredModelRef({
    cfg: cfg ?? ({} as WoodlsConfig),
...
export function getCommandDetection(_cfg?: WoodlsConfig): CommandDetection {
  const commands = getChatCommands();
...
export function maybeResolveTextAlias(raw: string, cfg?: WoodlsConfig) {
