...
describe("stageSandboxMedia", () => {
  it("stages inbound media into the sandbox workspace", async () => {
    await withSandboxMediaTempHome("woodls-triggers-", async (home) => {
      const inboundDir = join(home, ".woodls", "media", "inbound");
  ...
  it("rejects staging host files from outside the media directory", async () => {
    await withSandboxMediaTempHome("woodls-triggers-bypass-", async (home) => {
      // Sensitive host file outside .woodls
