...
import { scheduleGatewaySigusr1Restart, triggerWoodlsRestart } from "../../infra/restart.js";
...
export const handleRestartCommand: CommandHandler = async (params, allowTextCommands) => {
  ...
  if (hasSigusr1Listener) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: "⚙️ Restarting Woodls in-process (SIGUSR1); back in a few seconds.",
      },
    };
  }
  const restartMethod = triggerWoodlsRestart();
  if (!restartMethod.ok) {
  ...
  return {
    shouldContinue: false,
    reply: {
      text: `⚙️ Restarting Woodls via ${restartMethod.method}; give me a few seconds to come back online.`,
    },
  };
};
