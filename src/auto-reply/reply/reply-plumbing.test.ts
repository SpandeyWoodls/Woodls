...
describe("buildThreadingToolContext", () => {
  ...
  it("uses To for unknown/removed channel providers (e.g. signal direct)", () => {
    const sessionCtx = {
      Provider: "signal",
      ChatType: "direct",
    ...
    // signal channel was removed; fallback returns To as-is
    expect(result.currentChannelId).toBe("signal:+15550002");
  });

  it("uses To for unknown/removed channel providers (e.g. signal group)", () => {
    const sessionCtx = {
      Provider: "signal",
      ChatType: "group",
  ...
  it("uses To for unknown/removed channel providers (e.g. imessage direct)", () => {
    const sessionCtx = {
      Provider: "imessage",
      ChatType: "direct",
    ...
    // imessage channel was removed; fallback returns To as-is
  ...
  it("prefers MessageThreadId for Slack tool threading", () => {
    ...
    const result = buildThreadingToolContext({
      sessionCtx,
      config: { channels: { slack: { replyToMode: "all" } } } as WoodlsConfig,
      hasRepliedRef: undefined,
    });
