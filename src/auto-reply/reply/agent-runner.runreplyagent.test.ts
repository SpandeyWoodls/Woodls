...
describe("runReplyAgent typing (heartbeat)", () => {
  ...
  function snapshotStateEnv(): StateEnvSnapshot {
    return { WOODLS_STATE_DIR: process.env.WOODLS_STATE_DIR };
  }

  function restoreStateEnv(snapshot: StateEnvSnapshot) {
    if (snapshot.WOODLS_STATE_DIR === undefined) {
      delete process.env.WOODLS_STATE_DIR;
    } else {
      process.env.WOODLS_STATE_DIR = snapshot.WOODLS_STATE_DIR;
    }
  }
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "woodls-typing-heartbeat-"));
  });
...
describe("runReplyAgent memory flush", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(tmpdir(), "woodls-memory-flush-"));
  });
