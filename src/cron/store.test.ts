...
async function makeStorePath() {
  const dir = await fs.mkdtemp(path.join(os.tmpdir(), "woodls-cron-store-"));
  return {
    dir,
...
describe("resolveCronStorePath", () => {
  ...
  it("uses WOODLS_HOME for tilde expansion", () => {
    vi.stubEnv("WOODLS_HOME", "/srv/woodls-home");
    vi.stubEnv("HOME", "/home/other");

    const result = resolveCronStorePath("~/cron/jobs.json");
    expect(result).toBe(path.resolve("/srv/woodls-home", "cron", "jobs.json"));
  });
});
