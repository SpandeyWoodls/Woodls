...
import type { WoodlsConfig, ConfigFileSnapshot } from "../config/types.woodls.js";
...
vi.mock("../infra/woodls-root.js", () => ({
  resolveWoodlsPackageRoot: vi.fn(),
}));
...
const { resolveWoodlsPackageRoot } = await import("../infra/woodls-root.js");
...
describe("update-cli", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "woodls-update-tests-"));
  });
  ...
  const baseConfig = {} as WoodlsConfig;
  const baseSnapshot: ConfigFileSnapshot = {
    path: "/tmp/woodls-config.json",
    exists: true,
    raw: "{}",
    parsed: {},
    resolved: baseConfig,
  ...
  const mockPackageInstallStatus = (root: string) => {
    vi.mocked(resolveWoodlsPackageRoot).mockResolvedValue(root);
  ...
  const setupNonInteractiveDowngrade = async () => {
    const tempDir = await createCaseDir("woodls-update");
    setTty(false);
  ...
  beforeEach(() => {
    ...
    vi.mocked(resolveWoodlsPackageRoot).mockReset();
    ...
    vi.mocked(resolveWoodlsPackageRoot).mockResolvedValue(process.cwd());
    ...
    readPackageName.mockResolvedValue("woodls");
    readPackageVersion.mockResolvedValue("1.0.0");
    resolveGlobalManager.mockResolvedValue("npm");
    serviceLoaded.mockResolvedValue(false);
    prepareRestartScript.mockResolvedValue("/tmp/woodls-restart-test.sh");
  ...
  it("updateStatusCommand prints table output", async () => {
    ...
    expect(logs.join("\n")).toContain("Woodls update status");
  });
  ...
  it("uses stored beta channel when configured", async () => {
    vi.mocked(readConfigFileSnapshot).mockResolvedValue({
      ...baseSnapshot,
      config: { update: { channel: "beta" } } as WoodlsConfig,
    });
    vi.mocked(runGatewayUpdate).mockResolvedValue({
      status: "ok",
      mode: "git",
  ...
  it("honors --tag override", async () => {
    ...
    vi.mocked(resolveWoodlsPackageRoot).mockResolvedValue(tempDir);
    vi.mocked(runGatewayUpdate).mockResolvedValue({
      status: "ok",
      mode: "npm",
      steps: [],
  ...
  it("updateCommand continues after doctor sub-step and clears update flag", async () => {
    ...
    const envSnapshot = captureEnv(["WOODLS_UPDATE_IN_PROGRESS"]);
    ...
    try {
      ...
      expect(process.env.WOODLS_UPDATE_IN_PROGRESS).toBeUndefined();
  ...
  it("updateWizardCommand offers dev checkout and forwards selections", async () => {
    const tempDir = await createCaseDir("woodls-update-wizard");
    const envSnapshot = captureEnv(["WOODLS_GIT_DIR"]);
    try {
      setTty(true);
      process.env.WOODLS_GIT_DIR = tempDir;
