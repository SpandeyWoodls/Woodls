...
describe("daemon-cli coverage", () => {
  const originalEnv = {
    WOODLS_STATE_DIR: process.env.WOODLS_STATE_DIR,
    WOODLS_CONFIG_PATH: process.env.WOODLS_CONFIG_PATH,
    WOODLS_GATEWAY_PORT: process.env.WOODLS_GATEWAY_PORT,
    WOODLS_PROFILE: process.env.WOODLS_PROFILE,
  };

  beforeEach(() => {
    process.env.WOODLS_STATE_DIR = "/tmp/woodls-cli-state";
    process.env.WOODLS_CONFIG_PATH = "/tmp/woodls-cli-state/woodls.json";
    delete process.env.WOODLS_GATEWAY_PORT;
    delete process.env.WOODLS_PROFILE;
    serviceReadCommand.mockResolvedValue(null);
  });

  afterEach(() => {
    if (originalEnv.WOODLS_STATE_DIR !== undefined) {
      process.env.WOODLS_STATE_DIR = originalEnv.WOODLS_STATE_DIR;
    } else {
      delete process.env.WOODLS_STATE_DIR;
    }

    if (originalEnv.WOODLS_CONFIG_PATH !== undefined) {
      process.env.WOODLS_CONFIG_PATH = originalEnv.WOODLS_CONFIG_PATH;
    } else {
      delete process.env.WOODLS_CONFIG_PATH;
    }

    if (originalEnv.WOODLS_GATEWAY_PORT !== undefined) {
      process.env.WOODLS_GATEWAY_PORT = originalEnv.WOODLS_GATEWAY_PORT;
    } else {
      delete process.env.WOODLS_GATEWAY_PORT;
    }

    if (originalEnv.WOODLS_PROFILE !== undefined) {
      process.env.WOODLS_PROFILE = originalEnv.WOODLS_PROFILE;
    } else {
      delete process.env.WOODLS_PROFILE;
    }
  });
  ...
  it("derives probe URL from service args + env (json)", async () => {
    ...
    serviceReadCommand.mockResolvedValueOnce({
      ...
      environment: {
        WOODLS_PROFILE: "dev",
        WOODLS_STATE_DIR: "/tmp/woodls-daemon-state",
        WOODLS_CONFIG_PATH: "/tmp/woodls-daemon-state/woodls.json",
        WOODLS_GATEWAY_PORT: "19001",
      },
