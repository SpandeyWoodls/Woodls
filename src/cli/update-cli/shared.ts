...
import { resolveWoodlsPackageRoot } from "../../infra/woodls-root.js";
...
const WOODLS_REPO_URL = "https://github.com/woodls/woodls.git";
const MAX_LOG_CHARS = 8000;

export const DEFAULT_PACKAGE_NAME = "woodls";
...
export function normalizeTag(value?: string | null): string | null {
  ...
  if (trimmed.startsWith("woodls@")) {
    return trimmed.slice("woodls@".length);
  }
...
export function resolveGitInstallDir(): string {
  const override = process.env.WOODLS_GIT_DIR?.trim();
  if (override) {
...
export async function resolveUpdateRoot(): Promise<string> {
  return (
    (await resolveWoodlsPackageRoot({
...
export async function ensureGitCheckout(params: {
  ...
  if (!dirExists) {
    return await runUpdateStep({
      name: "git clone",
      argv: ["git", "clone", WOODLS_REPO_URL, params.dir],
      timeoutMs: params.timeoutMs,
  ...
  if (!(await isGitCheckout(params.dir))) {
    ...
    if (!empty) {
      throw new Error(
        `WOODLS_GIT_DIR points at a non-git directory: ${params.dir}. Set WOODLS_GIT_DIR to an empty folder or an woodls checkout.`,
      );
    }
  ...
  if (!(await isCorePackage(params.dir))) {
    throw new Error(`WOODLS_GIT_DIR does not look like a core checkout: ${params.dir}.`);
  }
...
export async function tryWriteCompletionCache(root: string, jsonMode: boolean): Promise<void> {
  const binPath = path.join(root, "woodls.mjs");
