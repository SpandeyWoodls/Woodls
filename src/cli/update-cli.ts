...
export function registerUpdateCli(program: Command) {
  const update = program
    .command("update")
    .description("Update Woodls and inspect update channel status")
    ...
    .addHelpText("after", () => {
      const examples = [
        ["woodls update", "Update a source checkout (git)"],
        ["woodls update --channel beta", "Switch to beta channel (git + npm)"],
        ["woodls update --channel dev", "Switch to dev channel (git + npm)"],
        ["woodls update --tag beta", "One-off update to a dist-tag or version"],
        ["woodls update --no-restart", "Update without restarting the service"],
        ["woodls update --json", "Output result as JSON"],
        ["woodls update --yes", "Non-interactive (accept downgrade prompts)"],
        ["woodls update wizard", "Interactive update wizard"],
        ["woodls --update", "Shorthand for woodls update"],
      ] as const;
...
${theme.heading("Switch channels:")}
  ...
  - Run woodls update status to see the active channel and source
...
${theme.muted("Docs:")} ${formatDocsLink("/cli/update", "docs.woodls.ai/cli/update")}`;
    })
    .action(async (opts) => {
      try {
        await updateCommand({
  ...
  update
    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["woodls update status", "Show channel + version status."],
          ["woodls update status --json", "JSON output."],
          ["woodls update status --timeout 10", "Custom timeout."],
        ])}\n\n${theme.heading("Notes:")}\n${theme.muted(
          "- Shows current update channel (stable/beta/dev) and source",
        )}\n${theme.muted("- Includes git tag/branch/SHA for source checkouts")}\n\n${theme.muted(
          "Docs:",
        )} ${formatDocsLink("/cli/update", "docs.woodls.ai/cli/update")}`,
    )
    .action(async (opts, command) => {
      try {
