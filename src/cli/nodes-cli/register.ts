    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["woodls nodes status", "List known nodes with live status."],
          ["woodls nodes pairing pending", "Show pending node pairing requests."],
          ['woodls nodes run --node <id> --raw "uname -a"', "Run a shell command on a node."],
          ["woodls nodes camera snap --node <id>", "Capture a photo from a node camera."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/nodes", "docs.woodls.ai/cli/nodes")}\n`,
    );
