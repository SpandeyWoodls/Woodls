...
const shouldRegisterPrimaryOnly = (argv: string[]) => {
  if (isTruthyEnvValue(process.env.WOODLS_DISABLE_LAZY_SUBCOMMANDS)) {
    return false;
  }
...
const shouldEagerRegisterSubcommands = (_argv: string[]) => {
  return isTruthyEnvValue(process.env.WOODLS_DISABLE_LAZY_SUBCOMMANDS);
};

const loadConfig = async (): Promise<WoodlsConfig> => {
...
const entries: SubCliEntry[] = [
  ...
  {
    name: "docs",
    description: "Search the live Woodls docs",
    hasSubcommands: false,
    register: async (program) => {
