...
${formatHelpExamples([
  ['woodls agent --to +15555550123 --message "status update"', "Start a new session."],
  ['woodls agent --agent ops --message "Summarize logs"', "Use a specific agent."],
  [
    'woodls agent --session-id 1234 --message "Summarize inbox" --thinking medium',
    "Target a session with explicit thinking level.",
  ],
  [
    'woodls agent --to +15555550123 --message "Trace logs" --verbose on --json',
    "Enable verbose logging and JSON output.",
  ],
  ['woodls agent --to +15555550123 --message "Summon reply" --deliver', "Deliver reply."],
  [
    'woodls agent --agent ops --message "Generate report" --deliver --reply-channel slack --reply-to "#reports"',
    "Send reply to a different channel/target.",
  ],
])}

${theme.muted("Docs:")} ${formatDocsLink("/cli/agent", "docs.woodls.ai/cli/agent")}`,
    )
    .action(async (opts) => {
  ...
  const agents = program
    ...
    .addHelpText(
      "after",
      () =>
        `\n${theme.muted("Docs:")} ${formatDocsLink("/cli/agents", "docs.woodls.ai/cli/agents")}\n`,
    );
...
${theme.heading("Examples:")}
${formatHelpExamples([
  ['woodls agents set-identity --agent main --name "Woodls" --emoji "🦞"', "Set name + emoji."],
  ["woodls agents set-identity --agent main --avatar avatars/woodls.png", "Set avatar path."],
  [
    "woodls agents set-identity --workspace ~/.woodls/workspace --from-identity",
    "Load from IDENTITY.md.",
  ],
  [
    "woodls agents set-identity --identity-file ~/.woodls/workspace/IDENTITY.md --agent main",
