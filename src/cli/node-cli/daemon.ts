...
function renderNodeServiceStartHints(): string[] {
  const base = [formatCliCommand("woodls node install"), formatCliCommand("woodls node start")];
...
export async function runNodeDaemonInstall(opts: NodeDaemonInstallOptions) {
  ...
  if (loaded && !opts.force) {
    ...
    if (!json) {
      ...
      defaultRuntime.log(`Reinstall with: ${formatCliCommand("woodls node install --force")}`);
    }
    return;
  }
...
export async function runNodeDaemonStatus(opts: NodeDaemonStatusOptions = {}) {
  ...
  const hintEnv = {
    ...baseEnv,
    WOODLS_LOG_PREFIX: baseEnv.WOODLS_LOG_PREFIX ?? "node",
