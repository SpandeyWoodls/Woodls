    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["woodls node run --host 127.0.0.1 --port 18789", "Run the node host in the foreground."],
          ["woodls node status", "Check node host service status."],
          ["woodls node install", "Install the node host service."],
          ["woodls node restart", "Restart the installed node host service."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/node", "docs.woodls.ai/cli/node")}\n`,
    );
