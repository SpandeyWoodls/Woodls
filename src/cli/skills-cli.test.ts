...
describe("skills-cli", () => {
  describe("formatSkillsList", () => {
    it("formats empty skills list", () => {
      ...
      expect(output).toContain("npx woodlshub");
    });
  ...
  describe("integration: loads real skills from bundled directory", () => {
    ...
    beforeAll(async () => {
      envSnapshot = captureEnv(["WOODLS_BUNDLED_SKILLS_DIR"]);
      tempWorkspaceDir = fs.mkdtempSync(path.join(os.tmpdir(), "woodls-skills-test-"));
      tempBundledDir = fs.mkdtempSync(path.join(os.tmpdir(), "woodls-bundled-skills-test-"));
      process.env.WOODLS_BUNDLED_SKILLS_DIR = tempBundledDir;
