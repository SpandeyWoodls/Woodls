    ...
    .addHelpText(
      ...
      () =>
        `\n${theme.heading("Examples:")}\n${formatHelpExamples([
          ["woodls memory status", "Show index and provider status."],
          ["woodls memory index --force", "Force a full reindex."],
          ['woodls memory search --query "deployment notes"', "Search indexed memory entries."],
          ["woodls memory status --json", "Output machine-readable JSON."],
        ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/memory", "docs.woodls.ai/cli/memory")}\n`,
    );
