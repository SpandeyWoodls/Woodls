      ...
      .addHelpText(
        ...
        () =>
          `\n${theme.heading("Examples:")}\n${formatHelpExamples([
            ["woodls gateway run", "Run the gateway in the foreground."],
            ["woodls gateway status", "Show service status and probe reachability."],
            ["woodls gateway discover", "Find local and wide-area gateway beacons."],
            ["woodls gateway call health", "Call a gateway RPC method directly."],
          ])}\n\n${theme.muted("Docs:")} ${formatDocsLink("/cli/gateway", "docs.woodls.ai/cli/gateway")}\n`,
      ),
  );
