...
function summarizeGroupPolicy(cfg: WoodlsConfig): {
  open: number;
  allowlist: number;
  other: number;
} {
...
function isGatewayRemotelyExposed(cfg: WoodlsConfig): boolean {
...
function collectModels(cfg: WoodlsConfig): ModelRef[] {
  const out: ModelRef[] = [];
...
function listKnownNodeCommands(cfg: WoodlsConfig): Set<string> {
  const baseCfg: WoodlsConfig = {
    ...cfg,
    gateway: {
      ...cfg.gateway,
      nodes: {
...
function hasWebSearchKey(cfg: WoodlsConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebSearchEnabled(cfg: WoodlsConfig, env: NodeJS.ProcessEnv): boolean {
...
function isWebFetchEnabled(cfg: WoodlsConfig): boolean {
...
function isBrowserEnabled(cfg: WoodlsConfig): boolean {
  try {
...
function listGroupPolicyOpen(cfg: WoodlsConfig): string[] {
  const out: string[] = [];
  const channels = cfg.channels as Record<string, unknown> | undefined;
  if (!channels || typeof channels !== "object") {
    return out;
...
export function collectAttackSurfaceSummaryFindings(cfg: WoodlsConfig): SecurityAuditFinding[] {
...
export function collectSyncedFolderFindings(params: {
  ...
  if (isProbablySyncedPath(params.stateDir) || isProbablySyncedPath(params.configPath)) {
    findings.push({
      ...
      remediation: `Keep WOODLS_STATE_DIR on a local-only volume and re-run "${formatCliCommand("woodls security audit --fix")}".`,
    });
  }
  return findings;
}

export function collectSecretsInConfigFindings(cfg: WoodlsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const password =
  ...
  if (password && !looksLikeEnvRef(password)) {
    findings.push({
      ...
      remediation:
        "Prefer WOODLS_GATEWAY_PASSWORD (env) and remove gateway.auth.password from disk.",
    });
  }
...
export function collectHooksHardeningFindings(
  ...
  const woodlsGatewayToken =
    typeof env.WOODLS_GATEWAY_TOKEN === "string" && env.WOODLS_GATEWAY_TOKEN.trim()
      ? env.WOODLS_GATEWAY_TOKEN.trim()
      : null;
  const gatewayToken =
...
export function collectGatewayHttpSessionKeyOverrideFindings(
  ...
  findings.push({
    ...
    detail:
      `${enabledEndpoints.join(", ")} accept x-woodls-session-key for per-request session routing. ` +
...
export function collectSandboxDockerNoopFindings(cfg: WoodlsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectSandboxDangerousConfigFindings(cfg: WoodlsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
  const agents = Array.isArray(cfg.agents?.list) ? cfg.agents.list : [];
...
export function collectNodeDenyCommandPatternFindings(cfg: WoodlsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectMinimalProfileOverrideFindings(cfg: WoodlsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectModelHygieneFindings(cfg: WoodlsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
...
export function collectExposureMatrixFindings(cfg: WoodlsConfig): SecurityAuditFinding[] {
  const findings: SecurityAuditFinding[] = [];
