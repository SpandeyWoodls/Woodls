...
describe("parseRelaySmokeTest", () => {
  ...
  it("parses env var smoke mode only when no args", () => {
    expect(parseRelaySmokeTest([], { WOODLS_SMOKE_QR: "1" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { WOODLS_SMOKE_QR: "1" })).toBe(null);
  });

  it("supports WOODLS_SMOKE=qr only when no args", () => {
    expect(parseRelaySmokeTest([], { WOODLS_SMOKE: "qr" })).toBe("qr");
    expect(parseRelaySmokeTest(["send"], { WOODLS_SMOKE: "qr" })).toBe(null);
  });
  ...
  it("prefers explicit --smoke over env vars", () => {
    expect(parseRelaySmokeTest(["--smoke", "qr"], { WOODLS_SMOKE: "nope" })).toBe("qr");
  });
});
