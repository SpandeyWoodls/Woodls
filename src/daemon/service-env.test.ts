...
describe("buildServiceEnvironment", () => {
  it("sets minimal PATH and gateway vars", () => {
    ...
    expect(env.WOODLS_GATEWAY_PORT).toBe("18789");
    expect(env.WOODLS_GATEWAY_TOKEN).toBe("secret");
    expect(env.WOODLS_SERVICE_MARKER).toBe("woodls");
    expect(env.WOODLS_SERVICE_KIND).toBe("gateway");
    expect(typeof env.WOODLS_SERVICE_VERSION).toBe("string");
    expect(env.WOODLS_SYSTEMD_UNIT).toBe("woodls-gateway.service");
    if (process.platform === "darwin") {
      expect(env.WOODLS_LAUNCHD_LABEL).toBe("ai.woodls.gateway");
    }
  });
  ...
  it("uses profile-specific unit and label", () => {
    const env = buildServiceEnvironment({
      env: { HOME: "/home/user", WOODLS_PROFILE: "work" },
      port: 18789,
    });
    expect(env.WOODLS_SYSTEMD_UNIT).toBe("woodls-gateway-work.service");
    if (process.platform === "darwin") {
      expect(env.WOODLS_LAUNCHD_LABEL).toBe("ai.woodls.work");
    }
  });
});
...
describe("resolveGatewayStateDir", () => {
  it("uses the default state dir when no overrides are set", () => {
    ...
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".woodls"));
  });

  it("appends the profile suffix when set", () => {
    const env = { HOME: "/Users/test", WOODLS_PROFILE: "rescue" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".woodls-rescue"));
  });

  it("treats default profiles as the base state dir", () => {
    const env = { HOME: "/Users/test", WOODLS_PROFILE: "Default" };
    expect(resolveGatewayStateDir(env)).toBe(path.join("/Users/test", ".woodls"));
  });

  it("uses WOODLS_STATE_DIR when provided", () => {
    const env = { HOME: "/Users/test", WOODLS_STATE_DIR: "/var/lib/woodls" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/var/lib/woodls"));
  });

  it("expands ~ in WOODLS_STATE_DIR", () => {
    const env = { HOME: "/Users/test", WOODLS_STATE_DIR: "~/woodls-state" };
    expect(resolveGatewayStateDir(env)).toBe(path.resolve("/Users/test/woodls-state"));
  });

  it("preserves Windows absolute paths without HOME", () => {
    const env = { WOODLS_STATE_DIR: "C:\\State\\woodls" };
    expect(resolveGatewayStateDir(env)).toBe("C:\\State\\woodls");
  });
});
