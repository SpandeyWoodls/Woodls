...
function auditGatewayToken(
  ...
  const serviceToken = command?.environment?.WOODLS_GATEWAY_TOKEN?.trim();
  ...
  issues.push({
    ...
    message:
      "Gateway service WOODLS_GATEWAY_TOKEN does not match gateway.auth.token in woodls.json",
...
export function checkTokenDrift(params: {
  ...
  if (configToken && serviceToken !== configToken) {
    return {
      ...
      detail: "Run `woodls gateway install --force` to sync the token.",
      level: "recommended",
    };
  }
