...
describe("launchctl list detection", () => {
  it("detects the resolved label in launchctl list", async () => {
    state.listOutput = "123 0 ai.woodls.gateway\n";
    const listed = await isLaunchAgentListed({
      env: { HOME: "/Users/test", WOODLS_PROFILE: "default" },
    });
...
describe("resolveLaunchAgentPlistPath", () => {
  it("uses default label when WOODLS_PROFILE is unset", () => {
    const env = { HOME: "/Users/test" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.woodls.gateway.plist",
    );
  });

  it("uses profile-specific label when WOODLS_PROFILE is set to a custom value", () => {
    const env = { HOME: "/Users/test", WOODLS_PROFILE: "jbphoenix" };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.woodls.jbphoenix.plist",
    );
  });

  it("prefers WOODLS_LAUNCHD_LABEL over WOODLS_PROFILE", () => {
    const env = {
      HOME: "/Users/test",
      WOODLS_PROFILE: "jbphoenix",
      WOODLS_LAUNCHD_LABEL: "com.custom.label",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
  ...
  it("trims whitespace from WOODLS_LAUNCHD_LABEL", () => {
    const env = {
      HOME: "/Users/test",
      WOODLS_LAUNCHD_LABEL: "  com.custom.label  ",
    };
  ...
  it("ignores empty WOODLS_LAUNCHD_LABEL and falls back to profile", () => {
    const env = {
      HOME: "/Users/test",
      WOODLS_PROFILE: "myprofile",
      WOODLS_LAUNCHD_LABEL: "   ",
    };
    expect(resolveLaunchAgentPlistPath(env)).toBe(
      "/Users/test/Library/LaunchAgents/ai.woodls.myprofile.plist",
    );
  });
});
