...
homepage: https://docs.woodls.ai/automation/hooks#command-logger
metadata:
  {
    "woodls":
      {
...
2. **Appends to log file** - Writes a JSON line to `~/.woodls/logs/commands.log`
...
1. **Manual rotation**:
   ...
   ```bash
   mv ~/.woodls/logs/commands.log ~/.woodls/logs/commands.log.old
   ```

2. **Use logrotate** (Linux):
   Create `/etc/logrotate.d/woodls`:
   ```
   /home/username/.woodls/logs/commands.log {
       weekly
       rotate 4
       compress
       missingok
