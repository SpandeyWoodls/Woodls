...
async function runNewWithPreviousSession(params: {
  ...
  cfg?: (tempDir: string) => WoodlsConfig;
}): Promise<{ tempDir: string; files: string[]; memoryContent: string }> {
  const tempDir = await makeTempWorkspace("woodls-session-memory-");
...
function makeSessionMemoryConfig(tempDir: string, messages?: number): WoodlsConfig {
  return {
    agents: { defaults: { workspace: tempDir } },
