...
This directory contains hooks that ship with Woodls. These hooks are automatically discovered and can be enabled/disabled via CLI or configuration.
...
Hooks can be configured in `~/.woodls/woodls.json`:
...
2. Restart gateway: `pkill -9 -f 'woodls.*gateway' && pnpm woodls gateway`
3. Enable the hook: `woodls hooks enable my-hook`
4. Trigger the event (e.g., send `/new` command)
5. Check gateway logs for hook execution

## Documentation

Full documentation: https://docs.woodls.ai/automation/hooks
