...
`;
    ...
    expect(parsed.woodls.emoji).toBe("💾");
    expect(parsed.woodls.events).toEqual(["command:new"]);
  });
...
`;
    ...
    const parsed = JSON.parse(result.metadata);
    expect(parsed.woodls.emoji).toBe("📝");
    expect(parsed.woodls.events).toEqual(["command"]);
    expect(parsed.woodls.requires.config).toEqual(["workspace.dir"]);
    expect(parsed.woodls.install[0].kind).toBe("bundled");
  });

  it("handles single-line metadata (inline JSON)", () => {
    const content = `---
name: simple-hook
metadata: {"woodls": {"events": ["test"]}}
---
`;
    const result = parseFrontmatter(content);
    expect(result.name).toBe("simple-hook");
    expect(result.metadata).toBe('{"woodls": {"events": ["test"]}}');
  });
...
describe("resolveWoodlsMetadata", () => {
  it("extracts woodls metadata from parsed frontmatter", () => {
    const frontmatter = {
      name: "test-hook",
    ...
    const result = resolveWoodlsMetadata(frontmatter);
  ...
  it("returns undefined when woodls key is missing", () => {
    const frontmatter = {
  ...
  it("handles install specs", () => {
    const frontmatter = {
      metadata: JSON.stringify({
        woodls: {
          events: ["command"],
          install: [
            { id: "bundled", kind: "bundled", label: "Bundled with Woodls" },
            { id: "npm", kind: "npm", package: "@woodls/hook" },
          ],
        },
      }),
    };
    ...
    expect(result?.install?.[1].package).toBe("@woodls/hook");
  });
    ...
    "woodls":
      {
        ...
        "events": ["command:new"],
        "requires": { "config": ["workspace.dir"] },
        "install": [{ "id": "bundled", "kind": "bundled", "label": "Bundled with Woodls" }],
      },
  }
---
...
`;
    ...
    const woodls = resolveWoodlsMetadata(frontmatter);
    expect(woodls).toBeDefined();
    expect(woodls?.emoji).toBe("💾");
    expect(woodls?.events).toEqual(["command:new"]);
    expect(woodls?.requires?.config).toEqual(["workspace.dir"]);
    expect(woodls?.install?.[0].kind).toBe("bundled");
  });
