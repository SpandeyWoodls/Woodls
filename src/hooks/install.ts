...
async function ensureWoodlsHooks(manifest: HookPackageManifest) {
  const allKeys = [MANIFEST_KEY, ...LEGACY_MANIFEST_KEYS] as const;
  let hooks: unknown;
  for (const key of allKeys) {
    const entry = (manifest as Record<string, { hooks?: unknown } | undefined>)[key];
    if (entry !== undefined) {
      hooks = entry.hooks;
      break;
    }
...
async function installHookPackageFromDir(params: {
  ...
  try {
    hookEntries = await ensureWoodlsHooks(manifest);
  } catch (err) {
...
export async function installHooksFromArchive(params: {
  ...
  return await withTempDir("woodls-hook-", async (tmpDir) => {
...
export async function installHooksFromNpmSpec(params: {
  ...
  return await withTempDir("woodls-hook-pack-", async (tmpDir) => {
