...
describe("loader", () => {
  ...
  beforeAll(async () => {
    fixtureRoot = await fs.mkdtemp(path.join(os.tmpdir(), "woodls-hooks-loader-"));
  });
  ...
  beforeEach(async () => {
    ...
    originalBundledDir = process.env.WOODLS_BUNDLED_HOOKS_DIR;
    process.env.WOODLS_BUNDLED_HOOKS_DIR = "/nonexistent/bundled/hooks";
  });
  ...
  afterEach(async () => {
    ...
    if (originalBundledDir === undefined) {
      delete process.env.WOODLS_BUNDLED_HOOKS_DIR;
    } else {
      process.env.WOODLS_BUNDLED_HOOKS_DIR = originalBundledDir;
    }
  });
