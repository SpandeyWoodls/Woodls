...
describe("media store", () => {
  ...
  beforeAll(async () => {
    envSnapshot = captureEnv(["HOME", "USERPROFILE", "HOMEDRIVE", "HOMEPATH", "WOODLS_STATE_DIR"]);
    home = await fs.mkdtemp(path.join(os.tmpdir(), "woodls-test-home-"));
    ...
    await fs.mkdir(path.join(home, ".woodls"), { recursive: true });
  ...
  it("creates and returns media directory", async () => {
    await withTempStore(async (store, home) => {
      ...
      expect(path.normalize(dir)).toContain(`${path.sep}.woodls${path.sep}media`);
