...
describe("runCommandWithTimeout", () => {
  ...
  it("merges custom env with process.env", async () => {
    const envSnapshot = captureEnv(["WOODLS_BASE_ENV"]);
    process.env.WOODLS_BASE_ENV = "base";
    try {
      const result = await runCommandWithTimeout(
        [
          process.execPath,
          "-e",
          'process.stdout.write((process.env.WOODLS_BASE_ENV ?? "") + "|" + (process.env.WOODLS_TEST_ENV ?? ""))',
        ],
        {
          timeoutMs: 5_000,
          env: { WOODLS_TEST_ENV: "ok" },
        },
      );
