...
export function resolveDefaultAgentWorkspaceDir(
  ...
  const profile = env.WOODLS_PROFILE?.trim();
  if (profile && profile.toLowerCase() !== "default") {
    return path.join(home, ".woodls", `workspace-${profile}`);
  }
  return path.join(home, ".woodls", "workspace");
}
...
const WORKSPACE_STATE_DIRNAME = ".woodls";
