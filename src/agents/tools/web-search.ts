...
type WebSearchConfig = NonNullable<WoodlsConfig["tools"]>["web"] extends infer Web
...
function resolveSearchConfig(cfg?: WoodlsConfig): WebSearchConfig {
...
function missingSearchKeyPayload(provider: (typeof SEARCH_PROVIDERS)[number]) {
  ...
  return {
    ...
    message: `web_search needs a Brave Search API key. Run \`${formatCliCommand("woodls configure --section web")}\` to store it, or set BRAVE_API_KEY in the Gateway environment.`,
    docs: "https://docs.woodls.ai/tools/web",
  };
}
...
async function runPerplexitySearch(params: {
  ...
  const res = await fetch(endpoint, {
    ...
    headers: {
      ...
      "HTTP-Referer": "https://woodls.ai",
      "X-Title": "Woodls Web Search",
    },
