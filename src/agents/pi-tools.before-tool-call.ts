...
import { budgetCheck, warnIfOverBudget } from "./tools/result-budget.js";
...
export function wrapToolWithBeforeToolCallHook(
  ...
  const wrappedTool: AnyAgentTool = {
    ...
    execute: async (toolCallId, params, signal, onUpdate) => {
      ...
      // Pre-execution budget check: inject size-limit parameters where supported.
      let effectiveParams = outcome.params;
      const budget = budgetCheck(
        toolName,
        isPlainObject(outcome.params) ? outcome.params : undefined,
      );
      if (budget.suggestedLimit !== undefined && isPlainObject(outcome.params)) {
        const paramsCopy = { ...outcome.params };
        if (budget.category === "web_fetch" && paramsCopy.maxChars === undefined) {
          paramsCopy.maxChars = budget.suggestedLimit;
          effectiveParams = paramsCopy;
        }
      }

      if (toolCallId) {
        adjustedParamsByToolCallId.set(toolCallId, effectiveParams);
      ...
      try {
        ...
        // Post-execution: warn if the result exceeds the tool's budget.
        try {
          ...
          if (Array.isArray(content)) {
            ...
            for (const block of content) {
              if (
                ...
                totalText += (block as { text: string }).text;
              }
            }
            if (totalText) {
              warnIfOverBudget(normalizedToolName, totalText);
            }
          }
        } catch {
          // Budget warning is best-effort; never fail the tool call.
        }
