...
describe("getApiKeyForModel", () => {
  it("migrates legacy oauth.json into auth-profiles.json", async () => {
    const envSnapshot = captureEnv(["WOODLS_STATE_DIR", "WOODLS_AGENT_DIR", "PI_CODING_AGENT_DIR"]);
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "woodls-oauth-"));

    try {
      process.env.WOODLS_STATE_DIR = tempDir;
      process.env.WOODLS_AGENT_DIR = path.join(tempDir, "agent");
      process.env.PI_CODING_AGENT_DIR = process.env.WOODLS_AGENT_DIR;
      ...
      const store = ensureAuthProfileStore(process.env.WOODLS_AGENT_DIR, {
  ...
  it("suggests openai-codex when only Codex OAuth is configured", async () => {
    ...
    const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "woodls-auth-"));
