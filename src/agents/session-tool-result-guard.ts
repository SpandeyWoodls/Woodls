...
import { warnIfOverBudget } from "./tools/result-budget.js";
...
export function installSessionToolResultGuard(
  ...
  const guardedAppend = (message: AgentMessage) => {
    ...
    if (nextRole === "toolResult") {
      ...
      // Log a budget warning for oversized tool results before capping.
      if (toolName) {
        try {
          const trContent = (nextMessage as { content?: unknown }).content;
          if (Array.isArray(trContent)) {
            ...
            for (const block of trContent) {
              if (
                ...
                trText += (block as { text: string }).text;
              }
            }
            if (trText) {
              warnIfOverBudget(toolName, trText);
            }
          }
        } catch {
          // Budget warning is best-effort.
        }
      }
