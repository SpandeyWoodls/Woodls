...
export function buildSandboxCreateArgs(params: {
  ...
  args.push("--label", "woodls.sandbox=1");
  args.push("--label", `woodls.sessionKey=${params.scopeKey}`);
  args.push("--label", `woodls.createdAtMs=${createdAtMs}`);
  if (params.configHash) {
    args.push("--label", `woodls.configHash=${params.configHash}`);
  }
...
async function readContainerConfigHash(containerName: string): Promise<string | null> {
  return await readDockerContainerLabel(containerName, "woodls.configHash");
}

function formatSandboxRecreateHint(params: { scope: SandboxConfig["scope"]; sessionKey: string }) {
  if (params.scope === "session") {
    return formatCliCommand(`woodls sandbox recreate --session ${params.sessionKey}`);
  }
  if (params.scope === "agent") {
    const agentId = resolveSandboxAgentId(params.sessionKey) ?? "main";
    return formatCliCommand(`woodls sandbox recreate --agent ${agentId}`);
  }
  return formatCliCommand("woodls sandbox recreate --all");
}
