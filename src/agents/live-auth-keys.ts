...
const GOOGLE_LIVE_SINGLE_KEY = "WOODLS_LIVE_GEMINI_KEY";
...
const PROVIDER_API_KEY_CONFIG: Record<string, Omit<ProviderApiKeyConfig, "fallbackVars">> = {
  anthropic: {
    liveSingle: "WOODLS_LIVE_ANTHROPIC_KEY",
    listVar: "WOODLS_LIVE_ANTHROPIC_KEYS",
  ...
  openai: {
    liveSingle: "WOODLS_LIVE_OPENAI_KEY",
...
function resolveProviderApiKeyConfig(provider: string): ProviderApiKeyConfig {
  ...
  const liveSingle = custom?.liveSingle ?? `WOODLS_LIVE_${base}_KEY`;
