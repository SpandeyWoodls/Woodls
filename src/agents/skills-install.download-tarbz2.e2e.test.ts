...
async function withTempWorkspace(
  ...
  const workspaceDir = await fs.mkdtemp(path.join(os.tmpdir(), "woodls-skills-install-"));
  try {
...
function restoreWoodlsStateDir(originalValue: string | undefined): void {
  if (originalValue === undefined) {
    delete process.env.WOODLS_STATE_DIR;
    return;
  }
  process.env.WOODLS_STATE_DIR = originalValue;
}

const originalStateDir = process.env.WOODLS_STATE_DIR;

afterEach(() => {
  restoreWoodlsStateDir(originalStateDir);
});
