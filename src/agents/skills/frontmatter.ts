...
import {
  ...
  normalizeStringList,
  parseWoodlsManifestInstallBase,
  parseFrontmatterBool,
  resolveWoodlsManifestBlock,
  resolveWoodlsManifestInstall,
  resolveWoodlsManifestOs,
  resolveWoodlsManifestRequires,
...
function parseInstallSpec(input: unknown): SkillInstallSpec | undefined {
  const parsed = parseWoodlsManifestInstallBase(input, ["brew", "node", "go", "uv", "download"]);
  if (!parsed) {
    return undefined;
  }
  const { raw } = parsed;
...
export function resolveWoodlsMetadata(
  frontmatter: ParsedSkillFrontmatter,
): WoodlsSkillMetadata | undefined {
  const metadataObj = resolveWoodlsManifestBlock({ frontmatter });
  if (!metadataObj) {
    return undefined;
  }
  const requires = resolveWoodlsManifestRequires(metadataObj);
  const install = resolveWoodlsManifestInstall(metadataObj, parseInstallSpec);
  const osRaw = resolveWoodlsManifestOs(metadataObj);
  return {
