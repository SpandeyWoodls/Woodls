...
} from "./woodls-tools.subagents.sessions-spawn.test-harness.js";
...
type CreateWoodlsTools = (typeof import("./woodls-tools.js"))["createWoodlsTools"];
type CreateWoodlsToolsOpts = Parameters<CreateWoodlsTools>[0];

async function getSessionsSpawnTool(opts: CreateWoodlsToolsOpts) {
  // Dynamic import: ensure harness mocks are installed before tool modules load.
  const { createWoodlsTools } = await import("./woodls-tools.js");
  const tool = createWoodlsTools(opts).find((candidate) => candidate.name === "sessions_spawn");
  if (!tool) {
...
describe("woodls-tools: subagents (sessions_spawn lifecycle)", () => {
  beforeEach(() => {
    resetSessionsSpawnConfigOverride();
  });
