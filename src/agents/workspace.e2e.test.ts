...
describe("resolveDefaultAgentWorkspaceDir", () => {
  it("uses WOODLS_HOME for default workspace resolution", () => {
    const dir = resolveDefaultAgentWorkspaceDir({
      WOODLS_HOME: "/srv/woodls-home",
      HOME: "/home/other",
    } as NodeJS.ProcessEnv);

    expect(dir).toBe(path.join(path.resolve("/srv/woodls-home"), ".woodls", "workspace"));
  });
});

const WORKSPACE_STATE_PATH_SEGMENTS = [".woodls", "workspace-state.json"] as const;
...
describe("ensureAgentWorkspace", () => {
  it("creates BOOTSTRAP.md and records a seeded marker for brand new workspaces", async () => {
    const tempDir = await makeTempWorkspace("woodls-workspace-");
