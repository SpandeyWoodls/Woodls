  ...
  for (let idx = 0; idx < parts.length; idx += 1) {
    ...
    try {
      ...
      if (stat.isSymbolicLink()) {
        ...
        // Even when allowFinalSymlink is set, validate that the target stays within
        // the sandbox root to prevent escape via crafted symlinks.
        if (options?.allowFinalSymlink && isLast) {
          const finalTarget = await tryRealpath(current);
          if (!isPathInside(rootReal, finalTarget)) {
            throw new Error(
              `Symlink target escapes sandbox root (${shortPath(rootReal)}): ${shortPath(current)} -> ${shortPath(finalTarget)}`,
            );
          }
          return;
        }
