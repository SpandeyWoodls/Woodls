...
describe("workspace path resolution", () => {
  it("reads relative paths against workspaceDir even after cwd changes", async () => {
    await withTempDir("woodls-ws-", async (workspaceDir) => {
      await withTempDir("woodls-cwd-", async (otherDir) => {
        ...
        try {
          const tools = createWoodlsCodingTools({ workspaceDir });
    ...
    await withTempDir("woodls-ws-", async (workspaceDir) => {
      await withTempDir("woodls-cwd-", async (otherDir) => {
        ...
        try {
          ...
          expect(updated).toBe("hello woodls");
        } finally {
          cwdSpy.mockRestore();
        }
      });
  ...
  it("lets exec workdir override the workspace default", async () => {
    await withTempDir("woodls-ws-", async (workspaceDir) => {
      await withTempDir("woodls-override-", async (overrideDir) => {
        const tools = createWoodlsCodingTools({
          workspaceDir,
          exec: { host: "gateway", ask: "off", security: "full" },
        });
...
describe("sandboxed workspace paths", () => {
  it("uses sandbox workspace for relative read/write/edit", async () => {
    await withTempDir("woodls-sandbox-", async (sandboxDir) => {
      await withTempDir("woodls-workspace-", async (workspaceDir) => {
        const sandbox = {
          enabled: true,
        ...
        const tools = createWoodlsCodingTools({ workspaceDir, sandbox });
        const readTool = tools.find((tool) => tool.name === "read");
        const writeTool = tools.find((tool) => tool.name === "write");
        const editTool = tools.find((tool) => tool.name === "edit");
