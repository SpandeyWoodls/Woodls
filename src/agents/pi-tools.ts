...
function resolveExecConfig(params: { cfg?: WoodlsConfig; agentId?: string }) {
...
function resolveFsConfig(params: { cfg?: WoodlsConfig; agentId?: string }) {
  const cfg = params.cfg;
...
export function createWoodlsCodingTools(options?: {
  ...
  const base = (codingTools as unknown as AnyAgentTool[]).flatMap((tool) => {
    if (tool.name === readTool.name) {
      ...
      const wrapped = createWoodlsReadTool(freshReadTool, {
        modelContextWindowTokens: options?.modelContextWindowTokens,
        imageSanitization,
      });
