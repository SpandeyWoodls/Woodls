...
async function validateScriptFileForShellBleed(params: {
  ...
  // Match both uppercase and lowercase shell variables, including single-char vars like $x.
  ...
  // Detect subshell injection patterns like $(cmd), $(...), which can be used for obfuscated injection.
  const subshellRegex = /\$\([^)]*\)/g;
  const subshellMatch = subshellRegex.exec(content);
  if (subshellMatch) {
    const idx = subshellMatch.index;
    const before = content.slice(0, idx);
    const line = before.split("\n").length;
    const token = subshellMatch[0];
    throw new Error(
      [
        `exec preflight: detected shell subshell expansion (${token}) in ${target.kind} script: ${path.basename(
          absPath,
        )}:${line}.`,
        `Subshell syntax like $(command) is shell-specific and should not appear in ${target.kind} code.`,
        "(If this is inside a string literal on purpose, escape it or restructure the code.)",
      ].join("\n"),
    );
  }
...
export function createExecTool(
  ...
  return {
    ...
    execute: async (_toolCallId, args, signal, onUpdate) => {
      ...
      if (bypassApprovals) {
        ask = "off";
        // Mandatory audit log for elevated command bypass.
        const commandHash = crypto.createHash("sha256").update(params.command).digest("hex");
        logInfo(
          `[ELEVATED AUDIT] timestamp=${new Date().toISOString()} ` +
            `agentId=${agentId ?? "unknown"} sessionKey=${defaults?.sessionKey ?? "unknown"} ` +
            `provider=${defaults?.messageProvider ?? "unknown"} ` +
            `elevatedMode=${elevatedMode} security=${security} ` +
            `commandHash=${commandHash} command=${truncateMiddle(params.command, 200)}`,
        );
      }
