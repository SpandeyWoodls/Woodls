  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        profiles: {
          woodls: { cdpPort: testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
describe("browser control evaluate gating", () => {
  beforeEach(async () => {
    ...
    prevGatewayPort = process.env.WOODLS_GATEWAY_PORT;
    process.env.WOODLS_GATEWAY_PORT = String(testPort - 2);
    prevGatewayToken = process.env.WOODLS_GATEWAY_TOKEN;
    prevGatewayPassword = process.env.WOODLS_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (prevGatewayPort === undefined) {
      ...
      process.env.WOODLS_GATEWAY_PORT = prevGatewayPort;
    }
    if (prevGatewayToken === undefined) {
      delete process.env.WOODLS_GATEWAY_TOKEN;
    } else {
      process.env.WOODLS_GATEWAY_TOKEN = prevGatewayToken;
    }
    if (prevGatewayPassword === undefined) {
      delete process.env.WOODLS_GATEWAY_PASSWORD;
    } else {
      process.env.WOODLS_GATEWAY_PASSWORD = prevGatewayPassword;
    }
