...
describe("profile name validation", () => {
  it("accepts valid lowercase names", () => {
    expect(isValidProfileName("woodls")).toBe(true);
...
describe("port collision prevention", () => {
  it("raw config vs resolved config - shows the data source difference", async () => {
    ...
    // But resolved config has implicit woodls profile with a derived port
    const resolved = resolveBrowserConfig({});
    const usedFromResolved = getUsedPorts(resolved.profiles);
    // The implicit woodls profile occupies at least one port
  ...
  it("create-profile must use resolved config to avoid port collision", async () => {
    ...
    // Raw config: first allocation gets CDP_PORT_RANGE_START (no implicit profiles)
    expect(buggyAllocatedPort).toBe(CDP_PORT_RANGE_START);

    // Resolved config: includes implicit woodls profile
    const resolved = resolveBrowserConfig(
    ...
    // Resolved: first NEW profile gets a port that avoids collision with implicit profile
    expect(fixedAllocatedPort).not.toBeNull();
    // The allocated port should not collide with any already-used port
    expect(fixedUsedPorts.has(fixedAllocatedPort!)).toBe(false);
  });
});
