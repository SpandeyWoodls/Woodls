...
const tmpDirMocks = vi.hoisted(() => ({
  resolvePreferredWoodlsTmpDir: vi.fn(() => "/tmp/woodls"),
}));
vi.mock("../infra/tmp-woodls-dir.js", () => tmpDirMocks);
...
describe("pw-tools-core", () => {
  beforeEach(() => {
    ...
    tmpDirMocks.resolvePreferredWoodlsTmpDir.mockReturnValue("/tmp/woodls");
  });
  ...
  it("uses preferred tmp dir when waiting for download without explicit path", async () => {
    tmpDirMocks.resolvePreferredWoodlsTmpDir.mockReturnValue("/tmp/woodls-preferred");
    const { res, outPath } = await waitForImplicitDownloadOutput({
      downloadUrl: "https://example.com/file.bin",
      suggestedFilename: "file.bin",
    });
    expect(typeof outPath).toBe("string");
    const expectedRootedDownloadsDir = path.join(path.sep, "tmp", "woodls-preferred", "downloads");
    const expectedDownloadsTail = `${path.join("tmp", "woodls-preferred", "downloads")}${path.sep}`;
    ...
    expect(tmpDirMocks.resolvePreferredWoodlsTmpDir).toHaveBeenCalled();
  });
  ...
  it("sanitizes suggested download filenames to prevent traversal escapes", async () => {
    ...
    expect(path.dirname(String(outPath))).toBe(
      path.join(path.sep, "tmp", "woodls-preferred", "downloads"),
    );
    expect(path.basename(String(outPath))).toMatch(/-passwd$/);
    expect(path.normalize(res.path)).toContain(
      path.normalize(`${path.join("tmp", "woodls-preferred", "downloads")}${path.sep}`),
    );
  });
