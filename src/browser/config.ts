import type { BrowserConfig, BrowserProfileConfig, WoodlsConfig } from "../config/config.js";
...
import {
  ...
  DEFAULT_WOODLS_BROWSER_ENABLED,
...
export type ResolvedBrowserProfile = {
  ...
  cdpIsLoopback: boolean;
  color: string;
  driver: "woodls" | "extension";
};

function normalizeHexColor(raw: string | undefined) {
  const value = (raw ?? "").trim();
  if (!value) {
    return DEFAULT_WOODLS_BROWSER_COLOR;
  }
...
/**
 * Ensure the default "woodls" profile exists in the profiles map.
...
function ensureDefaultProfile(
  ...
  if (!result[DEFAULT_WOODLS_BROWSER_PROFILE_NAME]) {
    result[DEFAULT_WOODLS_BROWSER_PROFILE_NAME] = {
 ...
 *
 * Note: this is an Woodls browser profile (routing config), not a Chrome user profile.
...
function ensureDefaultChromeExtensionProfile(
  ...
  // (legacy single-profile configs may use controlPort+1 for woodls/woodls CDP).
...
export function resolveBrowserConfig(
  ...
  rootConfig?: WoodlsConfig,
): ResolvedBrowserConfig {
  const enabled = cfg?.enabled ?? DEFAULT_WOODLS_BROWSER_ENABLED;
...
export function resolveProfile(
  ...
  let cdpUrl = "";
  const driver = profile.driver === "extension" ? "extension" : "woodls";
