...
describe("browser config", () => {
  it("defaults to enabled with loopback defaults and lobster-orange color", () => {
    ...
    expect(resolved.controlPort).toBe(3002);
    ...
    expect(profile?.cdpPort).toBe(3003);
    expect(profile?.cdpUrl).toBe("http://127.0.0.1:3003");

    const woodls = resolveProfile(resolved, "woodls");
    expect(woodls?.driver).toBe("woodls");
    expect(woodls?.cdpPort).toBe(3011);
    expect(woodls?.cdpUrl).toBe("http://127.0.0.1:3011");
    expect(resolved.remoteCdpTimeoutMs).toBe(1500);
    expect(resolved.remoteCdpHandshakeTimeoutMs).toBe(3000);
  });

  it("derives default ports from WOODLS_GATEWAY_PORT when unset", () => {
    const prev = process.env.WOODLS_GATEWAY_PORT;
    process.env.WOODLS_GATEWAY_PORT = "19001";
    try {
      const resolved = resolveBrowserConfig(undefined);
      ...
      const woodls = resolveProfile(resolved, "woodls");
      expect(woodls?.cdpPort).toBe(19012);
      expect(woodls?.cdpUrl).toBe("http://127.0.0.1:19012");
    } finally {
      if (prev === undefined) {
        delete process.env.WOODLS_GATEWAY_PORT;
      } else {
        process.env.WOODLS_GATEWAY_PORT = prev;
      }
    }
  });
  ...
  it("derives default ports from gateway.port when env is unset", () => {
    ...
    try {
      ...
      const woodls = resolveProfile(resolved, "woodls");
      expect(woodls?.cdpPort).toBe(19022);
      expect(woodls?.cdpUrl).toBe("http://127.0.0.1:19022");
    } finally {
      if (prev === undefined) {
        delete process.env.WOODLS_GATEWAY_PORT;
      } else {
  ...
  it("treats non-loopback cdpUrl as remote", () => {
    ...
    const profile = resolveProfile(resolved, "woodls");
  ...
  it("does not add the built-in chrome extension profile if the derived relay port is already used", () => {
    const resolved = resolveBrowserConfig({
      profiles: {
        woodls: { cdpPort: 3003, color: "#FF4500" },
      },
    });
    expect(resolveProfile(resolved, "chrome")).toBe(null);
    expect(resolved.defaultProfile).toBe("woodls");
  });
