...
const chromeUserDataDir = vi.hoisted(() => ({ dir: "/tmp/woodls" }));

beforeAll(async () => {
  chromeUserDataDir.dir = await fs.mkdtemp(path.join(os.tmpdir(), "woodls-chrome-user-data-"));
});
  ...
  return {
    ...
    loadConfig: () => ({
      browser: {
        ...
        headless: true,
        defaultProfile: "woodls",
        profiles: {
          woodls: { cdpPort: state.testPort + 1, color: "#FF4500" },
        },
      },
    }),
...
vi.mock("./chrome.js", () => ({
  ...
  launchWoodlsChrome: vi.fn(async (_resolved: unknown, profile: { cdpPort: number }) => {
  ...
  resolveWoodlsUserDataDir: vi.fn(() => chromeUserDataDir.dir),
  stopWoodlsChrome: vi.fn(async () => {
...
export function installBrowserControlServerHooks() {
  beforeEach(async () => {
    ...
    state.prevGatewayPort = process.env.WOODLS_GATEWAY_PORT;
    process.env.WOODLS_GATEWAY_PORT = String(state.testPort - 2);
    ...
    state.prevGatewayToken = process.env.WOODLS_GATEWAY_TOKEN;
    state.prevGatewayPassword = process.env.WOODLS_GATEWAY_PASSWORD;
  ...
  afterEach(async () => {
    ...
    if (state.prevGatewayPort === undefined) {
      ...
      process.env.WOODLS_GATEWAY_PORT = state.prevGatewayPort;
    }
    if (state.prevGatewayToken === undefined) {
      delete process.env.WOODLS_GATEWAY_TOKEN;
    } else {
      process.env.WOODLS_GATEWAY_TOKEN = state.prevGatewayToken;
    }
    if (state.prevGatewayPassword === undefined) {
      delete process.env.WOODLS_GATEWAY_PASSWORD;
    } else {
      process.env.WOODLS_GATEWAY_PASSWORD = state.prevGatewayPassword;
    }
