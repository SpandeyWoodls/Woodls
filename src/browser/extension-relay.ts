...
const RELAY_AUTH_HEADER = "x-woodls-relay-token";
...
function resolveGatewayAuthToken(): string | null {
  const envToken =
    process.env.WOODLS_GATEWAY_TOKEN?.trim() || process.env.WOODLS_GATEWAY_TOKEN?.trim();
  if (envToken) {
    return envToken;
  }
  try {
...
async function looksLikeWoodlsRelay(baseUrl: string): Promise<boolean> {
...
export async function ensureChromeExtensionRelayServer(opts: {
  ...
  const routeCdpCommand = async (cmd: CdpCommand): Promise<unknown> => {
    switch (cmd.method) {
      case "Browser.getVersion":
        return {
          protocolVersion: "1.3",
          product: "Chrome/Woodls-Extension-Relay",
          revision: "0",
          userAgent: "Woodls-Extension-Relay",
          jsVersion: "V8",
        };
  ...
  const server = createServer((req, res) => {
    ...
    if (
      ...
      const payload: Record<string, unknown> = {
        Browser: "Woodls/extension-relay",
  ...
  try {
    ...
    if (isAddrInUseError(err) && (await looksLikeWoodlsRelay(info.baseUrl))) {
