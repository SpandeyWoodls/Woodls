...
function decoratedMarkerPath(userDataDir: string) {
  return path.join(userDataDir, ".woodls-profile-decorated");
}
 ...
 */
export function decorateWoodlsProfile(
  userDataDir: string,
  opts?: { name?: string; color?: string },
) {
  const desiredName = opts?.name ?? DEFAULT_WOODLS_BROWSER_PROFILE_NAME;
  const desiredColor = (opts?.color ?? DEFAULT_WOODLS_BROWSER_COLOR).toUpperCase();
