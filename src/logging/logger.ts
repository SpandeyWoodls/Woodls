...
import type { WoodlsConfig } from "../config/types.js";
import { resolvePreferredWoodlsTmpDir } from "../infra/tmp-woodls-dir.js";
import { readLoggingConfig } from "./config.js";
import type { ConsoleStyle } from "./console.js";
import { type LogLevel, levelToMinLevel, normalizeLogLevel } from "./levels.js";
import { loggingState } from "./state.js";

export const DEFAULT_LOG_DIR = resolvePreferredWoodlsTmpDir();
export const DEFAULT_LOG_FILE = path.join(DEFAULT_LOG_DIR, "woodls.log"); // legacy single-file path

const LOG_PREFIX = "woodls";
...
function resolveSettings(): ResolvedSettings {
  let cfg: WoodlsConfig["logging"] | undefined =
    (loggingState.overrideSettings as LoggerSettings | null) ?? readLoggingConfig();
  if (!cfg) {
    try {
      const loaded = requireConfig("../config/config.js") as {
        loadConfig?: () => WoodlsConfig;
      };
  ...
  const defaultLevel =
    process.env.VITEST === "true" && process.env.WOODLS_TEST_FILE_LOG !== "1" ? "silent" : "info";
