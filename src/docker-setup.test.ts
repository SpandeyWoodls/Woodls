...
async function createDockerSetupSandbox(): Promise<DockerSetupSandbox> {
  const rootDir = await mkdtemp(join(tmpdir(), "woodls-docker-setup-"));
...
function createEnv(
  ...
  const env: NodeJS.ProcessEnv = {
    ...
    WOODLS_GATEWAY_TOKEN: "test-token",
    WOODLS_CONFIG_DIR: join(sandbox.rootDir, "config"),
    WOODLS_WORKSPACE_DIR: join(sandbox.rootDir, "woodls"),
  };
  ...
  it("handles env defaults, home-volume mounts, and apt build args", async () => {
    ...
    const result = spawnSync("bash", [sandbox.scriptPath], {
      ...
      env: createEnv(sandbox, {
        WOODLS_DOCKER_APT_PACKAGES: "ffmpeg build-essential",
        WOODLS_EXTRA_MOUNTS: undefined,
        WOODLS_HOME_VOLUME: "woodls-home",
      }),
      stdio: ["ignore", "ignore", "pipe"],
    });
    expect(result.status).toBe(0);
    const envFile = await readFile(join(sandbox.rootDir, ".env"), "utf8");
    expect(envFile).toContain("WOODLS_DOCKER_APT_PACKAGES=ffmpeg build-essential");
    expect(envFile).toContain("WOODLS_EXTRA_MOUNTS=");
    expect(envFile).toContain("WOODLS_HOME_VOLUME=woodls-home");
    const extraCompose = await readFile(join(sandbox.rootDir, "docker-compose.extra.yml"), "utf8");
    expect(extraCompose).toContain("woodls-home:/home/node");
    expect(extraCompose).toContain("volumes:");
    expect(extraCompose).toContain("woodls-home:");
    const log = await readFile(sandbox.logPath, "utf8");
    expect(log).toContain("--build-arg WOODLS_DOCKER_APT_PACKAGES=ffmpeg build-essential");
  });
