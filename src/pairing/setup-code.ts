...
function resolveGatewayPort(cfg: WoodlsConfig, env: NodeJS.ProcessEnv): number {
...
function resolveAuth(cfg: WoodlsConfig, env: NodeJS.ProcessEnv): ResolveAuthResult {
  ...
  const password =
    env.WOODLS_GATEWAY_PASSWORD?.trim() ||
    env.WOODLS_GATEWAY_PASSWORD?.trim() ||
