/**
 * Gateway auth middleware for HTTP and WebSocket connections.
 *
 * Contains the core authorization functions that were previously in
 * src/gateway/auth.ts, extracted here so they can be tested independently
...
import { maskSecret, safeEqualSecret } from "../../security/secret-equal.js";
import {
  isLoopbackAddress,
...
export function resolveGatewayAuth(params: {
  ...
  const token = authConfig.token ?? env.WOODLS_GATEWAY_TOKEN ?? undefined;
  const password = authConfig.password ?? env.WOODLS_GATEWAY_PASSWORD ?? undefined;
...
/**
 * Return a copy of ResolvedGatewayAuth with token/password masked for safe logging.
 */
export function sanitizeAuthForLog(auth: ResolvedGatewayAuth): Record<string, unknown> {
  return {
    mode: auth.mode,
    token: auth.token !== undefined ? maskSecret(auth.token) : undefined,
    password: auth.password !== undefined ? maskSecret(auth.password) : undefined,
...
export function assertGatewayAuthConfigured(auth: ResolvedGatewayAuth): void {
  if (auth.mode === "token" && !auth.token) {
    ...
    throw new Error(
      "gateway auth mode is token, but no token was configured (set gateway.auth.token or WOODLS_GATEWAY_TOKEN)",
    );
  }
