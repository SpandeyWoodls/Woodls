...
export function resolveHooksConfig(cfg: WoodlsConfig): HooksConfigResolved | null {
...
function resolveKnownAgentIds(cfg: WoodlsConfig, defaultAgentId: string): Set<string> {
...
export function extractHookToken(req: IncomingMessage): string | undefined {
  ...
  const headerToken =
    typeof req.headers["x-woodls-token"] === "string" ? req.headers["x-woodls-token"].trim() : "";
  if (headerToken) {
    return headerToken;
  }
  return undefined;
