    ...
    try {
...

      {
        await expectAgentSessionKeyMatch({
          body: { model: "woodls", messages: [{ role: "user", content: "hi" }] },
          headers: { "x-woodls-agent-id": "beta" },
          matcher: /^agent:beta:/,
        });
      }
...

      {
        await expectAgentSessionKeyMatch({
          ...
          headers: { "x-woodls-agent-id": "alpha" },
          matcher: /^agent:alpha:/,
        });
      }

      {
        ...
        const res = await postChatCompletions(
          port,
          { model: "woodls", messages: [{ role: "user", content: "hi" }] },
          {
            "x-woodls-agent-id": "beta",
            "x-woodls-session-key": "agent:beta:openai:custom",
          },
        );
        expect(res.status).toBe(200);
