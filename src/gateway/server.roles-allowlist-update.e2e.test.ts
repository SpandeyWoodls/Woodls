...
const connectNodeClient = async (params: {
  ...
  const token = process.env.WOODLS_GATEWAY_TOKEN;
  if (!token) {
    throw new Error("WOODLS_GATEWAY_TOKEN is required for node test clients");
  }
...
describe("gateway update.run", () => {
  test("writes sentinel and schedules restart", async () => {
    ...
    try {
      ...
      const sentinelPath = path.join(os.homedir(), ".woodls", "restart-sentinel.json");
