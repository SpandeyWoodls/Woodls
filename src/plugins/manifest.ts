...
export const PLUGIN_MANIFEST_FILENAME = "woodls.plugin.json";
...
// package.json "woodls" metadata (used for onboarding/catalog)
...
export type WoodlsPackageManifest = {
  extensions?: string[];
...
): WoodlsPackageManifest | undefined {
  if (!manifest) {
    return undefined;
  }
  const allKeys = [MANIFEST_KEY, ...LEGACY_MANIFEST_KEYS] as const;
  for (const key of allKeys) {
    const entry = (manifest as Record<string, WoodlsPackageManifest | undefined>)[key];
    if (entry !== undefined) {
      return entry;
    }
  }
