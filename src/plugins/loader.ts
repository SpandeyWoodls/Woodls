...
function resolvePluginModuleExport(moduleExport: unknown): {
  definition?: WoodlsPluginDefinition;
  register?: WoodlsPluginDefinition["register"];
} {
  const resolved =
    moduleExport &&
  ...
  if (typeof resolved === "function") {
    return {
      register: resolved as WoodlsPluginDefinition["register"],
    };
  }
  if (resolved && typeof resolved === "object") {
    const def = resolved as WoodlsPluginDefinition;
...
export function loadWoodlsPlugins(options: PluginLoadOptions = {}): PluginRegistry {
  ...
  const discovery = discoverWoodlsPlugins({
    workspaceDir: options.workspaceDir,
  ...
  const getJiti = () => {
    ...
    jitiLoader = createJiti(import.meta.url, {
      ...
      ...(pluginSdkAlias || pluginSdkAccountIdAlias
        ? {
            alias: {
              ...(pluginSdkAlias ? { "woodls/plugin-sdk": pluginSdkAlias } : {}),
              ...(pluginSdkAccountIdAlias
                ? { "woodls/plugin-sdk/account-id": pluginSdkAccountIdAlias }
                : {}),
            },
          }
        : {}),
  ...
  for (const candidate of discovery.candidates) {
    ...
    let mod: WoodlsPluginModule | null = null;
    try {
      mod = getJiti()(candidate.source) as WoodlsPluginModule;
    } catch (err) {
