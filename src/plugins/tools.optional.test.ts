...
const loadWoodlsPluginsMock = vi.fn();

vi.mock("./loader.js", () => ({
  loadWoodlsPlugins: (params: unknown) => loadWoodlsPluginsMock(params),
}));
...
function setRegistry(entries: MockRegistryToolEntry[]) {
  ...
  loadWoodlsPluginsMock.mockReturnValue(registry);
  return registry;
}

describe("resolvePluginTools optional tools", () => {
  beforeEach(() => {
    loadWoodlsPluginsMock.mockReset();
  });
