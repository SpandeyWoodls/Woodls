...
function makeTempDir() {
  const dir = path.join(os.tmpdir(), `woodls-plugin-install-${randomUUID()}`);
    ...
    JSON.stringify(
      {
        ...
        woodls: { extensions: params.extensions },
      },
      null,
      2,
    ),
...
async function expectArchiveInstallReservedSegmentRejection(params: {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({
      ...
      version: "0.0.1",
      woodls: { extensions: ["./dist/index.js"] },
    }),
    "utf-8",
  );
...
describe("installPluginFromArchive", () => {
  it("installs into ~/.woodls/extensions and uses unscoped id", async () => {
  ...
  it("rejects packages without woodls.extensions", async () => {
    const stateDir = makeTempDir();
    const workDir = makeTempDir();
    const pkgDir = path.join(workDir, "package");
    ...
    fs.writeFileSync(
      path.join(pkgDir, "package.json"),
      JSON.stringify({ name: "@woodls/nope", version: "0.0.1" }),
      "utf-8",
    );
    ...
    expect(result.error).toContain("woodls.extensions");
  });
...
describe("installPluginFromNpmSpec", () => {
  it("uses --ignore-scripts for npm pack and cleans up temp dir", async () => {
    ...
    expectSingleNpmPackIgnoreScriptsCall({
      ...
      expectedSpec: "@woodls/voice-call@0.0.1",
    });
