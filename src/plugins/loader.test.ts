...
import { loadWoodlsPlugins } from "./loader.js";

type TempPlugin = { dir: string; file: string; id: string };

const fixtureRoot = path.join(os.tmpdir(), `woodls-plugin-${randomUUID()}`);
let tempDirIndex = 0;
const prevBundledDir = process.env.WOODLS_BUNDLED_PLUGINS_DIR;
...
function writePlugin(params: {
  ...
  fs.writeFileSync(
    path.join(dir, "woodls.plugin.json"),
    JSON.stringify(
      {
        id: params.id,
...
function loadBundledMemoryPluginRegistry(options?: {
  ...
  process.env.WOODLS_BUNDLED_PLUGINS_DIR = bundledDir;

  return loadWoodlsPlugins({
    cache: false,
    config: {
      plugins: {
        slots: {
...
afterEach(() => {
  if (prevBundledDir === undefined) {
    ...
    process.env.WOODLS_BUNDLED_PLUGINS_DIR = prevBundledDir;
  }
});
...
describe("loadWoodlsPlugins", () => {
  it("disables bundled plugins by default", () => {
    ...
    const enabledRegistry = loadWoodlsPlugins({
      cache: false,
      config: {
        plugins: {
          allow: ["bundled"],
