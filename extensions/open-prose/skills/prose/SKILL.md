...
metadata: { "woodls": { "emoji": "🪶", "homepage": "https://www.prose.md" } }
---
...
- **Task tool** in the upstream spec == Woodls `sessions_spawn`
- **File I/O** == Woodls `read`/`write`
- **Remote fetch** == Woodls `web_fetch` (or `exec` with curl when POST is required)
