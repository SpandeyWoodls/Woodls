...
import { addWildcardAllowFrom, DEFAULT_ACCOUNT_ID, formatDocsLink } from "woodls/plugin-sdk";
...
function setFeishuDmPolicy(cfg: WoodlsConfig, dmPolicy: DmPolicy): WoodlsConfig {
  const allowFrom =
    dmPolicy === "open"
...
function setFeishuAllowFrom(cfg: WoodlsConfig, allowFrom: string[]): WoodlsConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
...
function setFeishuGroupAllowFrom(cfg: WoodlsConfig, groupAllowFrom: string[]): WoodlsConfig {
  return {
    ...cfg,
    channels: {
      ...cfg.channels,
