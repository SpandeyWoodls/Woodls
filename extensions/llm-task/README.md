...
`woodls.invoke --each`) without adding new Woodls code per workflow.
...
This extension depends on Woodls internal modules (the embedded agent runner).
It is intended to ship as a **bundled** Woodls extension (like `lobster`) and
be enabled via `plugins.entries` + tool allowlists.

It is **not** currently designed to be copied into
`~/.woodls/extensions` as a standalone plugin directory.
