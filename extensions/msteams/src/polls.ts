...
export function extractMSTeamsPollVote(
  ...
  const pollId =
    readNestedString(value, ["woodlsPollId"]) ??
    readNestedString(value, ["pollId"]) ??
    readNestedString(value, ["woodls", "pollId"]) ??
    readNestedString(value, ["woodls", "poll", "id"]) ??
    readNestedString(value, ["data", "woodlsPollId"]) ??
    readNestedString(value, ["data", "pollId"]) ??
    readNestedString(value, ["data", "woodls", "pollId"]);
  if (!pollId) {
    return null;
  }
    ...
    actions: [
      {
        ...
        msteams: {
          type: "messageBack",
          text: "woodls poll vote",
          displayText: "Vote recorded",
          value: { woodlsPollId: pollId, pollId },
        },
      },
    ],
  };
