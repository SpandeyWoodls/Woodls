/**
 * Twitch channel plugin for Woodls.
 *
...
import { buildChannelConfigSchema } from "woodls/plugin-sdk";
 ...
 * for Woodls. Supports message sending, receiving, access control, and
 * status monitoring.
 */
export const twitchPlugin: ChannelPlugin<TwitchAccountConfig> = {
  ...
  config: {
    ...
    listAccountIds: (cfg: WoodlsConfig): string[] => listAccountIds(cfg),

    /** Resolve an account config by ID */
    resolveAccount: (cfg: WoodlsConfig, accountId?: string | null): TwitchAccountConfig => {
    ...
    isConfigured: (_account: unknown, cfg: WoodlsConfig): boolean => {
