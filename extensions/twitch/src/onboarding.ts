 ...
 */
function setTwitchAccount(cfg: WoodlsConfig, account: Partial<TwitchAccountConfig>): WoodlsConfig {
...
async function noteTwitchSetupHelp(prompter: WizardPrompter): Promise<void> {
  await prompter.note(
    [
      ...
      "Env vars supported: WOODLS_TWITCH_ACCESS_TOKEN",
...
): Promise<{ cfg: WoodlsConfig } | null> {
  const useEnv = await prompter.confirm({
    message: "Twitch env var WOODLS_TWITCH_ACCESS_TOKEN detected. Use env token?",
    initialValue: true,
  });
  if (!useEnv) {
    return null;
...
export const twitchOnboardingAdapter: ChannelOnboardingAdapter = {
  ...
  configure: async ({ cfg, prompter, forceAllowFrom }) => {
    ...
    const envToken = process.env.WOODLS_TWITCH_ACCESS_TOKEN?.trim();
