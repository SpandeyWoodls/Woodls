...
import { approveDevicePairing, listDevicePairing } from "woodls/plugin-sdk";
...
function resolveGatewayPort(cfg: WoodlsPluginApi["config"]): number {
  const envPort =
    parsePositiveInteger(process.env.WOODLS_GATEWAY_PORT?.trim()) ??
    parsePositiveInteger(process.env.WOODLS_GATEWAY_PORT?.trim());
  if (envPort) {
    return envPort;
  }
...
async function resolveTailnetHost(api: WoodlsPluginApi): Promise<string | null> {
...
function resolveAuth(cfg: WoodlsPluginApi["config"]): ResolveAuthResult {
...
async function resolveGatewayUrl(api: WoodlsPluginApi): Promise<ResolveUrlResult> {
  ...
  api.registerCommand({
    ...
    handler: async (ctx) => {
      ...
      if (action === "qr") {
        ...
        if (channel === "telegram" && target) {
          try {
            ...
            if (send) {
              await send(
                target,
                ["Scan this QR code with the Woodls iOS app:", "", "```", qrAscii, "```"].join(
                  "\n",
                ),
                {
        ...
        return {
          text: [
            "Scan this QR code with the Woodls iOS app:",
            "",
            "```",
            qrAscii,
            "```",
