...
function resolveWoodlsRoot(): string {
  if (coreRootCache) {
    return coreRootCache;
  }
  const override = process.env.WOODLS_ROOT?.trim();
  if (override) {
  ...
  throw new Error("Unable to resolve core root. Set WOODLS_ROOT to the package root.");
}
...
async function importCoreExtensionAPI(): Promise<{
  ...
  const distPath = path.join(resolveWoodlsRoot(), "dist", "extensionAPI.js");
