...
import type { WoodlsPluginApi, WoodlsPluginService } from "woodls/plugin-sdk";
...
function normalizeDenyList(cfg: WoodlsPluginApi["config"]): string[] {
  return uniqSorted([...(cfg.gateway?.nodes?.denyCommands ?? [])]);
}

function normalizeAllowList(cfg: WoodlsPluginApi["config"]): string[] {
...
export default function register(api: WoodlsPluginApi) {
  ...
  const timerService: WoodlsPluginService = {
