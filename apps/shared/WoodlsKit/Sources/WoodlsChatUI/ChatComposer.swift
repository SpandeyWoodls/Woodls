...
@MainActor
struct WoodlsChatComposer: View {
    @Bindable var viewModel: WoodlsChatViewModel
        ...
        .background {
            ...
            if self.style == .standard {
                let shape = UnevenRoundedRectangle(
                    ...
                    style: .continuous)
                shape
                    .fill(WoodlsChatTheme.composerBackground)
                    .overlay(shape.strokeBorder(WoodlsChatTheme.composerBorder, lineWidth: 1))
        ...
        ScrollView(.horizontal, showsIndicators: false) {
            HStack(spacing: 6) {
                ForEach(
                    ...
                    id: \WoodlsPendingAttachment.id)
                { (att: WoodlsPendingAttachment) in
                    HStack(spacing: 6) {
                        if let img = att.preview {
                            WoodlsPlatformImageFactory.image(img)
                                .resizable()
                                .scaledToFill()
    ...
    private var editor: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(WoodlsChatTheme.composerField)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
                        .strokeBorder(WoodlsChatTheme.composerBorder)))
    ...
    private var connectionPill: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(WoodlsChatTheme.subtleCard)
        .clipShape(Capsule())
    }
