...
private let chatUILogger = Logger(subsystem: "ai.woodls", category: "WoodlsChatUI")

@MainActor
@Observable
public final class WoodlsChatViewModel {
    public private(set) var messages: [WoodlsChatMessage] = []
    ...
    public var attachments: [WoodlsPendingAttachment] = []
    ...
    public private(set) var pendingToolCalls: [WoodlsChatPendingToolCall] = []
    public private(set) var sessions: [WoodlsChatSessionEntry] = []
    private let transport: any WoodlsChatTransport
    ...
    private var pendingToolCallsById: [String: WoodlsChatPendingToolCall] = [:] {
        didSet {
    ...
    public init(sessionKey: String, transport: any WoodlsChatTransport) {
    ...
    public var sessionChoices: [WoodlsChatSessionEntry] {
        let now = Date().timeIntervalSince1970 * 1000
        let cutoff = now - (24 * 60 * 60 * 1000)
        let sorted = self.sessions.sorted { ($0.updatedAt ?? 0) > ($1.updatedAt ?? 0) }

        var result: [WoodlsChatSessionEntry] = []
    ...
    public func removeAttachment(_ id: WoodlsPendingAttachment.ID) {
    ...
    private static func decodeMessages(_ raw: [AnyCodable]) -> [WoodlsChatMessage] {
        let decoded = raw.compactMap { item in
            (try? ChatPayloadDecoding.decode(item, as: WoodlsChatMessage.self))
        }
        return Self.dedupeMessages(decoded)
    }

    private static func messageIdentityKey(for message: WoodlsChatMessage) -> String? {
    ...
    private static func reconcileMessageIDs(
        previous: [WoodlsChatMessage],
        incoming: [WoodlsChatMessage]) -> [WoodlsChatMessage]
    {
    ...
    private static func dedupeMessages(_ messages: [WoodlsChatMessage]) -> [WoodlsChatMessage] {
        var result: [WoodlsChatMessage] = []
    ...
    private static func dedupeKey(for message: WoodlsChatMessage) -> String? {
    ...
    private func performSend() async {
        ...
        var userContent: [WoodlsChatMessageContent] = [
            WoodlsChatMessageContent(
                type: "text",
                text: messageText,
                thinking: nil,
        ...
        let encodedAttachments = self.attachments.map { att -> WoodlsChatAttachmentPayload in
            WoodlsChatAttachmentPayload(
                type: att.type,
    ...
    private func placeholderSession(key: String) -> WoodlsChatSessionEntry {
        WoodlsChatSessionEntry(
            key: key,
            kind: nil,
            displayName: nil,
            surface: nil,
    ...
    private func handleTransportEvent(_ evt: WoodlsChatTransportEvent) {
        switch evt {
        case let .health(ok):
            self.healthOK = ok
        case .tick:
    ...
    private func handleChatEvent(_ chat: WoodlsChatEventPayload) {
    ...
    private func handleAgentEvent(_ evt: WoodlsAgentEventPayload) {
        ...
        case "tool":
            ...
            if phase == "start" {
                ...
                self.pendingToolCallsById[toolCallId] = WoodlsChatPendingToolCall(
    ...
    private static func previewImage(data: Data) -> WoodlsPlatformImage? {
        #if canImport(AppKit)
        NSImage(data: data)
        #elseif canImport(UIKit)
        UIImage(data: data)
