...
struct ChatMessageBubble: View {
    let message: WoodlsChatMessage
    let style: WoodlsChatView.Style
...
private struct ChatMessageBody: View {
    ...
    var body: some View {
        ...
        let textColor = self.isUser ? WoodlsChatTheme.userText : WoodlsChatTheme.assistantText
    ...
    private var inlineAttachments: [WoodlsChatMessageContent] {
    ...
    private var toolCalls: [WoodlsChatMessageContent] {
        self.message.content.filter { content in
    ...
    private var inlineToolResults: [WoodlsChatMessageContent] {
        self.message.content.filter { content in
            let kind = (content.type ?? "").lowercased()
    ...
    private var bubbleFillColor: Color {
        if self.isUser {
            return self.userAccent ?? WoodlsChatTheme.userBubble
        }
        if self.style == .onboarding {
            return WoodlsChatTheme.onboardingAssistantBubble
        }
        return WoodlsChatTheme.assistantBubble
    }
    ...
    private var bubbleBorderColor: Color {
        ...
        if self.style == .onboarding {
            return WoodlsChatTheme.onboardingAssistantBorder
        }
...
private struct AttachmentRow: View {
    let att: WoodlsChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(self.att.fileName ?? "Attachment")
                .font(.footnote)
                .lineLimit(1)
                .foregroundStyle(self.isUser ? WoodlsChatTheme.userText : WoodlsChatTheme.assistantText)
            Spacer()
        }
        .padding(10)
...
private struct ToolCallCard: View {
    let content: WoodlsChatMessageContent
    let isUser: Bool
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 12, style: .continuous)
                .fill(WoodlsChatTheme.subtleCard)
                .overlay(
                    RoundedRectangle(cornerRadius: 12, style: .continuous)
    ...
    var body: some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .fill(WoodlsChatTheme.assistantBubble))
        .overlay(
            RoundedRectangle(cornerRadius: 16, style: .continuous)
                .strokeBorder(Color.white.opacity(0.08), lineWidth: 1))
...
struct ChatPendingToolsBubble: View {
    let toolCalls: [WoodlsChatPendingToolCall]
        ...
        VStack(alignment: .leading, spacing: 10) {
            ForEach(segments) { segment in
                ...
                ChatMarkdownRenderer(
                    ...
                    variant: self.markdownVariant,
                    font: font,
                    textColor: WoodlsChatTheme.assistantText)
            }
        }
    }
}
