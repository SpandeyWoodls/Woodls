...
private final class TestChatTransport: @unchecked Sendable, WoodlsChatTransport {
    private let state = TestChatTransportState()
    private let historyResponses: [WoodlsChatHistoryPayload]
    private let sessionsResponses: [WoodlsChatSessionsListResponse]

    private let stream: AsyncStream<WoodlsChatTransportEvent>
    private let continuation: AsyncStream<WoodlsChatTransportEvent>.Continuation

    init(
        historyResponses: [WoodlsChatHistoryPayload],
        sessionsResponses: [WoodlsChatSessionsListResponse] = [])
    {
        self.historyResponses = historyResponses
        self.sessionsResponses = sessionsResponses
        var cont: AsyncStream<WoodlsChatTransportEvent>.Continuation!
    ...
    func requestHistory(sessionKey: String) async throws -> WoodlsChatHistoryPayload {
        ...
        return self.historyResponses.last ?? WoodlsChatHistoryPayload(
    ...
    func sendMessage(
        ...
        attachments _: [WoodlsChatAttachmentPayload]) async throws -> WoodlsChatSendResponse
    {
        await self.state.sentRunIdsAppend(idempotencyKey)
        return WoodlsChatSendResponse(runId: idempotencyKey, status: "ok")
    }
    ...
    func listSessions(limit _: Int?) async throws -> WoodlsChatSessionsListResponse {
        ...
        return self.sessionsResponses.last ?? WoodlsChatSessionsListResponse(
            ts: nil,
            path: nil,
            count: 0,
            defaults: nil,
    ...
    func emit(_ evt: WoodlsChatTransportEvent) {
...
@Suite struct ChatViewModelTests {
    @Test func streamsAssistantAndClearsOnFinal() async throws {
        ...
        let history1 = WoodlsChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
        let history2 = WoodlsChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [
                AnyCodable([
        ...
        let vm = await MainActor.run { WoodlsChatViewModel(sessionKey: "main", transport: transport) }
    ...
    @Test func clearsStreamingOnExternalFinalEvent() async throws {
        let sessionId = "sess-main"
        let history = WoodlsChatHistoryPayload(
            sessionKey: "main",
            sessionId: sessionId,
            messages: [],
            thinkingLevel: "off")
    ...
    @Test func sessionChoicesPreferMainAndRecent() async throws {
        ...
        let history = WoodlsChatHistoryPayload(
            ...
            messages: [],
            thinkingLevel: "off")
        let sessions = WoodlsChatSessionsListResponse(
            ts: now,
            path: nil,
            count: 4,
            defaults: nil,
    ...
    @Test func sessionChoicesIncludeCurrentWhenMissing() async throws {
        ...
        let transport = TestChatTransport(
            historyResponses: [history],
            sessionsResponses: [sessions])
        let vm = await MainActor.run { WoodlsChatViewModel(sessionKey: "custom", transport: transport) }
        await MainActor.run { vm.load() }
        try await waitUntil("sessions loaded") { await MainActor.run { !vm.sessions.isEmpty } }
