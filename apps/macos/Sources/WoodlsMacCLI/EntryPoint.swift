...
struct WoodlsMacCLI {
    static func main() async {
        ...
        default:
            fputs("woodls-mac: unknown command\n", stderr)
            printUsage()
            exit(1)
        }
    }
...
private func printUsage() {
    ...
    Usage:
      woodls-mac connect [--url <ws://host:port>] [--token <token>] [--password <password>]
                           [--mode <local|remote>] [--timeout <ms>] [--probe] [--json]
                           [--client-id <id>] [--client-mode <mode>] [--display-name <name>]
                           [--role <role>] [--scopes <a,b,c>]
      woodls-mac discover [--timeout <ms>] [--json] [--include-local]
      woodls-mac wizard [--url <ws://host:port>] [--token <token>] [--password <password>]
                          [--mode <local|remote>] [--workspace <path>] [--json]

    Examples:
      woodls-mac connect
      woodls-mac connect --url ws://127.0.0.1:18789 --json
      woodls-mac discover --timeout 3000 --json
      woodls-mac wizard --mode local
    """)
}
