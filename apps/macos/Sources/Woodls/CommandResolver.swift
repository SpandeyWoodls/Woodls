...
enum CommandResolver {
    private static let projectRootDefaultsKey = "woodls.gatewayProjectRootPath"
    private static let helperName = "woodls"

    static func gatewayEntrypoint(in root: URL) -> String? {
        let distEntry = root.appendingPathComponent("dist/index.js").path
        if FileManager().isReadableFile(atPath: distEntry) { return distEntry }
        let woodlsEntry = root.appendingPathComponent("woodls.mjs").path
        if FileManager().isReadableFile(atPath: woodlsEntry) { return woodlsEntry }
        let binEntry = root.appendingPathComponent("bin/woodls.js").path
    ...
    static func projectRoot() -> URL {
        ...
        let fallback = FileManager().homeDirectoryForCurrentUser
            .appendingPathComponent("Projects/woodls")
    ...
    static func preferredPaths(home: URL, current: [String], projectRoot: URL) -> [String] {
        ...
        #endif
        let woodlsPaths = self.woodlsManagedPaths(home: home)
        if !woodlsPaths.isEmpty {
            extras.insert(contentsOf: woodlsPaths, at: 1)
        }
        extras.insert(contentsOf: self.nodeManagerBinPaths(home: home), at: 1 + woodlsPaths.count)
        var seen = Set<String>()
        // Preserve order while stripping duplicates so PATH lookups remain deterministic.
        return (extras + current).filter { seen.insert($0).inserted }
    }

    private static func woodlsManagedPaths(home: URL) -> [String] {
        let bases = [
            home.appendingPathComponent(".woodls"),
        ]
        var paths: [String] = []
        for base in bases {
    ...
    static func woodlsExecutable(searchPaths: [String]? = nil) -> String? {
        self.findExecutable(named: self.helperName, searchPaths: searchPaths)
    }

    static func projectWoodlsExecutable(projectRoot: URL? = nil) -> String? {
        #if DEBUG
    ...
    static func nodeCliPath() -> String? {
        ...
        let candidates = [
            root.appendingPathComponent("woodls.mjs").path,
            root.appendingPathComponent("bin/woodls.js").path,
        ]
    ...
    static func hasAnyWoodlsInvoker(searchPaths: [String]? = nil) -> Bool {
        if self.woodlsExecutable(searchPaths: searchPaths) != nil { return true }
        ...
        searchPaths: [String]? = nil) -> [String]
    {
        ...
        case let .success(runtime):
            let root = self.projectRoot()
            if let woodlsPath = self.projectWoodlsExecutable(projectRoot: root) {
                return [woodlsPath, subcommand] + extraArgs
            }
            ...
            if let pnpm = self.findExecutable(named: "pnpm", searchPaths: searchPaths) {
                ...
                return [pnpm, "--silent", "woodls", subcommand] + extraArgs
            }
            if let woodlsPath = self.woodlsExecutable(searchPaths: searchPaths) {
                return [woodlsPath, subcommand] + extraArgs
            }

            let missingEntry = """
            woodls entrypoint missing (looked for dist/index.js or woodls.mjs); run pnpm build.
            """
    ...
    private static func sshNodeCommand(subcommand: String, extraArgs: [String], settings: RemoteSettings) -> [String]? {
        ...
        // Run the real woodls CLI on the remote host.
        let exportedPath = [
            "/opt/homebrew/bin",
            "/usr/local/bin",
            "/usr/bin",
        ...
        let projectSection = if userPRJ.isEmpty {
            """
            DEFAULT_PRJ="$HOME/Projects/woodls"
        ...
        \(cliSection)
        \(projectSection)
        if command -v woodls >/dev/null 2>&1; then
          CLI="$(command -v woodls)"
          woodls \(quotedArgs);
        ...
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/woodls.mjs" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/woodls.mjs"
            node "$PRJ/woodls.mjs" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif [ -n "${PRJ:-}" ] && [ -f "$PRJ/bin/woodls.js" ]; then
          if command -v node >/dev/null 2>&1; then
            CLI="node $PRJ/bin/woodls.js"
            node "$PRJ/bin/woodls.js" \(quotedArgs);
          else
            echo "Node >=22 required on remote host"; exit 127;
          fi
        elif command -v pnpm >/dev/null 2>&1; then
          CLI="pnpm --silent woodls"
          pnpm --silent woodls \(quotedArgs);
        else
          echo "woodls CLI missing on remote host"; exit 127;
        fi
        """
        let options: [String] = [
            "-o", "BatchMode=yes",
    ...
    {
        let root = configRoot ?? WoodlsConfigFile.loadDict()
