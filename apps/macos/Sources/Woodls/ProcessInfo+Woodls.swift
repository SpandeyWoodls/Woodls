...
extension ProcessInfo {
    ...
    /// Nix deployments may write defaults into a stable suite (`ai.woodls.mac`) even if the shipped
    ...
    {
        if environment["WOODLS_NIX_MODE"] == "1" { return true }
        if standard.bool(forKey: "woodls.nixMode") { return true }
        ...
        if isAppBundle, let stableSuite, stableSuite.bool(forKey: "woodls.nixMode") { return true }
