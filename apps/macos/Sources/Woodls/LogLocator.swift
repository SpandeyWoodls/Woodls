...
enum LogLocator {
    private static var logDir: URL {
        if let override = ProcessInfo.processInfo.environment["WOODLS_LOG_DIR"],
           !override.isEmpty
        {
            return URL(fileURLWithPath: override)
        }
        return URL(fileURLWithPath: "/tmp/woodls")
    }

    private static var stdoutLog: URL {
        logDir.appendingPathComponent("woodls-stdout.log")
    }

    private static var gatewayLog: URL {
        logDir.appendingPathComponent("woodls-gateway.log")
    }
    ...
    /// Returns the newest log file under /tmp/woodls/ (rolling or stdout), or nil if none exist.
