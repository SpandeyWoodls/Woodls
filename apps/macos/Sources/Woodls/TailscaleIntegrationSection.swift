...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in ~/.woodls/woodls.json. Prefer WOODLS_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to ~/.woodls/woodls.json. Restarting gateway…"
        } else {
            self.statusMessage = "Saved to ~/.woodls/woodls.json. Restart the gateway to apply."
        }
