...
private let webChatSwiftLogger = Logger(subsystem: "ai.woodls", category: "WebChatSwiftUI")
...
struct MacGatewayChatTransport: WoodlsChatTransport, Sendable {
    ...
    func listSessions(limit: Int?) async throws -> WoodlsChatSessionsListResponse {
        ...
        let data = try await GatewayConnection.shared.request(
            ...
            params: params,
            timeoutMs: 15000)
        return try JSONDecoder().decode(WoodlsChatSessionsListResponse.self, from: data)
    }
    ...
    static func mapPushToTransportEvent(_ push: GatewayPush) -> WoodlsChatTransportEvent? {
        switch push {
        case let .snapshot(hello):
            let ok = (try? JSONDecoder().decode(
                WoodlsGatewayHealthOK.self,
...
final class WebChatSwiftUIWindowController {
    ...
    private let hosting: NSHostingController<WoodlsChatView>
    ...
    init(sessionKey: String, presentation: WebChatPresentation, transport: any WoodlsChatTransport) {
        self.sessionKey = sessionKey
        self.presentation = presentation
        let vm = WoodlsChatViewModel(sessionKey: sessionKey, transport: transport)
        let accent = Self.color(fromHex: AppStateStore.shared.seamColorHex)
        self.hosting = NSHostingController(rootView: WoodlsChatView(
            viewModel: vm,
    ...
    private static func makeContentController(
        for presentation: WebChatPresentation,
        hosting: NSHostingController<WoodlsChatView>) -> NSViewController
    {
