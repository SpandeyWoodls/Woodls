...
enum DebugActions {
    private static let verboseDefaultsKey = "woodls.debug.verboseMain"
    private static let sessionMenuLimit = 12
    private static let onboardingSeenKey = "woodls.onboardingSeen"
    ...
    static func openConfigFolder() {
        let url = WoodlsPaths.stateDirURL
        NSWorkspace.shared.activateFileViewerSelecting([url])
    }
    ...
    static func sendTestNotification() async {
        _ = await NotificationManager().send(title: "Woodls", body: "Test notification", sound: nil)
    }
    ...
    private static func resolveSessionStorePath() -> String {
        ...
        let configURL = WoodlsPaths.configURL
        guard
