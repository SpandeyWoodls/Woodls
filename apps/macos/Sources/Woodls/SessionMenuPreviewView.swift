...
enum SessionMenuPreviewLoader {
    private static let logger = Logger(subsystem: "ai.woodls", category: "SessionPreview")
    ...
    private static func requestPreview(
        keys: [String],
        maxItems: Int) async throws -> WoodlsSessionsPreviewPayload
    {
    ...
    private static func snapshot(
        from entry: WoodlsSessionPreviewEntry,
    ...
    private static func cache(payload: WoodlsSessionsPreviewPayload, maxItems: Int) async {
    ...
    private static func previewItems(
        from payload: WoodlsChatHistoryPayload,
        maxItems: Int) -> [SessionPreviewItem]
    {
        let boundedItems = self.normalizeMaxItems(maxItems)
        let raw: [WoodlsKit.AnyCodable] = payload.messages ?? []
    ...
    private static func decodeMessages(_ raw: [WoodlsKit.AnyCodable]) -> [WoodlsChatMessage] {
        raw.compactMap { item in
            guard let data = try? JSONEncoder().encode(item) else { return nil }
            return try? JSONDecoder().decode(WoodlsChatMessage.self, from: data)
        }
    }
    ...
    private static func previewText(for message: WoodlsChatMessage) -> String? {
    ...
    private static func isToolCall(_ message: WoodlsChatMessage) -> Bool {
    ...
    private static func toolNames(for message: WoodlsChatMessage) -> [String] {
        var names: [String] = []
    ...
    private static func mediaSummary(for message: WoodlsChatMessage) -> String? {
