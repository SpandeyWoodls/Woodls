...
struct ControlAgentEvent: Codable, Sendable, Identifiable {
    ...
    let stream: String
    let ts: Double
    let data: [String: WoodlsProtocol.AnyCodable]
    let summary: String?
}
...
final class ControlChannel {
    ...
    private let logger = Logger(subsystem: "ai.woodls", category: "control")
    ...
    {
        do {
            let rawParams = params?.reduce(into: [String: WoodlsKit.AnyCodable]()) {
                $0[$1.key] = WoodlsKit.AnyCodable($1.value.base)
            }
    ...
    private static func bridgeToProtocolArgs(
        _ value: WoodlsProtocol.AnyCodable?) -> [String: WoodlsProtocol.AnyCodable]?
    {
        guard let value else { return nil }
        if let dict = value.value as? [String: WoodlsProtocol.AnyCodable] {
            return dict
        }
        if let dict = value.value as? [String: WoodlsKit.AnyCodable],
           let data = try? JSONEncoder().encode(dict),
           let decoded = try? JSONDecoder().decode([String: WoodlsProtocol.AnyCodable].self, from: data)
        {
            return decoded
        }
...
extension Notification.Name {
    static let controlHeartbeat = Notification.Name("woodls.control.heartbeat")
    static let controlAgentEvent = Notification.Name("woodls.control.agent")
}
