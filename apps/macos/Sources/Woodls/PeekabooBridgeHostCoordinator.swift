...
final class PeekabooBridgeHostCoordinator {
    ...
    private let logger = Logger(subsystem: "ai.woodls", category: "PeekabooBridge")

    private var host: PeekabooBridgeHost?
    private var services: WoodlsPeekabooBridgeServices?
    private static var woodlsSocketPath: String {
        let fileManager = FileManager.default
        let base = fileManager.urls(for: .applicationSupportDirectory, in: .userDomainMask).first
            ?? fileManager.homeDirectoryForCurrentUser.appendingPathComponent("Library/Application Support")
        let directory = base.appendingPathComponent("Woodls", isDirectory: true)
    ...
    private func startIfNeeded() async {
        ...
        let services = WoodlsPeekabooBridgeServices()
        ...
        let host = PeekabooBridgeHost(
            socketPath: Self.woodlsSocketPath,
            server: server,
        ...
        await host.start()
        self.logger
            .info("PeekabooBridge host started at \(Self.woodlsSocketPath, privacy: .public)")
    }
...
@MainActor
private final class WoodlsPeekabooBridgeServices: PeekabooBridgeServiceProviding {
    ...
    init() {
        let logging = LoggingService(subsystem: "ai.woodls.peekaboo")
