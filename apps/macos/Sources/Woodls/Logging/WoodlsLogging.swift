...
enum WoodlsLogging {
    ...
    private static let didBootstrap: Void = {
        LoggingSystem.bootstrap { label in
            ...
            let osHandler = WoodlsOSLogHandler(subsystem: subsystem, category: category)
            let fileHandler = WoodlsFileLogHandler(label: label)
...
extension Logging.Logger {
    init(subsystem: String, category: String) {
        WoodlsLogging.bootstrapIfNeeded()
        let label = WoodlsLogging.makeLabel(subsystem: subsystem, category: category)
...
struct WoodlsOSLogHandler: LogHandler {
...
struct WoodlsFileLogHandler: LogHandler {
    let label: String
    var metadata: Logger.Metadata = [:]
        ...
        line: UInt)
    {
        ...
        let (subsystem, category) = WoodlsLogging.parseLabel(self.label)
