...
@Suite struct MacGatewayChatTransportMappingTests {
    @Test func snapshotMapsToHealth() {
        let snapshot = Snapshot(
            presence: [],
            health: WoodlsProtocol.AnyCodable(["ok": WoodlsProtocol.AnyCodable(false)]),
    ...
    @Test func healthEventMapsToHealth() {
        let frame = EventFrame(
            type: "event",
            event: "health",
            payload: WoodlsProtocol.AnyCodable(["ok": WoodlsProtocol.AnyCodable(true)]),
            seq: 1,
            stateversion: nil)
    ...
    @Test func chatEventMapsToChat() {
        let payload = WoodlsProtocol.AnyCodable([
            "runId": WoodlsProtocol.AnyCodable("run-1"),
            "sessionKey": WoodlsProtocol.AnyCodable("main"),
            "state": WoodlsProtocol.AnyCodable("final"),
        ])
    ...
    @Test func unknownEventMapsToNil() {
        let frame = EventFrame(
            type: "event",
            event: "unknown",
            payload: WoodlsProtocol.AnyCodable(["a": WoodlsProtocol.AnyCodable(1)]),
            seq: 1,
            stateversion: nil)
        let mapped = MacGatewayChatTransport.mapPushToTransportEvent(.event(frame))
        #expect(mapped == nil)
