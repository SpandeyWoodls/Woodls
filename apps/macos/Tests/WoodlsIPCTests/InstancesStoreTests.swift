...
@Suite struct InstancesStoreTests {
    ...
    func presenceEventPayloadDecodesViaJSONEncoder() {
        ...
        let entry: [String: WoodlsProtocol.AnyCodable] = [
            "host": .init("gw"),
            "ip": .init("10.0.0.1"),
            "version": .init("2.0.0"),
            "mode": .init("gateway"),
        ...
        let payloadMap: [String: WoodlsProtocol.AnyCodable] = [
            "presence": .init([WoodlsProtocol.AnyCodable(entry)]),
        ]
        let payload = WoodlsProtocol.AnyCodable(payloadMap)
