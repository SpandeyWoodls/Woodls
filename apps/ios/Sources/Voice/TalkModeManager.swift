...
final class TalkModeManager: NSObject {
    ...
    private static let redactedConfigSentinel = "__WOODLS_REDACTED__"
    ...
    private var pttCompletion: CheckedContinuation<WoodlsTalkPTTStopPayload, Never>?
    ...
    func stop() {
        ...
        if pendingPTT {
            let payload = WoodlsTalkPTTStopPayload(
    ...
    func beginPushToTalk() async throws -> WoodlsTalkPTTStartPayload {
        guard self.gatewayConnected else {
            self.statusText = "Offline"
        ...
        if self.isPushToTalkActive, let captureId = self.activePTTCaptureId {
            return WoodlsTalkPTTStartPayload(captureId: captureId)
        }
    ...
    func endPushToTalk() async -> WoodlsTalkPTTStopPayload {
    ...
    func runPushToTalkOnce(maxDurationSeconds: TimeInterval = 12) async throws -> WoodlsTalkPTTStopPayload {
        ...
        if self.isPushToTalkActive {
            let captureId = self.activePTTCaptureId ?? UUID().uuidString
            return WoodlsTalkPTTStopPayload(
                captureId: captureId,
                transcript: nil,
                status: "busy")
        }
    ...
    func cancelPushToTalk() async -> WoodlsTalkPTTStopPayload {
        let captureId = self.activePTTCaptureId ?? UUID().uuidString
        guard self.isPushToTalkActive else {
            let payload = WoodlsTalkPTTStopPayload(
                captureId: captureId,
    ...
    private func finishPTTOnce(_ payload: WoodlsTalkPTTStopPayload) {
    ...
    private func streamAssistant(runId: String, gateway: GatewayNodeSession) async {
        ...
        for await evt in stream {
            ...
            guard let agentEvent = try? GatewayPayloadDecoding.decode(payload, as: WoodlsAgentEventPayload.self) else {
                continue
            }
