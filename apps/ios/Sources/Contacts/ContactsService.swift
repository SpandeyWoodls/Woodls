...
final class ContactsService: ContactsServicing {
    ...
    func search(params: WoodlsContactsSearchParams) async throws -> WoodlsContactsSearchPayload {
        ...
        return WoodlsContactsSearchPayload(contacts: payload)
    }
    ...
    func add(params: WoodlsContactsAddParams) async throws -> WoodlsContactsAddPayload {
        ...
        if !phoneNumbers.isEmpty || !emails.isEmpty {
            if let existing = try Self.findExistingContact(
                ...
                emails: emails)
            {
                return WoodlsContactsAddPayload(contact: Self.payload(from: existing))
            }
        }
        ...
        return WoodlsContactsAddPayload(contact: Self.payload(from: persisted))
    }
    ...
    private static func payload(from contact: CNContact) -> WoodlsContactPayload {
        WoodlsContactPayload(
