...
struct IOSGatewayChatTransport: WoodlsChatTransport, Sendable {
    private static let logger = Logger(subsystem: "ai.woodls", category: "ios.chat.transport")
    ...
    func listSessions(limit: Int?) async throws -> WoodlsChatSessionsListResponse {
        ...
        return try JSONDecoder().decode(WoodlsChatSessionsListResponse.self, from: res)
    }
    ...
    func requestHistory(sessionKey: String) async throws -> WoodlsChatHistoryPayload {
        ...
        return try JSONDecoder().decode(WoodlsChatHistoryPayload.self, from: res)
    }
        ...
        attachments: [WoodlsChatAttachmentPayload]) async throws -> WoodlsChatSendResponse
    {
        ...
        struct Params: Codable {
            ...
            var message: String
            var thinking: String
            var attachments: [WoodlsChatAttachmentPayload]?
            var timeoutMs: Int
        ...
        do {
            ...
            let decoded = try JSONDecoder().decode(WoodlsChatSendResponse.self, from: res)
    ...
    func requestHealth(timeoutMs: Int) async throws -> Bool {
        ...
        return (try? JSONDecoder().decode(WoodlsGatewayHealthOK.self, from: res))?.ok ?? true
    }
                ...
                for await evt in stream {
                    ...
                    case "health":
                        ...
                        let ok = (try? GatewayPayloadDecoding.decode(
                            payload,
                            as: WoodlsGatewayHealthOK.self))?.ok ?? true
                        continuation.yield(.health(ok: ok))
                    case "chat":
                        guard let payload = evt.payload else { break }
                        if let chatPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: WoodlsChatEventPayload.self)
                        {
                    ...
                    case "agent":
                        ...
                        if let agentPayload = try? GatewayPayloadDecoding.decode(
                            payload,
                            as: WoodlsAgentEventPayload.self)
                        {
