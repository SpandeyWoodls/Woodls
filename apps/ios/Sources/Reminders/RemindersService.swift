...
final class RemindersService: RemindersServicing {
    func list(params: WoodlsRemindersListParams) async throws -> WoodlsRemindersListPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[WoodlsReminderPayload], Error>) in
        ...
        return WoodlsRemindersListPayload(reminders: payload)
    }
    ...
    func add(params: WoodlsRemindersAddParams) async throws -> WoodlsRemindersAddPayload {
        ...
        let formatter = ISO8601DateFormatter()
        let due = reminder.dueDateComponents.flatMap { Calendar.current.date(from: $0) }
        let payload = WoodlsReminderPayload(
            identifier: reminder.calendarItemIdentifier,
            title: reminder.title,
            dueISO: due.map { formatter.string(from: $0) },
            completed: reminder.isCompleted,
        ...
        return WoodlsRemindersAddPayload(reminder: payload)
    }
