...
final class MotionService: MotionServicing {
    func activities(params: WoodlsMotionActivityParams) async throws -> WoodlsMotionActivityPayload {
        ...
        let mapped = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<[WoodlsMotionActivityEntry], Error>) in
        ...
        return WoodlsMotionActivityPayload(activities: mapped)
    }
    ...
    func pedometer(params: WoodlsPedometerParams) async throws -> WoodlsPedometerPayload {
        ...
        let payload = try await withCheckedThrowingContinuation { (cont: CheckedContinuation<WoodlsPedometerPayload, Error>) in
            pedometer.queryPedometerData(from: start, to: end) { data, error in
                if let error {
                    cont.resume(throwing: error)
                } else {
                    let formatter = ISO8601DateFormatter()
                    let payload = WoodlsPedometerPayload(
