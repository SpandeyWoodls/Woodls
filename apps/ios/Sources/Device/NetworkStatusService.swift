...
final class NetworkStatusService: @unchecked Sendable {
    func currentStatus(timeoutMs: Int = 1500) async -> WoodlsNetworkStatusPayload {
    ...
    private static func payload(from path: NWPath) -> WoodlsNetworkStatusPayload {
        let status: WoodlsNetworkPathStatus = switch path.status {
    ...
    private static func fallbackPayload() -> WoodlsNetworkStatusPayload {
        WoodlsNetworkStatusPayload(
            status: .unsatisfied,
            isExpensive: false,
            isConstrained: false,
            interfaces: [.other])
