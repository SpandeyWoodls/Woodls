...
final class GatewayConnectionController {
    ...
    private func currentCaps() -> [String] {
        var caps = [WoodlsCapability.canvas.rawValue, WoodlsCapability.screen.rawValue]
        ...
        if cameraEnabled { caps.append(WoodlsCapability.camera.rawValue) }

        let voiceWakeEnabled = UserDefaults.standard.bool(forKey: VoiceWakePreferences.enabledKey)
        if voiceWakeEnabled { caps.append(WoodlsCapability.voiceWake.rawValue) }

        let locationModeRaw = UserDefaults.standard.string(forKey: "location.enabledMode") ?? "off"
        let locationMode = WoodlsLocationMode(rawValue: locationModeRaw) ?? .off
        if locationMode != .off { caps.append(WoodlsCapability.location.rawValue) }

        caps.append(WoodlsCapability.device.rawValue)
        if WatchMessagingService.isSupportedOnDevice() {
            caps.append(WoodlsCapability.watch.rawValue)
        }
        caps.append(WoodlsCapability.photos.rawValue)
        caps.append(WoodlsCapability.contacts.rawValue)
        caps.append(WoodlsCapability.calendar.rawValue)
        caps.append(WoodlsCapability.reminders.rawValue)
        if Self.motionAvailable() {
            caps.append(WoodlsCapability.motion.rawValue)
        }
    ...
    private func currentCommands() -> [String] {
        var commands: [String] = [
            ...
            WoodlsScreenCommand.record.rawValue,
            WoodlsSystemCommand.notify.rawValue,
            WoodlsChatCommand.push.rawValue,
        ...
        if caps.contains(WoodlsCapability.camera.rawValue) {
            commands.append(WoodlsCameraCommand.list.rawValue)
            commands.append(WoodlsCameraCommand.snap.rawValue)
            commands.append(WoodlsCameraCommand.clip.rawValue)
        }
        if caps.contains(WoodlsCapability.location.rawValue) {
            commands.append(WoodlsLocationCommand.get.rawValue)
        }
        if caps.contains(WoodlsCapability.device.rawValue) {
            commands.append(WoodlsDeviceCommand.status.rawValue)
            commands.append(WoodlsDeviceCommand.info.rawValue)
        }
        if caps.contains(WoodlsCapability.watch.rawValue) {
            commands.append(WoodlsWatchCommand.status.rawValue)
            commands.append(WoodlsWatchCommand.notify.rawValue)
        }
        if caps.contains(WoodlsCapability.photos.rawValue) {
            commands.append(WoodlsPhotosCommand.latest.rawValue)
        }
        if caps.contains(WoodlsCapability.contacts.rawValue) {
            commands.append(WoodlsContactsCommand.search.rawValue)
            commands.append(WoodlsContactsCommand.add.rawValue)
        }
        if caps.contains(WoodlsCapability.calendar.rawValue) {
            commands.append(WoodlsCalendarCommand.events.rawValue)
            commands.append(WoodlsCalendarCommand.add.rawValue)
        }
        if caps.contains(WoodlsCapability.reminders.rawValue) {
            commands.append(WoodlsRemindersCommand.list.rawValue)
            commands.append(WoodlsRemindersCommand.add.rawValue)
        }
        if caps.contains(WoodlsCapability.motion.rawValue) {
            commands.append(WoodlsMotionCommand.activity.rawValue)
            commands.append(WoodlsMotionCommand.pedometer.rawValue)
        }
