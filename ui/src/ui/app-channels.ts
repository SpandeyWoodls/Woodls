...
export async function handleWhatsAppStart(host: WoodlsApp, force: boolean) {
  await startWhatsAppLogin(host, force);
  await loadChannels(host, true);
}

export async function handleWhatsAppWait(host: WoodlsApp) {
  await waitWhatsAppLogin(host);
  await loadChannels(host, true);
}

export async function handleWhatsAppLogout(host: WoodlsApp) {
  await logoutWhatsApp(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigSave(host: WoodlsApp) {
  await saveConfig(host);
  await loadConfig(host);
  await loadChannels(host, true);
}

export async function handleChannelConfigReload(host: WoodlsApp) {
  await loadConfig(host);
  await loadChannels(host, true);
}
...
function resolveNostrAccountId(host: WoodlsApp): string {
...
function resolveGatewayHttpAuthHeader(host: WoodlsApp): string | null {
...
function buildGatewayHttpHeaders(host: WoodlsApp): Record<string, string> {
...
export function handleNostrProfileCancel(host: WoodlsApp) {
...
export function handleNostrProfileToggleAdvanced(host: WoodlsApp) {
  const state = host.nostrProfileFormState;
  if (!state) {
    return;
  }
...
export async function handleNostrProfileSave(host: WoodlsApp) {
  const state = host.nostrProfileFormState;
...
export async function handleNostrProfileImport(host: WoodlsApp) {
  const state = host.nostrProfileFormState;
