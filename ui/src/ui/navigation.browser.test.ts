...
describe("control UI routing", () => {
  ...
  it("infers nested base paths", async () => {
    const app = mountApp("/apps/woodls/cron");
    await app.updateComplete;

    expect(app.basePath).toBe("/apps/woodls");
    expect(app.tab).toBe("cron");
    expect(window.location.pathname).toBe("/apps/woodls/cron");
  });

  it("honors explicit base path overrides", async () => {
    window.__WOODLS_CONTROL_UI_BASE_PATH__ = "/woodls";
    const app = mountApp("/woodls/sessions");
    await app.updateComplete;

    expect(app.basePath).toBe("/woodls");
    expect(app.tab).toBe("sessions");
    expect(window.location.pathname).toBe("/woodls/sessions");
  });
  ...
  it("hydrates token from URL params even when settings already set", async () => {
    localStorage.setItem("woodls.control.settings.v1", JSON.stringify({ token: "existing-token" }));
    const app = mountApp("/ui/overview?token=abc123");
    await app.updateComplete;
