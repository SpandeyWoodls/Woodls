...
- **Ask default:** `on-miss`.
- **Approvals store:** `~/.woodls/exec-approvals.json` (JSON, no legacy migration).
- **Runner:** headless system service; UI app hosts a Unix socket for approvals.
- **Node identity:** use existing `nodeId`.
- **Socket auth:** Unix socket + token (cross-platform); split later if needed.
- **Node host state:** `~/.woodls/node.json` (node id + pairing token).
...
Path: `~/.woodls/exec-approvals.json`
...
```json
{
  "version": 1,
  "socket": {
    "path": "~/.woodls/exec-approvals.sock",
...
- Unix socket at `~/.woodls/exec-approvals.sock` (0600).
