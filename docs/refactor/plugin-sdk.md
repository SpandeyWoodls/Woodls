...
- Publish as `woodls/plugin-sdk` (or export from core under `woodls/plugin-sdk`).
- Semver with explicit stability guarantees.

### 2) Plugin Runtime (execution surface, injected)

Scope: everything that touches core runtime behavior.
Accessed via `WoodlsPluginApi.runtime` so plugins never import `src/**`.
...
export type PluginRuntime = {
  channel: {
    text: {
      ...
      resolveTextChunkLimit(cfg: WoodlsConfig, channel: string, accountId?: string): number;
      hasControlCommand(text: string, cfg: WoodlsConfig): boolean;
    };
    reply: {
    ...
    mentions: {
      buildMentionRegexes(cfg: WoodlsConfig, agentId?: string): RegExp[];
    ...
    debounce: {
      ...
      resolveInboundDebounceMs(cfg: WoodlsConfig, channel: string): number;
    };
    commands: {
  ...
  state: {
    resolveStateDir(cfg: WoodlsConfig): string;
  };
};
```
...
- Introduce `woodls/plugin-sdk`.
- Add `api.runtime` to `WoodlsPluginApi` with the surface above.
...
- Plugins declare a required runtime range (e.g., `woodlsRuntime: ">=2026.2.0"`).
