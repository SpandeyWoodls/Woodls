---
summary: "CLI reference for `woodls security` (audit and fix common security footguns)"
read_when:
...
```bash
woodls security audit --json | jq '.summary'
woodls security audit --deep --json | jq '.findings[] | select(.severity=="critical") | .checkId'
```

If `--fix` and `--json` are combined, output includes both fix actions and final report:

```bash
woodls security audit --fix --json | jq '{fix: .fix.ok, summary: .report.summary}'
```
