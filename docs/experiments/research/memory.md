---
summary: "Research notes: offline memory system for Woodls workspaces (Markdown source-of-truth + derived index)"
read_when:
  - Designing workspace memory (~/.woodls/workspace) beyond daily Markdown logs
  - Deciding: standalone CLI vs deep Woodls integration
...
Target: Woodls-style workspace (`agents.defaults.workspace`, default `~/.woodls/workspace`) where “memory” is stored as one Markdown file per day (`memory/YYYY-MM-DD.md`) plus a small set of stable files (e.g. `memory.md`, `SOUL.md`).
...
Keep `~/.woodls/workspace` as canonical human-readable memory.
...
- `memory.md` remains “small + core-ish”: the things you want Woodls to see every session.
...
```
~/.woodls/workspace/.memory/index.sqlite
```
...
Recommendation: **deep integration in Woodls**, but keep a separable core library.

### Why integrate into Woodls?

- Woodls already knows:
  - the workspace path (`agents.defaults.workspace`)
  - the session model + heartbeats
  - logging + troubleshooting patterns
- You want the agent itself to call the tools:
  - `woodls memory recall "…" --k 25 --since 30d`
  - `woodls memory reflect --since 7d`
...
Pragmatic take for `~/.woodls/workspace`:
