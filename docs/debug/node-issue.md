...
read_when:
  ...
  - Investigating tsx/esbuild loader crashes in Woodls
...
Running Woodls via Node with `tsx` fails at startup with:

```
[woodls] Failed to start CLI: TypeError: __name is not a function
...
- Before that (Bun path), `woodls status` and `gateway:watch` worked.
...
- Use Node + tsc watch, then run compiled output:
  ...
  node --watch woodls.mjs status
  ```

- Confirmed locally: `pnpm exec tsc -p tsconfig.json` + `node woodls.mjs status` works on Node 25.
