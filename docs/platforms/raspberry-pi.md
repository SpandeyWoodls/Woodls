---
summary: "Woodls on Raspberry Pi (budget self-hosted setup)"
read_when:
  - Setting up Woodls on a Raspberry Pi
  - Running Woodls on ARM devices
  - Building a cheap always-on personal AI
title: "Raspberry Pi"
---

# Woodls on Raspberry Pi

## Goal

Run a persistent, always-on Woodls Gateway on a Raspberry Pi for **~$35-80** one-time cost (no monthly fees).
...
Most Woodls features work on ARM64, but some external binaries may need ARM builds:
...
sudo systemctl is-enabled woodls

# Enable if not
sudo systemctl enable woodls

# Start on boot
sudo systemctl start woodls
```
...
```bash
# Check logs
journalctl -u woodls --no-pager -n 100

# Common fix: rebuild
cd ~/woodls  # if using hackable install
npm run build
sudo systemctl restart woodls
```
