...
- `/usage cost` in chats: local cost summary aggregated from Woodls session logs.
- CLI: `woodls status --usage` prints a full per-provider breakdown.
- CLI: `woodls channels list` prints the same usage snapshot alongside provider config (use `--no-usage` to skip).
