---
summary: "Use MiniMax M2.1 in Woodls"
read_when:
  - You want MiniMax models in Woodls
...
```bash
woodls plugins enable minimax-portal-auth  # skip if already loaded.
woodls gateway restart  # restart if gateway is already running
woodls onboard --auth-choice minimax-portal
```
...
See [MiniMax OAuth plugin README](https://github.com/woodls/woodls/tree/main/extensions/minimax-portal-auth) for details.
...
Configure manually via `woodls.json`:
...
- Use `woodls models list` and `woodls models set minimax/MiniMax-M2.1` to switch.
...
- Running `woodls configure` and selecting **MiniMax M2.1**, or
