---
summary: "Use Venice AI privacy-focused models in Woodls"
read_when:
  - You want privacy-focused inference in Woodls
...
```bash
woodls onboard --auth-choice venice-api-key
```
...
```bash
woodls chat --model venice/llama-3.3-70b "Hello, are you working?"
```

## Model Selection

After setup, Woodls shows all available Venice models. Pick based on your needs:
...
```bash
woodls models set venice/claude-opus-45
woodls models set venice/llama-3.3-70b
```

List all available models:

```bash
woodls models list | grep venice
```

## Configure via `woodls configure`

1. Run `woodls configure`
2. Select **Model/auth**
3. Choose **Venice AI**
...
Woodls automatically discovers models from the Venice API when `VENICE_API_KEY` is set. If the API is unreachable, it falls back to a static catalog.
...
woodls chat --model venice/llama-3.3-70b

# Use Claude via Venice (anonymized)
woodls chat --model venice/claude-opus-45

# Use uncensored model
woodls chat --model venice/venice-uncensored

# Use vision model with image
woodls chat --model venice/qwen3-vl-235b-a22b

# Use coding model
woodls chat --model venice/qwen3-coder-480b-a35b-instruct
```
...
The Venice model catalog updates dynamically. Run `woodls models list` to see currently available models. Some models may be temporarily offline.
