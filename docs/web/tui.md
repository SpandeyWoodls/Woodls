...
```bash
woodls tui --url ws://<host>:<port> --token <gateway-token>
```
...
- Turn delivery on:
  ...
  - or start with `woodls tui --deliver`
...
- Check the Gateway logs: `woodls logs --follow`.
- Confirm the agent can run: `woodls status` and `woodls models status`.
...
- No agents in picker: check `woodls agents list` and your routing config.
