...
- optional prefix: set `gateway.controlUi.basePath` (e.g. `/woodls`)
...
If the page fails to load, start the Gateway first: `woodls gateway`.
...
you revoke it with `woodls devices revoke --device <id> --role <role>`. See
...
- Config: view/edit `~/.woodls/woodls.json` (`config.get`, `config.set`)
...
(`tailscale-user-login`) when `gateway.auth.allowTailscale` is `true`. Woodls
...
Woodls **blocks** Control UI connections without device identity.
