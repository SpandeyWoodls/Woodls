...
- **gateway host** → `woodls` process on the gateway machine
...
`~/.woodls/exec-approvals.json` directly.

CLI: `woodls approvals` supports gateway or node editing (see [Approvals CLI](/cli/approvals)).
