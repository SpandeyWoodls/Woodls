---
summary: "Chrome extension: let Woodls drive your existing Chrome tab"
read_when:
...
The Woodls Chrome extension lets the agent control your **existing Chrome tabs** (your normal Chrome window) instead of launching a separate woodls-managed Chrome profile.
...
Woodls then controls the attached tab through the normal `browser` tool surface (selecting the right profile).
...
The extension ships inside the Woodls release (npm package) as static files. There is no separate “build” step.

After upgrading Woodls:

- Re-run `woodls browser extension install` to refresh the installed files under your Woodls state directory.
- Chrome → `chrome://extensions` → click “Reload” on the extension.

## Use it (no extra config)

Woodls ships with a built-in browser profile named `chrome` that targets the extension relay on the default port.
...
- Open the tab you want Woodls to control.
...
- `ON`: attached; Woodls can drive that tab.
...
Debugging: `woodls sandbox explain`
...
`woodls browser extension path` prints the **installed** on-disk directory containing the extension files.

The CLI intentionally does **not** print a `node_modules` path. Always run `woodls browser extension install` first to copy the extension to a stable location under your Woodls state directory.
...
- **This is not isolated** like the dedicated woodls-managed profile.
