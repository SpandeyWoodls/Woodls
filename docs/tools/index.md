---
summary: "Agent tool surface for Woodls (browser, canvas, nodes, message, cron) replacing legacy `woodls-*` skills"
read_when:
  - Adding or modifying agent tools
  - Retiring or changing `woodls-*` skills
title: "Tools"
---

# Tools (Woodls)

Woodls exposes **first-class agent tools** for browser, canvas, nodes, and cron.
These replace the old `woodls-*` skills: the tools are typed, no shelling,
and the agent should rely on them directly.

## Disabling tools

You can globally allow/deny tools via `tools.allow` / `tools.deny` in `woodls.json`
...
- If `tools.allow` only references unknown or unloaded plugin tool names, Woodls logs a warning and ignores the allowlist so core tools stay available.
...
- `group:nodes`: `nodes`
- `group:woodls`: all built-in Woodls tools (excludes provider plugins)
...
- `host=node` can target a macOS companion app or a headless node host (`woodls node run`).
...
Woodls tracks recent tool-call history and blocks or warns when it detects repetitive no-progress loops.
...
- Requires a Brave API key (recommended: `woodls configure --section web`, or set `BRAVE_API_KEY`).
...
Control the dedicated Woodls-managed browser.
...
- Quick smoke: `woodls nodes canvas a2ui push --node <id> --text "Hello from A2UI"`.
...
- `restart` (authorizes + sends `SIGUSR1` for in-process restart; `woodls gateway` restart in-place)
...
- Sandbox clamp: when the current session is sandboxed and `agents.defaults.sandbox.sessionToolsVisibility: "spawned"`, Woodls clamps `tools.sessions.visibility` to `tree`.
