...
For debugging “why is this blocked?”, see [Sandbox vs Tool Policy vs Elevated](/gateway/sandbox-vs-tool-policy-vs-elevated) and `woodls sandbox explain`.
...
Legacy `agent.*` configs are migrated by `woodls doctor`; prefer `agents.defaults` + `agents.list` going forward.
...
2. **Verify sandbox containers:**
   ...
   ```exec
   docker ps --filter "name=woodls-sbx-"
   ```
...
4. **Monitor logs:**
   ...
   ```exec
   tail -f "${WOODLS_STATE_DIR:-$HOME/.woodls}/logs/gateway.log" | grep -E "routing|sandbox|tools"
   ```
