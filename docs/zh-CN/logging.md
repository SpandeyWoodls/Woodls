...
你可以在 `~/.woodls/woodls.json` 中覆盖此设置：

```json
{
  "logging": {
    "file": "/path/to/woodls.log"
  }
}
```
...
```bash
woodls channels logs --channel whatsapp
```
...
所有日志配置都在 `~/.woodls/woodls.json` 的 `logging` 下。

```json
{
  "logging": {
    "level": "info",
    "file": "/tmp/woodls/woodls-YYYY-MM-DD.log",
    "consoleLevel": "info",
    "consoleStyle": "pretty",
...
- Woodls 目前通过 **OTLP/HTTP（protobuf）** 导出。
...
```
WOODLS_DIAGNOSTICS=telegram.http,telegram.payload
```
...
- 你也可以使用 `woodls plugins enable diagnostics-otel` 启用插件。
...
- `woodls.tokens`（计数器，属性：`woodls.token`、`woodls.channel`、`woodls.provider`、`woodls.model`）
- `woodls.cost.usd`（计数器，属性：`woodls.channel`、`woodls.provider`、`woodls.model`）
- `woodls.run.duration_ms`（直方图，属性：`woodls.channel`、`woodls.provider`、`woodls.model`）
- `woodls.context.tokens`（直方图，属性：`woodls.context`、`woodls.channel`、`woodls.provider`、`woodls.model`）

消息流：

- `woodls.webhook.received`（计数器，属性：`woodls.channel`、`woodls.webhook`）
- `woodls.webhook.error`（计数器，属性：`woodls.channel`、`woodls.webhook`）
- `woodls.webhook.duration_ms`（直方图，属性：`woodls.channel`、`woodls.webhook`）
- `woodls.message.queued`（计数器，属性：`woodls.channel`、`woodls.source`）
- `woodls.message.processed`（计数器，属性：`woodls.channel`、`woodls.outcome`）
- `woodls.message.duration_ms`（直方图，属性：`woodls.channel`、`woodls.outcome`）

队列 + 会话：

- `woodls.queue.lane.enqueue`（计数器，属性：`woodls.lane`）
- `woodls.queue.lane.dequeue`（计数器，属性：`woodls.lane`）
- `woodls.queue.depth`（直方图，属性：`woodls.lane` 或 `woodls.channel=heartbeat`）
- `woodls.queue.wait_ms`（直方图，属性：`woodls.lane`）
- `woodls.session.state`（计数器，属性：`woodls.state`、`woodls.reason`）
- `woodls.session.stuck`（计数器，属性：`woodls.state`）
- `woodls.session.stuck_age_ms`（直方图，属性：`woodls.state`）
- `woodls.run.attempt`（计数器，属性：`woodls.attempt`）

### 导出的 span（名称 + 关键属性）

- `woodls.model.usage`
  - `woodls.channel`、`woodls.provider`、`woodls.model`
  - `woodls.sessionKey`、`woodls.sessionId`
  - `woodls.tokens.*`（input/output/cache_read/cache_write/total）
- `woodls.webhook.processed`
  - `woodls.channel`、`woodls.webhook`、`woodls.chatId`
- `woodls.webhook.error`
  - `woodls.channel`、`woodls.webhook`、`woodls.chatId`、`woodls.error`
- `woodls.message.processed`
  - `woodls.channel`、`woodls.outcome`、`woodls.chatId`、`woodls.messageId`、`woodls.sessionKey`、`woodls.sessionId`、`woodls.reason`
- `woodls.session.stuck`
  - `woodls.state`、`woodls.ageMs`、`woodls.queueDepth`、`woodls.sessionKey`、`woodls.sessionId`
...
- **Gateway 网关无法访问？** 先运行 `woodls doctor`。
