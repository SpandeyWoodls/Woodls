...
- During heartbeat runs, Woodls treats `HEARTBEAT_OK` as an ack when it appears
...
- `session`: optional session key for heartbeat runs.
  ...
  - Explicit session key (copy from `woodls sessions --json` or the [sessions CLI](/cli/sessions)).
...
If **all three** are false, Woodls skips the heartbeat run entirely (no model call).
...
```bash
woodls system event --text "Check for urgent follow-ups" --mode now
```
