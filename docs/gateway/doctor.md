...
`woodls doctor` is the repair + migration tool for Woodls. It fixes stale
...
```bash
woodls doctor --repair
```

Apply recommended repairs without prompting (repairs + restarts where safe).

```bash
woodls doctor --repair --force
```
...
- Extra workspace dir detection (`~/woodls`).
...
you to run `woodls doctor`.

Doctor will:

- Explain which legacy keys were found.
- Show the migration it applied.
- Rewrite `~/.woodls/woodls.json` with the updated schema.
...
- Sessions store + transcripts:
  - from `~/.woodls/sessions/` to `~/.woodls/agents/<agentId>/sessions/`
- Agent dir:
  - from `~/.woodls/agent/` to `~/.woodls/agents/<agentId>/agent/`
- WhatsApp auth state (Baileys):
  - from legacy `~/.woodls/credentials/*.json` (except `oauth.json`)
  - to `~/.woodls/credentials/whatsapp/<accountId>/...` (default account id: `default`)

These migrations are best-effort and idempotent; doctor will emit warnings when
it leaves any legacy folders behind as backups. The Gateway/CLI also auto-migrates
the legacy sessions + agent dir on startup so history/auth/models land in the
per-agent path without a manual doctor run. WhatsApp auth is intentionally only
migrated via `woodls doctor`.
...
- **Multiple state dirs**: warns when multiple `~/.woodls` folders exist across
  home directories or when `WOODLS_STATE_DIR` points elsewhere (history can
  split between installs).
- **Remote mode reminder**: if `gateway.mode=remote`, doctor reminds you to run
  it on the remote host (the state lives there).
- **Config file permissions**: warns if `~/.woodls/woodls.json` is
...
offers to remove them and install the Woodls service using the current gateway
port. It can also scan for extra gateway-like services and print cleanup hints.
Profile-named Woodls gateway services are considered first-class and are not
flagged as "extra."
...
generate a token. Use `woodls doctor --generate-gateway-token` to force token
...
- `woodls doctor` prompts before rewriting supervisor config.
- `woodls doctor --yes` accepts the default repair prompts.
- `woodls doctor --repair` applies recommended fixes without prompts.
- `woodls doctor --repair --force` overwrites custom supervisor configs.
- You can always force a full rewrite via `woodls gateway install --force`.
