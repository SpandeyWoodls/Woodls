...
description: "Complete field-by-field reference for ~/.woodls/woodls.json"
---

# Configuration Reference

Every field available in `~/.woodls/woodls.json`. For a task-oriented overview, see [Configuration](/gateway/configuration).

Config format is **JSON5** (comments + trailing commas allowed). All fields are optional — Woodls uses safe defaults when omitted.
...
- Legacy single-account Baileys auth dir is migrated by `woodls doctor` into `whatsapp/default`.
...
```json5
{
  channels: {
    discord: {
      ...
      dm: { enabled: true, groupEnabled: false, groupChannels: ["woodls-dm"] },
      guilds: {
        "123456789012345678": {
...
Mattermost ships as a plugin: `woodls plugins install @woodls/mattermost`.
...
Woodls spawns `imsg rpc` (JSON-RPC over stdio). No daemon or port required.
...
```json5
{
  ...
  agents: {
    list: [
      {
        id: "main",
        groupChat: { mentionPatterns: ["reisponde", "@woodls"] },
      },
    ],
  },
}
...
- `config: true` enables `/config` (reads/writes `woodls.json`).
...
Optional repository root shown in the system prompt's Runtime line. If unset, Woodls auto-detects by walking upward from the workspace.
...
- `model.primary`: format `provider/model` (e.g. `anthropic/claude-opus-4-6`). If you omit the provider, Woodls assumes `anthropic` (deprecated).
...
- `none`: per-scope sandbox workspace under `~/.woodls/sandboxes`
    ...
    list: [
      {
        ...
        agentDir: "~/.woodls/agents/main/agent",
        ...
        groupChat: { mentionPatterns: ["@woodls"] },
        sandbox: { mode: "off" },
...
| `group:nodes`      | `nodes`                                                                                  |
| `group:woodls`     | All built-in tools (excludes provider plugins)                                           |
...
Woodls uses the pi-coding-agent model catalog. Add custom providers via `models.providers` in config or `~/.woodls/agents/<agentId>/agent/models.json`.
...
- Override agent config root with `WOODLS_AGENT_DIR` (or `PI_CODING_AGENT_DIR`).
...
Set `OPENCODE_API_KEY` (or `OPENCODE_ZEN_API_KEY`). Shortcut: `woodls onboard --auth-choice opencode-zen`.
...
Set `ZAI_API_KEY`. `z.ai/*` and `z-ai/*` are accepted aliases. Shortcut: `woodls onboard --auth-choice zai-api-key`.
...
For the China endpoint: `baseUrl: "https://api.moonshot.cn/v1"` or `woodls onboard --auth-choice moonshot-api-key-cn`.
...
Anthropic-compatible, built-in provider. Shortcut: `woodls onboard --auth-choice kimi-code-api-key`.
...
Base URL should omit `/v1` (Anthropic client appends it). Shortcut: `woodls onboard --auth-choice synthetic-api-key`.
...
Set `MINIMAX_API_KEY`. Shortcut: `woodls onboard --auth-choice minimax-api`.
...
- Loaded from `~/.woodls/extensions`, `<workspace>/.woodls/extensions`, plus `plugins.load.paths`.
...
```json5
{
  gateway: {
    ...
    auth: {
      ...
      token: "your-token",
      // password: "your-password", // or WOODLS_GATEWAY_PASSWORD
...
- `port`: single multiplexed port for WS + HTTP. Precedence: `--port` > `WOODLS_GATEWAY_PORT` > `gateway.port` > `18789`.
...
Convenience flags: `--dev` (uses `~/.woodls-dev` + port `19001`), `--profile <name>` (uses `~/.woodls-<name>`).
...
```json5
{
  hooks: {
    ...
    presets: ["gmail"],
    transformsDir: "~/.woodls/hooks/transforms",
    mappings: [
      {
        match: { path: "gmail" },
        action: "agent",
...
Auth: `Authorization: Bearer <token>` or `x-woodls-token: <token>`.
...
- Gateway auto-starts `gog gmail watch serve` on boot when configured. Set `WOODLS_SKIP_GMAIL_WATCHER=1` to disable.
...
```json5
{
  canvasHost: {
    ...
    liveReload: true,
    // enabled: false, // or WOODLS_SKIP_CANVAS_HOST=1
  },
}
```

- Serves agent-editable HTML/CSS/JS and A2UI over HTTP under the Gateway port:
  - `http://<gateway-host>:<gateway.port>/__woodls__/canvas/`
  - `http://<gateway-host>:<gateway.port>/__woodls__/a2ui/`
...
- Also serves A2UI at `/__woodls__/a2ui/`.
...
- Hostname defaults to `woodls`. Override with `WOODLS_MDNS_HOSTNAME`.
...
Writes a unicast DNS-SD zone under `~/.woodls/dns/`. For cross-network discovery, pair with a DNS server (CoreDNS recommended) + Tailscale split DNS.

Setup: `woodls dns setup --apply`.
...
- `.env` files: CWD `.env` + `~/.woodls/.env` (neither overrides existing vars).
...
{
  gateway: {
    auth: { token: "${WOODLS_GATEWAY_TOKEN}" },
  },
}
```
...
- Legacy OAuth imports from `~/.woodls/credentials/oauth.json`.
...
- Default log file: `/tmp/woodls/woodls-YYYY-MM-DD.log`.
...
Current builds no longer include the TCP bridge. Nodes connect over the Gateway WebSocket. `bridge.*` keys are no longer part of the config schema (validation fails until removed; `woodls doctor --fix` can strip unknown keys).
