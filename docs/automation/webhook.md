...
read_when:
  ...
  - Wiring external systems into Woodls
title: "Webhooks"
---
...
- `hooks.transformsDir` + `transform.module` loads a JS/TS module for custom logic.
  - `hooks.transformsDir` (if set) must stay within the transforms root under your Woodls config directory (typically `~/.woodls/hooks/transforms`).
...
- `woodls webhooks gmail setup` writes `hooks.gmail` config for `woodls webhooks gmail run`.
