...
woodls system heartbeat last
woodls logs --follow
woodls config get agents.defaults.heartbeat
woodls channels status --probe
```
...
```bash
woodls config get agents.defaults.heartbeat.activeHours
woodls config get agents.defaults.heartbeat.activeHours.timezone
woodls config get agents.defaults.userTimezone || echo "agents.defaults.userTimezone not set"
woodls cron list
woodls logs --follow
```
