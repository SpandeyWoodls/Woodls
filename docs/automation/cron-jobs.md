...
- Jobs persist under `~/.woodls/cron/` so restarts don’t lose schedules.
...
woodls cron list
woodls cron run <job-id>
woodls cron runs --id <job-id>
```
...
Cron jobs are persisted on the Gateway host at `~/.woodls/cron/jobs.json` by default.
The Gateway loads the file into memory and writes it back on changes, so manual edits
are only safe when the Gateway is stopped. Prefer `woodls cron add/edit` or the cron
...
To reduce top-of-hour load spikes across many gateways, Woodls applies a
...
If `delivery` is omitted for isolated jobs, Woodls defaults to `announce`.
...
- Job store: `~/.woodls/cron/jobs.json` (Gateway-managed JSON).
- Run history: `~/.woodls/cron/runs/<jobId>.jsonl` (JSONL, auto-pruned).
...
woodls cron add --name "Ops sweep" --cron "0 6 * * *" --session isolated --message "Check ops queue" --agent ops

# Switch or clear the agent on an existing job
woodls cron edit <jobId> --agent ops
woodls cron edit <jobId> --clear-agent
```

Manual run (force is the default, use `--due` to only run when due):

```bash
woodls cron run <jobId>
woodls cron run <jobId> --due
```

Edit an existing job (patch fields):

```bash
woodls cron edit <jobId> \
...
```bash
woodls system event --mode now --text "Next heartbeat: check battery."
```
...
- `cron.run` (force or due), `cron.runs`
  For immediate system events without a job, use [`woodls system event`](/cli/system).

## Troubleshooting

### “Nothing runs”

- Check cron is enabled: `cron.enabled` and `WOODLS_SKIP_CRON`.
- Check the Gateway is running continuously (cron runs inside the Gateway process).
- For `cron` schedules: confirm timezone (`--tz`) vs the host timezone.

### A recurring job keeps delaying after failures

- Woodls applies exponential retry backoff for recurring jobs after consecutive errors:
