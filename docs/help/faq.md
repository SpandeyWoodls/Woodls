...
Yes. Add extra directories via `skills.load.extraDirs` in `~/.woodls/woodls.json` (lowest precedence). Default precedence remains: `<workspace>/skills` → `~/.woodls/skills` → bundled → `skills.load.extraDirs`. `clawhub` installs into `./skills` by default, which Woodls treats as `<workspace>/skills`.
...
ClawHub installs into `./skills` under your current directory (or falls back to your configured Woodls workspace); Woodls treats that as `<workspace>/skills` on the next session. For shared skills across agents, place them in `~/.woodls/skills/<name>/SKILL.md`. Some skills expect binaries installed via Homebrew; on Linux that means Linuxbrew (see the Homebrew Linux FAQ entry above). See [Skills](/tools/skills) and [ClawHub](/tools/clawhub).
