...
Deploy Woodls on Railway with a one-click template and finish setup in your browser.
...
<a href="https://railway.com/deploy/woodls-railway-template" target="_blank" rel="noreferrer">
  Deploy on Railway
</a>
...
- `https://<your-railway-domain>/woodls` — Control UI

## What you get

- Hosted Woodls Gateway + Control UI
...
- `WOODLS_STATE_DIR=/data/.woodls` (recommended)
- `WOODLS_WORKSPACE_DIR=/data/workspace` (recommended)
- `WOODLS_GATEWAY_TOKEN` (recommended; treat as an admin secret)
...
This exports your Woodls state + workspace so you can migrate to another host without losing config or memory.
