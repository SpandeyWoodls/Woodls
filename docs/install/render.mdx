...
Deploy Woodls on Render using Infrastructure as Code. The included `render.yaml` Blueprint defines your entire stack declaratively, service, disk, environment variables, so you can deploy with a single click and version your infrastructure alongside your code.
...
[Deploy to Render](https://render.com/deploy?repo=https://github.com/woodls/woodls)
...
repository configures everything needed to run Woodls:
...
services:
  - type: web
    ...
    envVars:
      ...
      - key: WOODLS_STATE_DIR
        value: /data/.woodls
      - key: WOODLS_WORKSPACE_DIR
        value: /data/workspace
      - key: WOODLS_GATEWAY_TOKEN
...
The web dashboard is available at `https://<your-service>.onrender.com/woodls`.
...
If you use the original Woodls repository, Render will not auto-deploy your Woodls. To update it, run a manual Blueprint sync from the dashboard.
...
For Woodls, vertical scaling is usually sufficient. Horizontal scaling requires sticky sessions or external state management.
...
This downloads a portable backup you can restore on any Woodls host.
