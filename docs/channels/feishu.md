...
Feishu (Lark) is a team chat platform used by companies for messaging and collaboration. This plugin connects Woodls to a Feishu/Lark bot using the platform’s WebSocket event subscription so messages can be received without exposing a public webhook URL.
...
```bash
woodls plugins install @woodls/feishu
```

Local checkout (when running from a git repo):

```bash
woodls plugins install ./extensions/feishu
```
...
If you just installed Woodls, run the wizard:
...
2. Configuring app credentials in Woodls
...
1. You already ran `woodls channels add` for Feishu
2. The gateway is running (`woodls gateway status`)
...
```bash
woodls pairing approve feishu <CODE>
```
...
2. Run `woodls logs --follow` and look for `chat_id`
...
2. Run `woodls logs --follow` and look for `open_id`
...
| Command                  | Description                   |
| ------------------------ | ----------------------------- |
| `woodls gateway status`  | Show gateway status           |
| `woodls gateway install` | Install/start gateway service |
| `woodls gateway stop`    | Stop gateway service          |
| `woodls gateway restart` | Restart gateway service       |
| `woodls logs --follow`   | Tail gateway logs             |
...
4. Check logs: `woodls logs --follow`
...
5. Ensure the gateway is running: `woodls gateway status`
6. Check logs: `woodls logs --follow`
...
```json5
{
  agents: {
    list: [
      ...
      {
        id: "woodls-fan",
        workspace: "/home/user/woodls-fan",
        agentDir: "/home/user/.woodls/agents/woodls-fan/agent",
      },
      {
        id: "woodls-xi",
        workspace: "/home/user/woodls-xi",
        agentDir: "/home/user/.woodls/agents/woodls-xi/agent",
      },
    ],
  },
  bindings: [
