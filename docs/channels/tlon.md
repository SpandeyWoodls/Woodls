...
Tlon is a decentralized messenger built on Urbit. Woodls connects to your Urbit ship and can
...
```bash
woodls plugins install @woodls/tlon
```

Local checkout (when running from a git repo):

```bash
woodls plugins install ./extensions/tlon
```
...
By default, Woodls blocks private/internal hostnames and IP ranges for this plugin (SSRF hardening).
...
Use these with `woodls message send` or cron delivery:
...
- Thread replies: if the inbound message is in a thread, Woodls replies in-thread.
