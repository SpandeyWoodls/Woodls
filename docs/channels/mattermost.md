---
summary: "Mattermost bot setup and Woodls config"
read_when:
...
```bash
woodls plugins install @woodls/mattermost
```

Local checkout (when running from a git repo):

```bash
woodls plugins install ./extensions/mattermost
```
...
4. Configure Woodls and start the gateway.
...
- Approve via:
  - `woodls pairing list mattermost`
  - `woodls pairing approve mattermost <CODE>`
...
Use these target formats with `woodls message send` or cron/webhooks:
