...
read_when:
  - You want Woodls to receive DMs via Nostr
...
Nostr is a decentralized protocol for social networking. This channel enables Woodls to receive and respond to encrypted direct messages (DMs) via NIP-04.

## Install (on demand)

### Onboarding (recommended)

- The onboarding wizard (`woodls onboard`) and `woodls channels add` list optional channel plugins.
...
```bash
woodls plugins install @woodls/nostr
```

Use a local checkout (dev workflows):

```bash
woodls plugins install --link <path-to-woodls>/extensions/nostr
```
  ...
  "channels": {
    "nostr": {
      ...
      "profile": {
        ...
        "nip05": "woodls@example.com",
        "lud16": "woodls@example.com"
      }
    }
  }
}
