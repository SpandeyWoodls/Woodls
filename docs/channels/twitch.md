...
read_when:
  - Setting up Twitch chat integration for Woodls
title: "Twitch"
---

# Twitch (plugin)

Twitch chat support via IRC connection. Woodls connects as a Twitch user (bot account) to receive and send messages in channels.
...
```bash
woodls plugins install @woodls/twitch
```

Local checkout (when running from a git repo):

```bash
woodls plugins install ./extensions/twitch
```
...
4. Configure the token:
   - Env: `WOODLS_TWITCH_ACCESS_TOKEN=...` (default account only)
...
```json5
{
  channels: {
    twitch: {
      enabled: true,
      username: "woodls", // Bot's Twitch account
      accessToken: "oauth:abc123...", // OAuth Access Token (or use WOODLS_TWITCH_ACCESS_TOKEN env var)
...
```bash
WOODLS_TWITCH_ACCESS_TOKEN=oauth:abc123...
```
