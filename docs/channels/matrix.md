...
Matrix is an open, decentralized messaging protocol. Woodls connects as a Matrix **user**
...
```bash
woodls plugins install @woodls/matrix
```

Local checkout (when running from a git repo):

```bash
woodls plugins install ./extensions/matrix
```
...
1. Install the Matrix plugin:
   - From npm: `woodls plugins install @woodls/matrix`
   - From a local checkout: `woodls plugins install ./extensions/matrix`
...
3. Get an access token for the bot account:
   ...
   - Or set `channels.matrix.userId` + `channels.matrix.password`: Woodls calls the same
     login endpoint, stores the access token in `~/.woodls/credentials/matrix/credentials.json`,
...
- On first connection, Woodls requests device verification from your other sessions.
- Verify the device in another Matrix client (Element, etc.) to enable key sharing.
- If the crypto module cannot be loaded, E2EE is disabled and encrypted rooms will not decrypt;
  Woodls logs a warning.
...
`~/.woodls/matrix/accounts/<account>/<homeserver>__<user>/<token-hash>/crypto/`
...
- Approve via:
  - `woodls pairing list matrix`
  - `woodls pairing approve matrix <CODE>`
...
- On startup, Woodls resolves room/user names in allowlists to IDs and logs the mapping; unresolved entries are ignored for allowlist matching.
