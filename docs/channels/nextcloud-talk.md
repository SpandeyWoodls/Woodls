...
```bash
woodls plugins install @woodls/nextcloud-talk
```

Local checkout (when running from a git repo):

```bash
woodls plugins install ./extensions/nextcloud-talk
```
...
2. On your Nextcloud server, create a bot:
   ...
   ```bash
   ./occ talk:bot:install "Woodls" "<shared-secret>" "<webhook-url>" --feature reaction
   ```
...
- Approve via:
  - `woodls pairing list nextcloud-talk`
  - `woodls pairing approve nextcloud-talk <CODE>`
