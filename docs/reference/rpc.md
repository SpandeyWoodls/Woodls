...
Woodls integrates external CLIs via JSON-RPC. Two patterns are used today.
...
- Woodls owns lifecycle when `channels.signal.autoStart=true`.
...
- Woodls spawns `imsg rpc` as a child process (legacy iMessage integration).
