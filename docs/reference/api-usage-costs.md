...
Woodls features that can generate provider usage or paid API calls.
...
- `woodls status --usage` and `woodls channels list` show provider **usage windows**
...
Woodls can pick up credentials from:
...
`woodls models scan` can probe OpenRouter models and uses `OPENROUTER_API_KEY` when
probing is enabled.
