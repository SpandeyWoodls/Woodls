---
summary: "How Woodls builds prompt context and reports token usage + costs"
read_when:
...
Woodls tracks **tokens**, not characters. Tokens are model-specific, but most
OpenAI-style models average ~4 characters per token for English text.

## How the system prompt is built

Woodls assembles its own system prompt on every run. It includes:
...
For images, Woodls downscales transcript/tool image payloads before provider calls.
...
- `/usage cost` → shows a local cost summary from Woodls session logs.

Other surfaces:

- **TUI/Web TUI:** `/status` + `/usage` are supported.
- **CLI:** `woodls status --usage` and `woodls channels list` show
...
`cacheWrite`. If pricing is missing, Woodls shows tokens only. OAuth tokens
never show dollar cost.

## Cache TTL and pruning impact

Provider prompt caching only applies within the cache TTL window. Woodls can
...
Anthropic's 1M context window is currently beta-gated. Woodls can inject the
