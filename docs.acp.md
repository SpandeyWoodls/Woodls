...
This document describes how the Woodls ACP (Agent Client Protocol) bridge works,
how it maps ACP sessions to Gateway sessions, and how IDEs should invoke it.

## Overview

`woodls acp` exposes an ACP agent over stdio and forwards prompts to a running
Woodls Gateway over WebSocket. It keeps ACP session ids mapped to Gateway
...
drive a Woodls Gateway session.

Quick steps:

1. Run a Gateway (local or remote).
2. Configure the Gateway target (`gateway.remote.url` + auth) or pass flags.
3. Point the IDE to run `woodls acp` over stdio.

Example config:

```bash
woodls config set gateway.remote.url wss://gateway-host:18789
woodls config set gateway.remote.token <token>
```

Example run:

```bash
woodls acp --url wss://gateway-host:18789 --token <token>
```
...
```bash
woodls acp --session agent:main:main
woodls acp --session agent:design:main
woodls acp --session agent:qa:bug-123
```
...
In Zed, open the Agent panel and select “Woodls ACP” to start a thread.

## Execution Model

- ACP client spawns `woodls acp` and speaks ACP messages over stdio.
...
```bash
woodls acp --session agent:main:main
woodls acp --session-label "support inbox"
woodls acp --reset-session
```
...
`woodls acp` resolves the Gateway URL and auth from CLI flags or config:
